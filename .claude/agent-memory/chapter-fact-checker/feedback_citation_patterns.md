...
**`sedra-smith-2014` misuse in EV chapters (confirmed Ch.41):** Sedra/Smith *Microelectronic Circuits* covers BJT/MOSFET amplifier topologies, small-signal models, ideal op-amp golden rules, and basic switched-mode intro. It does NOT cover: BMS architectures, automotive isolated DC-DC design, SiC MOSFET traction-inverter efficiency specs, or EV cell-balancing strategies. In Ch.41 it was misused for BMS monitoring (line 288), isolated DC-DC design (line 398), SiC efficiency specs (line 468), and SiC R_DS(on) / switching-speed claims (line 1363). Correct source for those is `erickson-maksimovic-2020`.

**`horowitz-hill-2015` as EV/battery catch-all (confirmed Ch.41):** H&H is overused even more severely in Ch.41 than in earlier chapters — it was cited for NMC chemistry development, passive BMS balancing, cold-weather R_int rise, solid-state 500 Wh/kg spec, cold-weather range figures, and calendar/cycle battery aging. None of these are topics H&H covers. Correct sources: `linden-reddy-2011` for all battery-spec and aging claims; `erickson-maksimovic-2020` for converter/inverter design. **Pattern flag:** when `horowitz-hill-2015` appears more than once in a chapter that is primarily about batteries or EVs, treat every instance as suspect until verified.

**How to apply:** When auditing any chapter, check: (1) every `feynman-II-*` cite matches the correct Feynman chapter topic AND is in the page's sources array, (2) `gauss-1813` is only for the pure divergence theorem, (3) `codata-2018` is only for CODATA constants — not for P=VI arithmetic or derived load currents, (4) `griffiths-2017` is not a catch-all for measured/experimental values, (5) material conductivity orderings are verified against CRC values, (6) `kanthal` only backs nichrome/NiCr claims, not manganin claims, (7) `feynman-II-2` is never the right cite for energy-in-field, (8) `horowitz-hill-2015` is not a battery energy-density, EV/BMS, or clinical-specs reference — for all of those use `linden-reddy-2011` or `erickson-maksimovic-2020`, (9) `maxwell-1873` is only for mesh-current/network analysis topics, NOT for displacement current or quasi-static/antenna boundary, (10) `ul-498` is for plug/receptacle geometry and temperature-rise only, NOT for GFCI trip threshold or end-of-life electronics (use `ul-943`), (11) `sedra-smith-2014` is for BJT/MOSFET amplifier circuit analysis only — NOT for BMS, isolated DC-DC, SiC inverters, or automotive electronics.
