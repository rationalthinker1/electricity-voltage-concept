...
export function Readout({ sym, label, value, unit, highlight }: ReadoutProps) {
  return (
    <div
      className={`gap-md flex items-baseline justify-between ${highlight ? 'bg-accent-soft -mx-2xl my-md py-lg px-2xl border-0' : 'py-lg border-border border-b last:border-b-0'}`}
    >
      <span className={`font-1 text-4 ${highlight ? 'text-accent' : 'text-text-dim'}`}>
        <span className={`font-2 text-6 mr-sm italic ${highlight ? 'text-accent' : 'text-teal'}`}>
          {sym}
        </span>
        {label}
      </span>
      <span
        className={`font-3 text-right tracking-normal whitespace-nowrap ${highlight ? 'text-accent text-6' : 'text-text text-5'}`}
      >
        {value ?? '—'}
