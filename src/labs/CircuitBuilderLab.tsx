...
import { buildNodeMap, eng, makeContext, pkey, resetContext, step } from './circuit-builder/solver';
import type {
  CircuitDoc,
  ComponentKind,
  GridPoint,
...
export default function CircuitBuilderLab() {
  ...
  const structureSig = useMemo(() => {
    return (
      doc.components
        .map((c) => c.id + ':' + c.kind + ':' + c.x + ',' + c.y + ':' + c.rotation)
        .join('|') +
      '||' +
      doc.wires
        .map((w) => w.id + ':' + w.from.x + ',' + w.from.y + '->' + w.to.x + ',' + w.to.y)
        .join('|')
    );
          ...
          if (resultLatest.t - lastSampleT >= SCOPE_WINDOW / 200) {
            ...
            if (probe?.at) {
              ...
              if (idx !== undefined) {
                ...
                while (scope.length > 0 && resultLatest.t - scope[0].t > SCOPE_WINDOW)
                  scope.shift();
              }
            }
  ...
  const placeWire = useCallback((from: GridPoint, to: GridPoint) => {
    setDoc((prev) => ({
      ...
      wires: [
        ...prev.wires,
        { id: `w${Date.now()}-${Math.floor(Math.random() * 9999)}`, from, to },
      ],
    }));
  }, []);
  ...
  const placeVoltProbe = useCallback((at: GridPoint) => {
    setDoc((prev) => ({
      ...
      probes: [
        ...prev.probes,
        { id: `p${Date.now()}-${Math.floor(Math.random() * 9999)}`, kind: 'voltmeter', at },
      ],
    }));
    setArmed(null);
  }, []);
  ...
  const placeAmmProbe = useCallback((componentId: string) => {
    setDoc((prev) => ({
      ...
      probes: [
        ...prev.probes,
        { id: `p${Date.now()}-${Math.floor(Math.random() * 9999)}`, kind: 'ammeter', componentId },
      ],
    }));
    setArmed(null);
  }, []);
    ...
    <div className="gap-lg mt-md flex flex-col">
      <div className="gap-lg pb-md border-border flex flex-wrap items-center justify-between border-b">
        <div className="gap-sm flex flex-wrap items-center">
          <span className="font-3 text-1 text-text-muted mr-xs tracking-[.2em] uppercase">
            Presets:
          </span>
          {PRESETS.map((p) => (
            <button
        ...
        <div className="gap-sm flex flex-wrap items-center">
          <button
            type="button"
            className={
              'font-3 text-2 px-md rounded-2 duration-fast bg-bg-card hover:bg-bg-card-hover cursor-pointer border py-[6px] transition-all ' +
              (running
          ...
          <button
            type="button"
            className="bg-bg-card text-text-muted border-border font-3 text-2 px-md rounded-2 duration-fast hover:text-pink hover:border-pink hover:bg-bg-card-hover cursor-pointer border py-[6px] transition-all"
            onClick={clearAll}
          >
            Clear
          </button>
      ...
      <div className="grid grid-cols-[220px_1fr_260px] items-start gap-[20px] max-xl:grid-cols-1">
        <aside className="bg-bg-card border-border rounded-3 p-lg border">
          ...
          <div className="bg-bg-card border-border rounded-3 p-lg border">
            <div className="gap-xl flex flex-wrap">
              <div className="min-w-[220px] flex-1">
                <div className="font-3 text-1 text-accent mb-sm tracking-[.2em] uppercase">
                  Probes
                </div>
                {readouts.length === 0 && (
                ...
                {readouts.map((r) => (
                  <div
                    key={r.id}
                    className="gap-md border-border text-3 flex justify-between border-b border-dashed py-[5px] last:border-b-0"
                  >
              ...
              <div className="min-w-[220px] flex-1">
                ...
                <div className="gap-md border-border text-3 flex justify-between border-b border-dashed py-[5px] last:border-b-0">
                  <span className="text-text-dim font-1">Components / wires</span>
                  <span className="text-text font-3 font-medium">
                    {summary.components} / {summary.wires}
                  </span>
                </div>
                <div className="gap-md border-border text-3 flex justify-between border-b border-dashed py-[5px] last:border-b-0">
                ...
                <div className="gap-md border-border text-3 flex justify-between border-b border-dashed py-[5px] last:border-b-0">
                  ...
                  <span
                    ...
                    style={{ color: summary.hasGround ? 'var(--teal)' : 'var(--pink)' }}
                  >
                ...
                <div className="gap-md border-border text-3 flex justify-between border-b border-dashed py-[5px] last:border-b-0">
                  ...
                  <span
                    className="text-text font-3 font-medium"
                    style={{ color: summary.ok ? 'var(--teal)' : 'var(--pink)' }}
                  >
  ...
  const prose = (
    <>
      ...
      <p className="mb-prose-3">
        Everything left of this paragraph runs through the same machinery that powers SPICE —
        Modified Nodal Analysis, formulated in its current matrix-stamping form by Ho, Ruehli, and
        Brennan in 1975
        <Cite id="ho-ruehli-brennan-1975" in={SOURCES} />. The trick is to take Kirchhoff's current
        law
        <Cite id="kirchhoff-1845" in={SOURCES} /> — sum of currents into every node is zero — and
        write it as a matrix equation
      </p>
      <MathBlock>G · v = i</MathBlock>
      <p className="mb-prose-3">
        with one row per node. Each resistor of value{' '}
        <strong className="text-text font-medium">R</strong> contributes a conductance{' '}
        <strong className="text-text font-medium">1/R</strong> to four entries of{' '}
        <strong className="text-text font-medium">G</strong>:{' '}
        <em className="text-text italic">+1/R</em> on each diagonal at its two pin nodes, and{' '}
        <em className="text-text italic">−1/R</em> on the two off-diagonals between them. Stamp
        every passive component, solve, and you have the operating point.
      </p>

      <h3 className="lab-section-h3">Voltage sources need an extra unknown</h3>
      <p className="mb-prose-3">
        A battery between nodes <em className="text-text italic">i</em> and{' '}
        <em className="text-text italic">j</em> isn't a conductance — it commits to a voltage
        difference no matter what current flows. The MNA fix is to add a row and a column for the
        branch current through that source. The new row says
        <strong className="text-text font-medium">
          {' '}
          v<sub>i</sub> − v<sub>j</sub> = V<sub>src</sub>
        </strong>
        ; the new column tells the other rows how that current enters and leaves the connected
        nodes. The matrix gets one entry larger per voltage source, which is why the technique is
        called <em className="text-text italic">modified</em> nodal analysis
        <Cite id="nilsson-riedel-2018" in={SOURCES} />.
      </p>
      <Pullout>
        Every passive component is a contribution to G. Every voltage source buys a new row and
        column. That is the whole secret.
      </Pullout>

      <h3 className="lab-section-h3">Capacitors and inductors: companion models</h3>
      <p className="mb-prose-3">
        At a single instant, a capacitor has whatever voltage it has — and lets through whatever
        current it pleases. The clever idea (trapezoidal rule, Nilsson–Riedel §8) is to discretise
        the time derivative and treat the cap as if it were a small resistor in parallel with an
        ideal current source whose value is set by the state at the previous step:
      </p>
      <MathBlock>
        i<sub>n+1</sub> = (2C/Δt) (v<sub>n+1</sub> − v<sub>n</sub>) − i<sub>n</sub>
      </MathBlock>
      <p className="mb-prose-3">
        Rearranged to expose the linear part, the cap contributes a conductance{' '}
        <strong className="text-text font-medium">2C/Δt</strong> and the right-hand side gets a
        "memory" current of{' '}
        <strong className="text-text font-medium">
          2C v<sub>n</sub>/Δt + i<sub>n</sub>
        </strong>
        . Inductors get the dual treatment: an effective resistance{' '}
        <strong className="text-text font-medium">2L/Δt</strong> in series with a voltage source{' '}
        <strong className="text-text font-medium">
          v<sub>n</sub> + (2L/Δt) i<sub>n</sub>
        </strong>
      ...
      <p className="mb-prose-3">
        The full Shockley equation{' '}
        <strong className="text-text font-medium">
          I = I<sub>s</sub>(e<sup>qV/kT</sup> − 1)
        </strong>{' '}
        is nonlinear and would require Newton–Raphson iteration
        <Cite id="shockley-1949" in={SOURCES} />. We use a textbook simplification: assume the diode
        is either fully on (
        <strong className="text-text font-medium">
          V<sub>F</sub> = 0.7 V
        </strong>{' '}
        drop in series with a tiny 0.05 Ω resistor) or fully off (~1 MΩ). Per step we solve assuming
        the previous state, check whether the answer is consistent (off diodes have V &lt; 0.7 V; on
        diodes carry positive current), and flip up to five times until it stabilises. For the
        half-wave rectifier preset that converges every step on the first or second try
        <Cite id="horowitz-hill-2015" in={SOURCES} />.
      </p>

      <h3 className="lab-section-h3">AC sources, switches, bulbs</h3>
      <p className="mb-prose-3">
        AC sources are just batteries whose value is recomputed each step as{' '}
        <strong className="text-text font-medium">
          V(t) = V<sub>pk</sub> sin(2π f t)
        </strong>{' '}
        — time-domain integration is enough; we don't need a separate phasor solver. Switches are
        resistors that swap between 1 mΩ and 1 GΩ depending on state. Bulbs are resistors that glow
        at intensity proportional to <strong className="text-text font-medium">I²R</strong> — the
        same dissipation Joule measured in 1841 (Lab 3.4).
      </p>

      <h3 className="lab-section-h3">Try it</h3>
      <p className="mb-prose-3">
        Load the <strong className="text-text font-medium">RC Charging</strong> preset and watch the
        scope trace climb toward 5 V on a 1 ms time constant. Load{' '}
        <strong className="text-text font-medium">RLC Resonator</strong> and watch the loop voltage
        trade between L and C at the resonant frequency
        <strong className="text-text font-medium"> ω = 1/√(LC) ≈ 3.18 kHz</strong>. Load{' '}
        <strong className="text-text font-medium">Half-Wave Rectifier</strong> and see the negative
        half of the sine clipped off below the diode's forward drop.
      </p>
      <p className="mb-prose-3">
        Then build your own: drop a battery, two resistors, a ground, wire them in a divider, and
        confirm{' '}
        <strong className="text-text font-medium">
...
function PresetInfo({ preset }: { preset: CircuitPreset | null }) {
  ...
  return (
    <div className="mt-lg pt-lg border-border flex max-h-[520px] flex-col gap-[14px] overflow-y-auto border-t">
      <div className="font-3 text-accent text-[9px] tracking-[.25em] uppercase">{preset.topic}</div>
      <div className="font-2 text-text text-[18px] leading-2 italic">{preset.name}</div>
      <div className="text-3 text-text-dim py-sm bg-bg-elevated rounded-2 border-border border px-[10px] leading-[1.5]">
        <strong>Goal:</strong> {preset.goal}
      </div>
      <div className="gap-sm flex flex-col">
        <div className="font-3 text-1 text-teal tracking-[.15em] uppercase">Theory</div>
        {preset.theory.split('\n\n').map((para, i) => (
          <p key={i} className="text-3 text-text-dim m-0 leading-[1.55]">
            {para}
          </p>
        ))}
      </div>
      <div className="gap-sm flex flex-col">
        <div className="font-3 text-1 text-teal tracking-[.15em] uppercase">Formulas</div>
        {preset.formulas.map((f, i) => (
          <div
            key={i}
            className="font-4 text-4 text-text bg-bg-elevated rounded-2 border-border border px-[10px] py-[6px] leading-[1.5] italic"
          >
            {f}
          </div>
        ))}
      ...
      <div className="gap-sm flex flex-col">
        <div className="font-3 text-1 text-teal tracking-[.15em] uppercase">Calculation</div>
        <ol className="text-2 text-text-dim [&>li::marker]:text-accent m-0 pl-[18px] leading-[1.55] [&>li]:mb-[5px]">
      ...
      <div className="gap-sm flex flex-col">
        <div className="font-3 text-1 text-teal tracking-[.15em] uppercase">Try It</div>
        <ul className="text-2 text-text-dim [&>li::marker]:text-accent m-0 pl-[18px] leading-[1.55] [&>li]:mb-[5px]">
