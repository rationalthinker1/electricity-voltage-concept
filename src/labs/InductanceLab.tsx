...
export default function InductanceLab() {
  ...
  const prose = (
    <>
      ...
      <p>
        <strong>L</strong> self-inductance (henries). <strong>µ<sub>0</sub></strong> = 4π×10⁻⁷ T·m/A, the permeability of free space<Cite id="codata-2018" in={SOURCES} />.
        <strong>N</strong> number of turns, <strong>A</strong> cross-sectional area of the coil, <strong>ℓ</strong> solenoid length.
        <strong>V</strong> the induced back-EMF when current changes at rate <strong>dI/dt</strong>. <strong>U</strong> stored energy at current I.
      </p>
      ...
      <p>
        ...
        the circuit. The coil <em>fights</em> any change in its current by generating whatever voltage is needed to keep electrons flowing
      ...
      <Pullout>
        An inductor is a flywheel for electricity. It hates the <em>new</em> current. It loves the current you already gave it.
      </Pullout>

      <h3>Reasoning</h3>
      <p>
        Why <strong>N²</strong>, not just N? Doubling the turns does two things: it doubles the magnetic field (B = µ<sub>0</sub>nI scales
      ...
      <p>
        Why <strong>A</strong> on top? Larger cross-section means more area for each loop to link, so more flux per turn.
        Why <strong>ℓ</strong> on the bottom? A longer solenoid at fixed N has fewer turns per unit length, so the field inside is weaker.
      ...
      <p>
        ...
        leg contributes, and the enclosed current is <strong>NI/ℓ × ℓ<sub>loop</sub></strong><Cite id="griffiths-2017" in={SOURCES} />:
      </p>
      <Formula>B = µ<sub>0</sub> n I = µ<sub>0</sub> (N/ℓ) I</Formula>
      <p>Step two — the flux through one turn:</p>
      <Formula>Φ = B A = µ<sub>0</sub> (N/ℓ) I A</Formula>
      <p>Step three — the total flux linkage. All N turns thread the same Φ:</p>
      <Formula>N Φ = µ<sub>0</sub> N² A I / ℓ</Formula>
      <p>Step four — by definition, L is flux linkage per unit current:</p>
      ...
      <p>
        Equivalently, the energy density of the field is <strong>u<sub>B</sub> = B²/(2µ<sub>0</sub>)</strong>, and multiplying by the
      ...
      <TryIt
        tag="Problem 4.2.1"
        question={<>An air-core solenoid: <strong>ℓ = 10 cm</strong>, <strong>N = 500</strong>, <strong>A = 1 cm²</strong>. What is L?</>}
        answer={
          <>
            <Formula>L = µ<sub>0</sub> N² A / ℓ = (4π×10⁻⁷)(500²)(10⁻⁴) / (0.10)</Formula>
            <Formula>L = (4π×10⁻⁷)(2.5×10⁵)(10⁻³) ≈ <strong>3.14×10⁻⁴ H</strong> ≈ 314 µH</Formula>
            <p>A few hundred microhenries — a typical RF choke value<Cite id="griffiths-2017" in={SOURCES} />.</p>
          </>
        }
      />
      ...
      <TryIt
        tag="Problem 4.2.2"
        question={<>Same solenoid, now wound on an iron core with <strong>µ<sub>r</sub> ≈ 1000</strong>. New L?</>}
        hint="Iron magnifies µ. Below saturation, L scales linearly."
        answer={
          <>
            <Formula>L<sub>iron</sub> = µ<sub>r</sub> L<sub>air</sub> = 1000 × 314 µH = <strong>0.314 H</strong></Formula>
            <p>Three orders of magnitude bigger for the same geometry. This is why iron is in every transformer — until the core saturates
      ...
      <TryIt
        tag="Problem 4.2.3"
        question={<>Current through a <strong>50 mH</strong> inductor is ramping at <strong>10 A/s</strong>. What is the induced EMF?</>}
        answer={
          <>
            <Formula>|V| = L |dI/dt| = (50×10⁻³ H)(10 A/s) = <strong>0.5 V</strong></Formula>
            <p>Half a volt of back-EMF opposes the change — a steady, modest push that says "no, you don't get to ramp current any faster
      ...
      <TryIt
        tag="Problem 4.2.4"
        question={<>A series RL circuit with <strong>L = 10 mH</strong> and <strong>R = 100 Ω</strong> is suddenly connected to a battery.
          What is the time constant? What fraction of the steady-state current flows at <strong>t = τ</strong>?</>}
        answer={
          <>
            <Formula>τ = L / R = (10×10⁻³ H) / (100 Ω) = 10⁻⁴ s = <strong>100 µs</strong></Formula>
            <p>At t = τ, the current has risen to (1 − e⁻¹) ≈ <strong>63.2%</strong> of its final value.</p>
            <Formula>I(τ) = I<sub>∞</sub> · (1 − e⁻¹) ≈ 0.632 I<sub>∞</sub></Formula>
            <p>RL circuits parallel RC in form, with the roles of voltage and current swapped<Cite id="griffiths-2017" in={SOURCES} />.</p>
          </>
        }
      />
      ...
      <TryIt
        tag="Problem 4.2.5"
        question={<>Energy stored in a <strong>10 mH</strong> inductor carrying <strong>2 A</strong>?</>}
        answer={
          <>
            <Formula>U = ½ L I² = ½ (10×10⁻³)(4) = <strong>0.02 J</strong> = 20 mJ</Formula>
            <p>Modest energy — but try to interrupt that 2 A in a microsecond and you'll need to dissipate it somewhere. dI/dt = −2×10⁶ A/s
            implies a voltage spike of L · |dI/dt| = (10 mH)(2×10⁶) = <strong>20,000 V</strong> across whatever opens the circuit.
      ...
      <TryIt
        tag="Problem 4.2.6"
        question={<>Two coils share a common core with ideal coupling (<strong>k = 1</strong>). Primary: N<sub>1</sub> = 100 turns, L<sub>1</sub> = 10 mH.
          Secondary: N<sub>2</sub> = 10 turns. Find L<sub>2</sub> and the mutual inductance M.</>}
        hint="L scales as N². For ideal coupling, M = √(L₁L₂)."
        answer={
          <>
            <Formula>L<sub>2</sub> = L<sub>1</sub> (N<sub>2</sub>/N<sub>1</sub>)² = 10 mH × (1/10)² = <strong>0.1 mH</strong></Formula>
            <Formula>M = √(L<sub>1</sub> L<sub>2</sub>) = √(10⁻² · 10⁻⁴) = <strong>10⁻³ H = 1 mH</strong></Formula>
            <p>The mutual coupling matters more than either self-inductance for transformer action<Cite id="jackson-1999" in={SOURCES} />.</p>
          </>
        }
      />
      ...
      <TryIt
        tag="Problem 4.2.7"
        question={<>An ideal transformer with turns ratio <strong>1000 : 100</strong>. If primary voltage is 120 V<sub>rms</sub> at primary
        ...
        answer={
          <>
            <Formula>V<sub>2</sub> = V<sub>1</sub> (N<sub>2</sub>/N<sub>1</sub>) = 120 (100/1000) = <strong>12 V</strong></Formula>
            <Formula>I<sub>2</sub> = I<sub>1</sub> (N<sub>1</sub>/N<sub>2</sub>) = 0.5 × 10 = <strong>5 A</strong></Formula>
            <p>Power check: P<sub>1</sub> = 120 × 0.5 = 60 W, P<sub>2</sub> = 12 × 5 = 60 W. Transformers step voltage up or down at fixed
      ...
      <TryIt
        tag="Problem 4.2.8"
        question={<>An RL circuit reaches <strong>99%</strong> of its final current. How many time constants τ have passed?</>}
        answer={
          <>
            <p>Set I(t) = 0.99 I<sub>∞</sub> in I(t) = I<sub>∞</sub>(1 − e<sup>−t/τ</sup>):</p>
            <Formula>0.99 = 1 − e<sup>−t/τ</sup> &nbsp;⇒&nbsp; e<sup>−t/τ</sup> = 0.01 &nbsp;⇒&nbsp; t/τ = ln 100 ≈ <strong>4.6 τ</strong></Formula>
            <p>Rule of thumb: five time constants is "fully settled" to within a fraction of a percent.</p>
          </>
        }
      />
      ...
      <TryIt
        tag="Problem 4.2.9"
        question={<>A flyback transformer with primary <strong>L = 1 mH</strong> switches at <strong>f = 100 kHz</strong>, peak current
          <strong> I = 10 A</strong>. Energy stored per cycle? Average power delivered?</>}
        answer={
          <>
            <Formula>U<sub>peak</sub> = ½ L I² = ½ (10⁻³)(100) = <strong>0.05 J = 50 mJ</strong></Formula>
            <Formula>P = U · f = (0.05)(10⁵) = <strong>5000 W = 5 kW</strong></Formula>
            <p>Each switching cycle, the primary builds up 50 mJ in its B field, then dumps it (through magnetic coupling) into the secondary.
      ...
      <TryIt
        tag="Problem 4.2.10"
        question={<>Why does an inductor "kick" hard when current is suddenly interrupted? Estimate the voltage spike if a <strong>10 mH</strong>
          inductor's current drops at <strong>dI/dt = −10⁶ A/s</strong>.</>}
        answer={
          <>
            <Formula>|V<sub>spike</sub>| = L |dI/dt| = (10⁻²)(10⁶) = <strong>10,000 V = 10 kV</strong></Formula>
            <p>Ten kilovolts. The inductor refuses to let its stored magnetic energy go to zero faster than the circuit will permit. If the
      ...
      <TryIt
        ...
        answer={
          <>
            ...
            <Formula>f<sub>0</sub> = ω<sub>0</sub>/(2π) ≈ <strong>5.0 kHz</strong></Formula>
            <p>At this frequency, energy oscillates back and forth between the cap's electric field and the inductor's magnetic field, with
