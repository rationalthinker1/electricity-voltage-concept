  ...
  return (
    <ChapterShell chapter={chapter}>
      <p className="chapter-intro">
        We have spent nine chapters insisting that electricity is a story about{' '}
        <em className="text-text italic">fields</em>: E and B filling space, energy flowing through
        the Poynting vector, signals propagating at c, a current-carrying wire is just a
      ...
      <p className="mb-prose-3">
        ...
        compression from "field theory" to "schematic" is one of the most successful idealizations
        in physics — and this chapter is about when it's allowed, and what it gives you.
      </p>
      ...
      <p className="mb-prose-3">
        ...
        <Cite id="horowitz-hill-2015" in={SOURCES} />. Currents and voltages have unambiguous
        instantaneous values at every node — no propagation delay, no field structure, no spatial
      ...
      <p className="mb-prose-3">
        ...
        it sees the same instantaneous voltage to within a part in a million. At 2.4 GHz, λ ≈ 12 cm;
        the lumped picture breaks down across your phone's antenna and you have to treat the antenna
        as a distributed object — a transmission line, an electromagnetic resonator, an{' '}
      ...
      <p className="mb-prose-3">
        ...
        impedance, filters, transformers — every working piece of analog electronics — falls out of
        those two conservation laws and the constitutive relations of three components.
      </p>
      ...
      <p className="mb-prose-3">
          ...
          <Term
            def={
              <>
                ...
                <em className="text-text italic">Σ I = 0</em>. A statement of charge conservation in
        ...
        At any node in a circuit, the sum of currents flowing in equals the sum flowing out. This is
        charge conservation. Charge doesn't pile up in a wire — every electron that enters a
        junction has to leave by some other branch in the same instant. Algebraically:
      </p>
      <Formula>
      ...
      <p className="mb-prose-3">
          ...
          <Term
            def={
              <>
                ...
                <em className="text-text italic">Σ V = 0</em>. A statement of energy conservation in
                the lumped-element limit.
              </>
            }
          >
        ...
        Around any closed loop, the algebraic sum of voltage rises and drops is zero. This is energy
        conservation. Voltage is potential energy per unit charge; if a test charge made a round
        trip through a loop and didn't end up with the same potential energy it started with, you'd
        have an energy source for free.
      </p>
      <Formula>
        Σ V<sub>loop</sub> = 0
      </Formula>
      ...
      <p className="mb-prose-3">
        ...
        limit. KCL is what ∇·J = −∂ρ/∂t reduces to when no charge can accumulate (ρ doesn't change
        at a node). KVL is what ∮E·dℓ = 0 reduces to when the time-varying flux through the loop is
        negligible. The schematic inherits the conservation laws of the underlying field theory.
      </p>
      ...
      <p className="mb-prose-3">
        ...
        microcontroller's input range, and it is the first thing you draw on the back of an envelope
      ...
      <p className="mb-prose-3">
        ...
        nominal V<sub>in</sub>/2 midpoint, tapped off two 100 kΩ resistors, collapses appreciably
      ...
      <TryIt
        ...
        hint={
          <>
            Unloaded: V<sub>out</sub> = V<sub>in</sub> R<sub>2</sub>/(R<sub>1</sub>+R<sub>2</sub>).
            Loaded: replace R<sub>2</sub> with R<sub>2</sub> ∥ R<sub>L</sub>.
          </>
        }
        answer={
          <>
            <Formula>
              V<sub>out, no load</sub> = 9 · 2/(1+2) ={' '}
              <strong className="text-text font-medium">6.0 V</strong>
            </Formula>
            <p className="mb-prose-1 last:mb-0">
              With the 6 kΩ load in place, the lower leg becomes
            </p>
            <Formula>
              R<sub>2</sub> ∥ R<sub>L</sub> = (2·6)/(2+6) = 1.5 kΩ
            </Formula>
            <Formula>
              V<sub>out, loaded</sub> = 9 · 1.5/(1+1.5) ={' '}
              <strong className="text-text font-medium">5.4 V</strong>
            </Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <p className="mb-prose-3">
        ...
        <Cite id="keysight-34465a-datasheet" in={SOURCES} />) so that almost no current is drawn
        from the circuit under test — the meter reads the node voltage without perturbing it. The
      ...
      <p className="mb-prose-3">
        ...
        <Cite id="keysight-34465a-datasheet" in={SOURCES} />. For a sinusoid of peak V<sub>p</sub>,
        the answer is V<sub>p</sub>/√2; for a pure DC level, V_AC reads zero. That's the right
      ...
      <p className="mb-prose-3">
        ...
        <Cite id="horowitz-hill-2015" in={SOURCES} />. You have to break the wire and let the
        current pass through the meter — putting the probes on two arbitrary TPs doesn't give a
        current reading, because there's no series path through the meter. In this demo we cheat
      ...
      <p className="mb-prose-3">
        ...
        here we model by mentally shorting the battery and removing the (open at DC) capacitor. The
        reading is then the equivalent Thévenin resistance of the rest of the network seen between
        the two probe tips.
      </p>
      ...
      <p className="mb-prose-3">
        ...
        sits at ground potential. Switch to Ω mode and probe between TP1 and GND: the meter sees R₁
        in parallel with (R₂ + R₃), about 339 Ω, exactly the value you'd compute by hand. The
        schematic is doing real predictive work.
      </p>
      ...
      <p className="mb-prose-3">
        ...
        <Cite id="horowitz-hill-2015" in={SOURCES} />. This is the timing element behind every
        digital pulse-width-modulation controller, every radio receiver's de-coupling stage, every
        camera flash. The 555 timer chip is built around exactly this exponential.
      </p>
      ...
      <TryIt
        ...
        question={
          <>
            ...
            <strong className="text-text font-medium">169.7 V</strong>?
          </>
        }
        hint={
          <>
            For a pure sine, V<sub>rms</sub> = V<sub>p</sub>/√2.
          </>
        }
        answer={
          <>
            <Formula>
              V<sub>rms</sub> = 169.7 V / √2 ≈ 120.0 V
            </Formula>
            <p className="mb-prose-1 last:mb-0">
              That is exactly the North American nominal:{' '}
              <strong className="text-text font-medium">
                120 V<sub>rms</sub>
              </strong>
              , which is why wall outlets are rated for ≈170 V peak insulation even though "120 V"
      ...
      <p className="mb-prose-3">
        ...
        total U<sub>C</sub> + U<sub>L</sub> stays constant
        <Cite id="griffiths-2017" in={SOURCES} />.
      </p>
      ...
      <PredictThenObserve
        ...
        question={
          <>
            An <em className="text-text italic">ideal</em> LC tank (no resistance) is started with
            the capacitor fully charged. What happens?
          </>
        }
        spec={{
          kind: 'multiple-choice',
      ...
      <TryIt
        ...
        question={
          <>
            ...
            <em className="text-text italic">L</em> = 10 mH and{' '}
            <em className="text-text italic">C</em> = 10 µF.
          </>
        }
        hint={<>f₀ = 1 / (2π √(L C)).</>}
        answer={
          <>
            ...
            <Formula>
              f<sub>0</sub> = 1 / (2π · 3.162×10⁻⁴) ≈ 503 Hz
            </Formula>
            <p className="mb-prose-1 last:mb-0">
              Resonant frequency <strong className="text-text font-medium">≈ 503 Hz</strong>. Drop
              either L or C by 100× and f₀ scales up by 10×
              <Cite id="horowitz-hill-2015" in={SOURCES} />
              <Cite id="irwin-circuit-analysis-2015" in={SOURCES} />.
            </p>
          </>
      ...
      <p className="mb-prose-3">
        ...
        cycle), and the system responds preferentially to one driving frequency —
        <strong className="text-text font-medium">
          {' '}
          <Term
            def={
              <>
                ...
                <em className="text-text italic">ω₀ = 1/√(LC)</em>, where the inductive and
          ...
          <Term
            def={
              <>
                ...
                . For a series RLC, <em className="text-text italic">Q = ω₀L/R = (1/R)√(L/C)</em>.
      ...
      <PredictThenObserve
        ...
        question={
          <>
            A <em className="text-text italic">series</em> RLC circuit is driven by an AC source at
            its resonant frequency. The current is …
          </>
        }
        spec={{
          kind: 'multiple-choice',
      ...
      <TryIt
        ...
        question={
          <>
            For the series RLC with <em className="text-text italic">R</em> = 1 Ω,{' '}
            <em className="text-text italic">L</em> = 10 mH, <em className="text-text italic">C</em>{' '}
            = 10 µF, compute the quality factor Q.
          </>
        }
        hint={<>Q = (1/R) √(L/C).</>}
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              Q ≈ <strong className="text-text font-medium">31.6</strong> — sharp enough to ring for
              ~10 cycles before losing 1/e of its energy. Drop R to 0.1 Ω and Q jumps to 316;
              crystal oscillators reach 10⁶ on the same formula
              <Cite id="horowitz-hill-2015" in={SOURCES} />
              <Cite id="irwin-circuit-analysis-2015" in={SOURCES} />.
            </p>
          </>
        ...
        <Term
          def={
            <>
              ...
              Time-derivatives <em className="text-text italic">d/dt</em> become multiplications by{' '}
              <em className="text-text italic">jω</em>, and linear differential equations turn into
              linear algebra in ℂ.
            </>
          }
        >
      ...
      <p className="mb-prose-3">
        ...
        Capacitors are negative-imaginary: current
        <em className="text-text italic"> leads</em> voltage by 90°, because I = C dV/dt
        <Cite id="horowitz-hill-2015" in={SOURCES} />. The frequency-dependent magnitude of the
        imaginary part is the component's{' '}
        <Term
          def={
            <>
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">Z(ω)</strong> is the total complex series
        impedance (in ohms) at angular frequency{' '}
        <strong className="text-text font-medium">ω</strong> (in radians per second),{' '}
        <strong className="text-text font-medium">R</strong> is the resistance (in ohms, the real
        part — dissipative), <strong className="text-text font-medium">L</strong> is the inductance
        (in henries), <strong className="text-text font-medium">C</strong> is the capacitance (in
        farads), and
        <strong className="text-text font-medium"> j = √−1</strong>. The bracketed imaginary part is
        the net reactance, also in ohms. The magnitude <em className="text-text italic">|Z|</em>{' '}
        sets the current amplitude per volt and the argument{' '}
        <em className="text-text italic">∠Z</em> sets the V–I phase shift
        <Cite id="irwin-circuit-analysis-2015" in={SOURCES} />.
      </p>
      ...
      <TryIt
        ...
        question={
          <>
            ...
            capacitor at 60 Hz, and of a <strong className="text-text font-medium">10 mH</strong>{' '}
            inductor at 60 Hz.
          </>
        }
        hint={
          <>
            X<sub>C</sub> = 1/(2π f C); X<sub>L</sub> = 2π f L.
          </>
        }
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              At 60 Hz, the 10 µF cap looks like{' '}
              <strong className="text-text font-medium">≈ 265 Ω</strong> and the 10 mH inductor like{' '}
              <strong className="text-text font-medium">≈ 3.77 Ω</strong>. Bump f to 60 kHz and the
              cap shrinks by 1000× while the inductor grows by 1000× — which is why coupling caps
              and choke inductors swap roles in high-frequency design
              <Cite id="horowitz-hill-2015" in={SOURCES} />
              <Cite id="irwin-circuit-analysis-2015" in={SOURCES} />.
            </p>
          </>
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">⟨P⟩</strong> is the average real power
        ...
        <strong className="text-text font-medium"> φ</strong> is the phase angle (in radians) by
        which the current lags or leads the voltage.{' '}
        <strong className="text-text font-medium">cos(φ)</strong> is the dimensionless power factor.
      </p>
      <p className="mb-prose-3">
        with{' '}
        <Term
          ...
          <Term
            def={
              <>
                ...
                to apparent power, <em className="text-text italic">cos(φ)</em>, where{' '}
                <em className="text-text italic">φ</em> is the voltage–current phase shift. 1 for a
        ...
        . A purely resistive load has φ = 0 and cos(φ) = 1 — every watt of VI is real, useful,
        heat-generating power. A purely reactive load (pure L or pure C) has φ = ±90° and cos(φ) = 0
      ...
      <ul>
        <li>
          ...
          P = V<sub>rms</sub> I<sub>rms</sub> cos(φ), units watts (W). What dissipates as heat or
        ...
        <li>
            ...
            <Term
              def={
                <>
                  ...
                  <em className="text-text italic">
                    Q = V<sub>rms</sub> I<sub>rms</sub> sin(φ)
                  </em>
          ...
          Q = V<sub>rms</sub> I<sub>rms</sub> sin(φ), units VAR (volt-amperes reactive). Sloshes
          between source and load every cycle.
        </li>
        <li>
          <strong className="text-text font-medium">
            <Term
          ...
          S = V<sub>rms</sub> I<sub>rms</sub>, units VA. The magnitude of the complex power phasor;
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium"> power-factor correction capacitors</strong> to
        cancel the inductive lag of their motor banks
      ...
      <p className="mb-prose-3">
        ...
        inductor), p(t) is symmetric about zero, the time integral is zero, and the source absorbs
        as much energy in one quarter cycle as it delivered in the previous one. Real loads sit
        between the extremes; the power factor cos(φ) tells you what fraction of the volt-ampere
        product translates into useful work
        <Cite id="grainger-power-systems-2003" in={SOURCES} />.
      </p>
      ...
      <TryIt
        ...
        question={
          <>
            ...
            and <strong className="text-text font-medium">750 VAR</strong> of reactive power at{' '}
            <strong className="text-text font-medium">
              240 V<sub>rms</sub> / 60 Hz
            </strong>
            . What is its power factor? What line current does it draw?
          </>
        }
        hint={
          <>
            S = √(P² + Q²); pf = P/S; I<sub>rms</sub> = S / V<sub>rms</sub>.
          </>
        }
        answer={
          <>
            ...
            <Formula>
              pf = P/S = 1000 / 1250 = <strong className="text-text font-medium">0.80</strong>
            </Formula>
            <Formula>
              I<sub>rms</sub> = S / V<sub>rms</sub> = 1250 / 240 ≈{' '}
              <strong className="text-text font-medium">5.21 A</strong>
            </Formula>
            <p className="mb-prose-1 last:mb-0">
              The wires actually carry 5.21 A even though only 4.17 A would suffice for a unity-pf
              load delivering the same 1 kW. Power-factor-correction caps shrink the apparent power
              back toward 1000 VA and the line current toward 4.17 A
              <Cite id="grainger-power-systems-2003" in={SOURCES} />.
            </p>
          </>
        }
      ...
      <TryIt
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              Real power <strong className="text-text font-medium">= 700 W</strong>; reactive power{' '}
              <strong className="text-text font-medium">≈ 714 VAR</strong>. The wires still carry
              the full 1000 VA, which is why utilities bill heavy-industry customers for apparent
              power and why power-factor-correction capacitors pay for themselves so quickly
              <Cite id="grainger-power-systems-2003" in={SOURCES} />.
            </p>
          </>
        }
      ...
      <p className="mb-prose-3">
        ...
        , 120° apart in time. Sum the three voltages and you get zero at every instant, which means
        a balanced three-phase load needs no neutral return wire. Three conductors carry √3 ≈ 1.73×
        the power of a single-phase line of the same conductor weight, and a three-phase motor
      ...
      <p className="mb-prose-3">
        ...
        the same Maxwell theory we started with, the schematic is the same Kirchhoff circuit we've
        been drawing — but the wiring is shaped by the geometry of three vectors at 120° agreeing to
        add to zero.
      </p>
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium">
          single voltage source V<sub>th</sub> in series with a single resistance R<sub>th</sub>
        </strong>
        . The equivalence is exact for every external load you might connect to those two terminals
        <Cite id="horowitz-hill-2015" in={SOURCES} />
        <Cite id="irwin-circuit-analysis-2015" in={SOURCES} />.
      </p>
      <p className="mb-prose-3">
        ...
        between them — that <em className="text-text italic">open-circuit voltage</em> is V
        <sub>th</sub>. To find{' '}
        <strong className="text-text font-medium">
          R<sub>th</sub>
        </strong>
      ...
      <p className="mb-prose-3">
        ...
        <sub>L</sub>/(R<sub>th</sub>+R<sub>L</sub>). Maximum power transfer happens at R<sub>L</sub>{' '}
        = R<sub>th</sub>, with half of V<sub>th</sub>·I delivered to the load and half wasted in R
        <sub>th</sub>. Every real source — battery, signal generator, op-amp output — has a Thévenin
        equivalent, and you cannot get more power out of it than its V<sub>th</sub>²/(4 R
        <sub>th</sub>) <Cite id="horowitz-hill-2015" in={SOURCES} />.
      </p>
      ...
      <TryIt
        ...
        question={
          <>
            ...
            <strong className="text-text font-medium">1 Ω</strong>. You connect a
            <strong className="text-text font-medium"> 8 Ω</strong> load across its terminals. What
            are V<sub>th</sub>, R<sub>th</sub>, and the voltage seen across the load?
          </>
        }
        hint={
          <>
            The battery's open-circuit voltage is V<sub>th</sub>; its internal resistance is R
            <sub>th</sub>. Then V<sub>load</sub> = V<sub>th</sub> R<sub>L</sub>/(R<sub>th</sub> + R
            <sub>L</sub>).
          </>
        }
        answer={
          <>
            ...
            <Formula>
              V<sub>th</sub> = 9 V, R<sub>th</sub> = 1 Ω
            </Formula>
            <Formula>
              V<sub>load</sub> = 9 · 8 / (1 + 8) = 8.0 V
            </Formula>
            <p className="mb-prose-1 last:mb-0">
              The load drops <strong className="text-text font-medium">8.0 V</strong>; the remaining
              1.0 V is wasted inside the battery. That 1 Ω is why an alkaline cell can't crank a car
              starter motor — the starter wants 100 A, which would force 100 V across the internal
              resistance and the cell collapses
              <Cite id="horowitz-hill-2015" in={SOURCES} />.
            </p>
          </>
        }
      ...
      <CaseStudies intro="Three places where the lumped, complex-impedance picture of this chapter does the heavy lifting in working hardware.">
        <CaseStudy
          ...
          summary={
            <>
              60 Hz three-phase, from a 120/240 V wall outlet to a 765 kV transmission line — one
              network, six orders of magnitude in voltage.
            </>
          }
          specs={[
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            <strong className="text-text font-medium">
              {' '}
              P<sub>loss</sub> = (P<sub>load</sub>/V)² R
            </strong>
            ...
            than the towers holding them up.
          </p>
          <p className="mb-prose-2 last:mb-0">
            Three-phase is the second trick. Three voltages 120° apart, summed at a balanced load,
            cancel exactly — no neutral return current required. Three conductors then carry √3
            times the power of a single-phase pair of the same weight, and a three-phase motor sees
            a magnetic field that rotates automatically at synchronous speed, no commutator or
          ...
          <p className="mb-prose-2 last:mb-0">
            ...
            transformer core volume scales roughly as 1/<em className="text-text italic">f</em> for
        ...
        <CaseStudy
          ...
          summary={
            <>
              A capacitor, an inductor, and the entire AM and FM dial — selectivity is just
              resonance.
            </>
          }
          specs={[
        ...
        <CaseStudy
          ...
          summary={
            <>
              A switched-mode supply running at 50–500 kHz beats a 60-Hz linear regulator by an
              order of magnitude in size and a factor of two in efficiency.
            </>
          }
          specs={[
            {
              label: 'Mains input (NA)',
              value: (
                <>
                  120 V<sub>rms</sub>, 60 Hz
                </>
              ),
            },
            ...
            { label: 'Linear regulator efficiency', value: '~50 % (20 V → 5 V)' },
            {
              ...
              value: (
                <>
                  ∝ 1/<em className="text-text italic">f</em>
                </>
              ),
            },
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            then burns the excess voltage as heat across a pass transistor — efficiency is roughly V
            <sub>out</sub>/V<sub>in</sub>, so stepping rectified ~170 V<sub>peak</sub> down to 5 V
          ...
          <p className="mb-prose-2 last:mb-0">
            ...
            1/<em className="text-text italic">f</em> for a given power handling, so jumping from 60
          ...
          <p className="mb-prose-2 last:mb-0">
            ...
            best linear regulators converting from rectified mains
            <Cite id="erickson-maksimovic-2020" in={SOURCES} />. The price is electromagnetic
            complexity: the same 100 kHz square-wave switching edge that miniaturises the
            transformer also produces broadband EMI, which is why every SMPS contains line filters,
            Y-capacitors, and a snubber circuit on the primary side. The schematic abstractions of
            this chapter — Z<sub>L</sub>, Z<sub>C</sub>, power factor — are the working engineer's
            day-to-day tools for taming all of it
            <Cite id="horowitz-hill-2015" in={SOURCES} />.
          </p>
        </CaseStudy>
      </CaseStudies>
      ...
      <FAQ intro="The questions a careful reader asks after meeting a soldering iron for the first time.">
        <FAQItem q="When does the 'lumped element' picture stop being valid?">
          <p>
            When the circuit's spatial extent <strong className="text-text font-medium">ℓ</strong>{' '}
            is no longer small compared to the wavelength{' '}
            <strong className="text-text font-medium">λ = c/f</strong> of any signal in it. A
        ...
        <FAQItem q="Why is the time constant τ = RC and not, say, R/C?">
          <p>
            Dimensional analysis pins it. R is in volts per amp; C is in coulombs per volt; amps are
        ...
        <FAQItem q="Why is reactive power called 'reactive' — what is it doing?">
          <p>
            Storing energy in a field that releases it back to the source half a cycle later. In a
            pure capacitor, the cap charges up over one quarter cycle (drawing real current), then
            discharges back through the source over the next quarter cycle (sourcing real current).
            The time average is zero — no energy actually leaves the source. But the wires still
            carry the current both ways, and the conductors and transformers have to be sized for
            it. "Reactive" because it doesn't do work; it just reacts to the applied field
            <Cite id="grainger-power-systems-2003" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="What is RMS voltage and why divide by √2?">
          <p>
            ...
            of DC. North American household 120 V<sub>rms</sub> has peak voltage ≈ 170 V, which is
            why your appliance insulation has to be rated for the higher number
            <Cite id="horowitz-hill-2015" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="Why does a poorly-designed amplifier 'oscillate'?">
          <p>
            ...
            Every wire is a tiny inductor; every adjacent pair of conductors is a tiny capacitor. If
            the gain stage couples its output back to its input through one of these parasitic
        ...
        <FAQItem q="Is impedance just a complex resistance?">
          <p>
            ...
            steady-state signal, V = IZ is exactly Ohm's law with complex numbers in place of reals.
            The real part absorbs energy; the imaginary part stores and returns it. For
            non-sinusoidal signals (square waves, transients) you'd Fourier-decompose into
            sinusoids, apply Z(ω) component by component, and inverse-Fourier the result — which is
            how every modern circuit simulator works under the hood
            <Cite id="irwin-circuit-analysis-2015" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="Why is impedance matching important?">
          <p>
            ...
            the source's open-circuit power ends up in the load, half is wasted in the source's own
            internal resistance. Mismatched loads either steal too little current (load too heavy in
            Z) or burn too much in the source (load too light). In RF design, mismatched
            transmission-line terminations produce <em className="text-text italic">reflections</em>
            , which is a different and worse failure mode
            <Cite id="horowitz-hill-2015" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="What's a transmission line's characteristic impedance, and how does it differ from a resistor?">
          <p>
            ...
            where L and C are <em className="text-text italic">per unit length</em>. Coax cable runs
            50 or 75 Ω; twisted-pair Ethernet, 100 Ω. Unlike a resistor, Z₀ is purely real but it
            does not dissipate energy — it sets the ratio of voltage to current in a propagating
            wave. Terminate the line in a real resistor equal to Z₀ and the wave is absorbed
            cleanly; mismatch the termination and part of the wave reflects back along the line. The
            lumped model doesn't capture this; the field model does
            <Cite id="griffiths-2017" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="Why do USB chargers use AC then convert to DC?">
          <p>
            Because the power coming out of the wall is AC (so it can be transmitted long distances
            and step-transformed with simple iron-core devices), but USB devices want DC (so they
            can charge a battery and feed silicon logic that has no concept of polarity reversal).
            The charger has a rectifier (AC → pulsed DC), a filter capacitor (pulsed DC → smooth
            DC), and usually a switching regulator (smooth DC at one voltage → smooth DC at
            another). Step-down via a high-frequency transformer is much smaller than at 60 Hz —
            which is why a modern 5 V USB charger is a small brick, not a lunchbox-sized linear
            transformer.
          </p>
        </FAQItem>
        ...
        <FAQItem q="How does a power-factor-correction capacitor work?">
          <p>
            ...
            adds a current that <em className="text-text italic">leads</em> the voltage by 90°,
            partially cancelling the inductive lag. Sized correctly, the cap brings the net φ back
            near zero, the apparent power S drops down to the real power P, and the line current
            shrinks accordingly. The energy the inductor still wants to slosh back and forth now
            sloshes between the inductor and the local capacitor instead of all the way back to the
            utility, so the transmission losses drop
            <Cite id="grainger-power-systems-2003" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="Where does the 'missing' reactive power go each cycle?">
          <p>
            Into the field, and then back out. For an inductor, the energy that flowed in during the
            first quarter cycle was stored in the magnetic field around the coil (½LI²). In the next
        ...
        <FAQItem q="Why is there a 90° phase shift exactly, and not 45° or 135°?">
          <p>
            ...
            or a fractional derivative. d/dt applied to cos(ωt) gives −ω sin(ωt), which is the same
            cosine rotated by exactly π/2 radians in the complex plane. Two derivatives (as in the
            LC equation L d²Q/dt² + Q/C = 0) rotate by π, flipping sign. Real components with finite
            losses produce mixed phase shifts — a real inductor with series resistance looks like
            jωL + r, whose phase is less than 90° — but in the ideal limit of a pure reactance the
            phase is exactly π/2
            <Cite id="horowitz-hill-2015" in={SOURCES} />.
          </p>
        </FAQItem>
