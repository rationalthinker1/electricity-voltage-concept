  ...
  return (
    <ChapterShell chapter={chapter}>
      <p>
        ...
        of silicon, scaled up to a fingernail-sized die, holds <strong>around 19 billion</strong>
      ...
      <p>
        ...
        deliberate impurity in it); the <strong>p-n junction</strong> (two doped pieces back to
      ...
      <h2>From conductor to semiconductor</h2>
      ...
      <p>
        ...
        principle forces those levels to broaden into continuous <em>bands</em> of
        allowed energy, separated by forbidden gaps. The highest fully-occupied band at
        T = 0 is the <Term def={<><strong>valence band</strong> — the highest fully-occupied energy band at zero temperature. Electrons in this band participate in covalent bonds; they cannot conduct because every quantum state is already full.</>}>valence band</Term>; the lowest empty band above
        it is the <Term def={<><strong>conduction band</strong> — the band of energy states above the bandgap, empty at T = 0. An electron promoted into this band is free to move under an applied field and carries current.</>}>conduction band</Term>. The width of the gap between
        them — the <Term def={<><strong>bandgap</strong> E<sub>g</sub> — the forbidden energy interval between the top of the valence band and the bottom of the conduction band. For silicon, 1.12 eV.</>}>bandgap</Term> E<sub>g</sub> — sets the whole story
      ...
      <p>
        ...
        so the resulting <em>intrinsic carrier density</em> is around 10¹⁰ cm⁻³ at 300 K. Pure
      ...
      <p>
        ...
        cm³, a million times the intrinsic level. The crystal is now <Term def={<><strong>n-type</strong> — a semiconductor doped with donor impurities (Group V: P, As, Sb) so that the dominant mobile carriers are electrons.</>}>n-type</Term>.
      </p>
      <p>
        Substitute boron (Group III — three valence electrons) instead, and the missing electron
        leaves a vacancy in the valence band: a <em>hole</em>, which behaves as a positively-charged
        mobile carrier. Doping with boron makes the silicon <Term def={<><strong>p-type</strong> — a semiconductor doped with acceptor impurities (Group III: B, Ga, In) so that the dominant mobile carriers are holes (missing electrons in the valence band, which behave as positive charges).</>}>p-type</Term>, with holes
      ...
      <p>
        ...
        p-side do the mirror image. Each carrier that crosses leaves behind an <em>ionised
        ...
        <Term def={<><strong>depletion region</strong> — the thin slab straddling a p-n junction in which mobile carriers have been swept away, leaving only ionised dopants. The built-in field across it opposes further diffusion.</>}>depletion region</Term>
      ...
      <p>
        ...
        <Cite id="shockley-1949" in={SOURCES} /> — the <Term def={<><strong>Shockley equation</strong> — the closed-form I-V relation for an ideal p-n junction: I = I<sub>s</sub>(exp(qV/nkT) − 1). The exponential covers fifteen orders of magnitude across a ~0.5 V swing.</>}>Shockley diode equation</Term>:
      </p>
      ...
      <TryIt
        ...
        answer={
          <>
            <p>
              KVL around the loop: <em>V<sub>supply</sub> − V<sub>R</sub> − V<sub>F</sub> = 0</em>.
              The resistor drops 5 − 0.7 = 4.3 V. Ohm's law gives the current:
            </p>
            <Formula>I = V<sub>R</sub> / R = 4.3 V / 1000 Ω = <strong>4.3 mA</strong></Formula>
            <p>
      ...
      <h2>Diodes — and their many flavours</h2>
      ...
      <p>
        A <em>standard silicon p-n diode</em> like the 1N4148 has V<sub>F</sub> ≈ 0.7 V and a
        peak inverse voltage of 100 V; perfectly suited to mains-frequency rectification and
        small-signal switching. A <em><Term def={<><strong>Schottky diode</strong> — a metal-semiconductor junction (typically Pt or PtSi on n-Si) used as a diode. Lower V<sub>F</sub> (~0.3 V) and majority-carrier conduction (picosecond switching).</>}>Schottky diode</Term></em> replaces one of the p-n
      ...
      <p>
        ...
        voltage reference. An <em><Term def={<><strong>LED</strong> — light-emitting diode. A forward-biased p-n junction in a direct-gap semiconductor (GaAs, GaN, InGaN); recombining electron-hole pairs emit photons whose energy matches the bandgap.</>}>LED</Term></em>
      ...
      <h2>The bipolar junction transistor</h2>

      <p>
        Stack two p-n junctions back-to-back, sharing a thin middle layer, and you have a
        three-terminal device with a wildly nonlinear input-output relation. In an <em>npn</em>
        transistor, the layers are: a heavily-doped n-type <Term def={<><strong>emitter</strong> — the heavily-doped terminal of a BJT that injects minority carriers into the base.</>}>emitter</Term>, a thin lightly-doped
        p-type <Term def={<><strong>base</strong> — the thin lightly-doped middle layer of a BJT. The base-emitter junction's forward voltage controls the much larger collector current.</>}>base</Term>, and a moderately-doped n-type <Term def={<><strong>collector</strong> — the third terminal of a BJT, biased in reverse with respect to the base; it collects most of the carriers injected by the emitter.</>}>collector</Term>. The base-emitter junction is forward-biased
      ...
      <p>
        ...
        breaks into three regions. Below V<sub>CE</sub> ≈ 0.2 V the device is in <em>saturation</em>:
      ...
      <p>
        There are three ways to wire the device into a circuit. <Term def={<><strong>Common-emitter</strong> — the standard amplifier configuration: input drives the base, output is taken from the collector, emitter is grounded (or grounded for AC through a bypass cap). Both voltage and current gain are large; the output is inverted.</>}>Common-emitter</Term>
        (input at base, output at collector, emitter grounded) gives both voltage and current
        gain, with the output inverted; it is the workhorse amplifier topology. <em>Common-base</em>
        (input at emitter, output at collector) has low input impedance and gives only voltage
        gain — useful at very high frequencies. <em>Common-collector</em> (input at base, output
      ...
      <TryIt
        ...
        answer={
          <>
            <Formula>I<sub>C</sub> = β · I<sub>B</sub> = 100 · 10 µA = <strong>1.0 mA</strong></Formula>
            <Formula>I<sub>E</sub> = I<sub>C</sub> + I<sub>B</sub> = 1.0 mA + 0.01 mA = <strong>1.01 mA</strong></Formula>
            <p>
      ...
      <h2>The field-effect transistor</h2>

      <p>
        The BJT works by injecting carriers across a forward-biased junction. The <Term def={<><strong>field-effect transistor</strong> (FET) — a transistor whose channel conductance between source and drain is modulated by the voltage on an electrically-isolated gate. No current (ideally) flows into the gate.</>}>field-effect transistor</Term>
      ...
      <p>
        ...
        2–10 nm layer of SiO<sub>2</sub>. Below threshold — the <Term def={<><strong>threshold voltage</strong> V<sub>T</sub> — the gate-source voltage at which an inversion layer of mobile carriers just begins to form under the gate. Typically 0.3–1 V for modern process nodes.</>}>threshold voltage</Term> V<sub>T</sub> —
      ...
      <p>
        ...
        <Cite id="sedra-smith-2014" in={SOURCES} />. In <em>triode</em> (V<sub>DS</sub> &lt; V<sub>GS</sub> − V<sub>T</sub>):
      </p>
      ...
      <TryIt
        ...
        answer={
          <>
            <p>The overdrive voltage is V<sub>OV</sub> = V<sub>GS</sub> − V<sub>T</sub> = 2 V.</p>
            <Formula>I<sub>D</sub> = (k<sub>n</sub>/2) V<sub>OV</sub>² = (1 mA/V² / 2)(2 V)² = <strong>2 mA</strong></Formula>
            <p>
      ...
      <h2>Small-signal model and load lines</h2>

      <p>
        Real amplifiers operate around a fixed DC bias point — the <Term def={<><strong>operating point</strong> (Q-point) — the DC values of voltages and currents in a circuit when no signal is present. Sets where on the device's nonlinear I-V curve the small-signal swing happens.</>}>operating point</Term>, or Q-point —
        ...
        to a small base-emitter perturbation is captured by a single number: the <Term def={<><strong>transconductance</strong> g<sub>m</sub> = ∂I<sub>C</sub>/∂V<sub>BE</sub>. The slope of the device's I-V curve at the operating point; for a BJT, g<sub>m</sub> = I<sub>C</sub>/V<sub>T</sub>. Sets the small-signal gain of any amplifier built from the device.</>}>transconductance</Term>
      ...
      <p>
        ...
        working <em>common-emitter amplifier</em>. The bias divider R<sub>1</sub>/R<sub>2</sub>
      ...
      <TryIt
        ...
        answer={
          <>
            <Formula>g<sub>m</sub> = I<sub>C</sub> / V<sub>T</sub> = 2 mA / 25.85 mV ≈ <strong>77.4 mS</strong></Formula>
            <Formula>A<sub>v</sub> = −g<sub>m</sub> · R<sub>C</sub> = −(0.0774 S)(5000 Ω) ≈ <strong>−387</strong></Formula>
            <p>
      ...
      >
        ...
        >
          <p>
            ...
            power transfer; an <em>optoisolator</em> handles the feedback signal that tells
      ...
      >
        ...
        <FAQItem q={'What does "saturation" mean for a BJT vs. for a MOSFET? They seem opposite.'}>
          <p>
            They are opposite. For a <em>BJT</em>, "saturation" is the low-V<sub>CE</sub>, fully-on
            regime where both junctions are forward-biased and the device acts almost as a
            closed switch; the active region is above V<sub>CE(sat)</sub>. For a <em>MOSFET</em>,
        ...
        <FAQItem q="How does a transistor 'amplify' — where does the extra energy come from?">
          <p>
            ...
            <em> controlled valve</em>. A small input signal modulates how much current the
        ...
        <FAQItem q={"What's the difference between enhancement and depletion MOSFETs?"}>
          <p>
            Whether the channel exists at V<sub>GS</sub> = 0. An <em>enhancement</em> n-MOSFET
            has V<sub>T</sub> &gt; 0; the channel does not exist at zero gate voltage, and you
            have to "enhance" it by raising V<sub>GS</sub> above V<sub>T</sub>. A <em>depletion</em>
        ...
        <FAQItem q={"What's a FinFET / GAAFET and how does it differ from a planar MOSFET?"}>
          <p>
            ...
            from the sides and the gate loses control — the device leaks. A <em>FinFET</em>
            ...
            the <em>gate-all-around</em> FET (GAAFET, also called nanosheet FET, Samsung and
