  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <p className="mb-prose-3">
        ...
        <em className="italic text-text"> N<sub>s</sub></em> turns around another leg of the same ring. Drive the first coil — the
        <Term def={<><strong className="text-text font-medium">primary</strong> — the winding connected to the source. It accepts power from the supply and produces the changing flux in the core.</>}>primary</Term> —
        with an AC voltage source <em className="italic text-text">V<sub>p</sub>(t)</em>. Current flows in the primary; that current creates an
        alternating magnetic field; the iron core funnels essentially all of that magnetic flux into the
        <em className="italic text-text"> secondary</em> leg, where it threads the second coil. By Faraday's law of induction, each turn of the
        secondary sees an EMF equal to <em className="italic text-text">−dΦ/dt</em>, the time rate of change of flux through it. Add the EMFs of
        all <em className="italic text-text">N<sub>s</sub></em> turns and you get the
        <Term def={<><strong className="text-text font-medium">secondary</strong> — the winding connected to the load. Its terminal voltage is set by the ratio of turn counts to the primary.</>}>secondary's</Term>
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">V<sub>s</sub>(t)</strong> is the instantaneous secondary terminal voltage (in volts),
        <strong className="text-text font-medium"> N<sub>s</sub></strong> is the number of turns on the secondary winding (dimensionless), and
        <strong className="text-text font-medium"> dΦ/dt</strong> is the time rate of change of the magnetic flux threading each turn of the core
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">V<sub>p</sub>(t)</strong> is the instantaneous primary terminal voltage (in volts),
        <strong className="text-text font-medium"> N<sub>p</sub></strong> is the number of turns on the primary winding (dimensionless), and
        <strong className="text-text font-medium"> dΦ/dt</strong> is the same shared core flux (in webers per second) that threads both windings.
      </p>
      <p className="mb-prose-3">
        Divide the two equations and the <em className="italic text-text">dΦ/dt</em> cancels. What survives is the most famous one-liner in
        electrical engineering:
      </p>
      <Formula>V<sub>s</sub> / V<sub>p</sub> = N<sub>s</sub> / N<sub>p</sub></Formula>
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">V<sub>p</sub></strong> and <strong className="text-text font-medium">V<sub>s</sub></strong> are the primary and secondary
        terminal voltages (in volts, RMS or peak — the ratio holds either way), and
        <strong className="text-text font-medium"> N<sub>p</sub></strong> and <strong className="text-text font-medium">N<sub>s</sub></strong> are the corresponding turn counts
      ...
      <p className="mb-prose-3">
        Notice what the picture says about <em className="italic text-text">energy</em>. The flux in the core is set entirely by the primary's
        ...
        <Term def={<><strong className="text-text font-medium">magnetizing current</strong> — the small primary current required to establish the core flux even with no load. Sets the no-load draw of a transformer; ideally would be zero.</>}>magnetizing current</Term>
      ...
      <p className="mb-prose-3">
        ...
        <em className="italic text-text"> I<sub>s</sub> = V<sub>s</sub>/R</em>. That current also creates magnetic flux in the core — and by
        ...
        <Term def={<><strong className="text-text font-medium">ideal transformer</strong> — the lossless, infinitely-permeable, no-leakage limit. Used as the conceptual starting point; every real transformer is a perturbation of this model.</>}>ideal transformer</Term>
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">V<sub>p</sub></strong>, <strong className="text-text font-medium">V<sub>s</sub></strong> are the primary and secondary terminal
        voltages (in volts) and <strong className="text-text font-medium">I<sub>p</sub></strong>, <strong className="text-text font-medium">I<sub>s</sub></strong> are the corresponding
      ...
      <p className="mb-prose-3">
        Combine that with <em className="italic text-text">V<sub>s</sub>/V<sub>p</sub> = N<sub>s</sub>/N<sub>p</sub></em> and the current ratio
        falls out as the <em className="italic text-text">inverse</em> of the turns ratio:
      </p>
      <Formula>I<sub>s</sub> / I<sub>p</sub> = N<sub>p</sub> / N<sub>s</sub></Formula>
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">I<sub>p</sub></strong> and <strong className="text-text font-medium">I<sub>s</sub></strong> are the primary and secondary
        winding currents (in amperes), and <strong className="text-text font-medium">N<sub>p</sub></strong>, <strong className="text-text font-medium">N<sub>s</sub></strong> are the
      ...
      <p className="mb-prose-3">
        A 10:1 <Term def={<><strong className="text-text font-medium">step-down</strong> — the secondary turn count is smaller than the primary; voltage decreases, current increases. The pole transformer outside your house is step-down.</>}>step-down</Term>
        transformer takes 1000 V at 1 A on the primary and delivers 100 V at 10 A on the secondary. Same kilowatt.
        A 1:50 <Term def={<><strong className="text-text font-medium">step-up</strong> — secondary turn count larger than primary; voltage rises, current falls. The transformer at every generating station is step-up.</>}>step-up</Term>
      ...
      <TryIt
        tag="Try 18.1"
        question={<>A pole-mounted distribution transformer steps <strong className="text-text font-medium">12,470 V</strong> down to <strong className="text-text font-medium">240 V</strong> for residential service. What is the turns ratio?</>}
        hint="N_p / N_s = V_p / V_s."
        answer={
          <>
            <Formula>N<sub>p</sub> / N<sub>s</sub> = 12,470 / 240 ≈ <strong className="text-text font-medium">52 : 1</strong></Formula>
      ...
      <TryIt
        tag="Try 18.2"
        question={<>A <strong className="text-text font-medium">60 W</strong> halogen lamp runs on the <strong className="text-text font-medium">12 V</strong> secondary of a small transformer. The primary is fed from a <strong className="text-text font-medium">120 V</strong> outlet. What is the turns ratio, and what current does the primary draw?</>}
        hint="Voltage ratio gives the turns ratio. Power balance (ideal) gives the primary current."
        answer={
          <>
            <Formula>N<sub>p</sub> / N<sub>s</sub> = 120 / 12 = <strong className="text-text font-medium">10 : 1</strong></Formula>
            <p className="mb-prose-1 last:mb-0">The secondary current is <em className="italic text-text">I<sub>s</sub> = 60 W / 12 V = 5 A</em>. By power balance:</p>
            <Formula>I<sub>p</sub> = 60 W / 120 V = <strong className="text-text font-medium">0.5 A</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <TryIt
        tag="Try 18.5"
        question={<>A step-up transformer raises <strong className="text-text font-medium">12 V</strong> to <strong className="text-text font-medium">240 V</strong> for a small inverter's high-voltage rail. If the primary draws <strong className="text-text font-medium">5 A</strong>, what current does an ideal secondary deliver?</>}
        hint="Power balance: V_p · I_p = V_s · I_s."
        answer={
          <>
            <Formula>I<sub>s</sub> = V<sub>p</sub> · I<sub>p</sub> / V<sub>s</sub> = 12 · 5 / 240 = <strong className="text-text font-medium">0.25 A</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <p className="mb-prose-3">
        ...
        Stanley's <em className="italic text-text">induction coils</em> — purpose-built step-down transformers of his own design — dropped it to
      ...
      <p className="mb-prose-3">
        ...
        back down at the load. Edison's competing DC system couldn't transform — DC produces no <em className="italic text-text">dΦ/dt</em>, so
      ...
      <p className="mb-prose-3">
        The reason is brutal arithmetic. The power dissipated in a transmission line of resistance <em className="italic text-text">R</em>
        carrying current <em className="italic text-text">I</em> is <em className="italic text-text">P<sub>loss</sub> = I²R</em>. To deliver power <em className="italic text-text">P</em> at voltage
        <em className="italic text-text"> V</em>, the line carries current <em className="italic text-text">I = P/V</em>. So the loss is:
      </p>
      <Formula>P<sub>loss</sub> = (P / V)² · R = P² R / V²</Formula>
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">P<sub>loss</sub></strong> is the I²R power dissipated in the line (in watts),
        <strong className="text-text font-medium"> P</strong> is the real power being delivered to the far end (in watts),
        <strong className="text-text font-medium"> V</strong> is the line voltage at which that power is sent (in volts), and
        <strong className="text-text font-medium"> R</strong> is the total resistance of the transmission line (in ohms).
      </p>
      <p className="mb-prose-3">
      ...
      <TryIt
        tag="Try 18.3"
        question={<>Deliver <strong className="text-text font-medium">1 MW</strong> over a <strong className="text-text font-medium">100 km</strong> transmission line whose total resistance is <strong className="text-text font-medium">R = 10 Ω</strong>. Compare the I²R loss at <strong className="text-text font-medium">V = 4 kV</strong> (Stanley's era) to that at <strong className="text-text font-medium">V = 400 kV</strong> (modern HV transmission).</>}
        hint="Compute I = P/V for each case, then I²R."
        answer={
          <>
            <p className="mb-prose-1 last:mb-0">At 4 kV:</p>
            <Formula>I = 10⁶ / 4×10³ = 250 A; &nbsp; P<sub>loss</sub> = (250)² · 10 = <strong className="text-text font-medium">625 kW</strong></Formula>
            <p className="mb-prose-1 last:mb-0">62.5 % of the input is lost as heat in the wires before the energy ever reaches the town. Useless.</p>
            <p className="mb-prose-1 last:mb-0">At 400 kV:</p>
            <Formula>I = 10⁶ / 4×10⁵ = 2.5 A; &nbsp; P<sub>loss</sub> = (2.5)² · 10 = <strong className="text-text font-medium">62.5 W</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
              A ten-thousand-fold reduction in loss for a hundred-fold increase in voltage — exactly the <em className="italic text-text">V²</em>
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">impedance transformation</strong> — the relationship Z_p = (N_p/N_s)² · Z_s by which a transformer makes a load on its secondary look like a different impedance to the source on its primary. The basis of all transformer-based matching networks.</>}>impedance</Term>.
        Put a load <em className="italic text-text">Z<sub>L</sub></em> on the secondary. From the primary's point of view, the transformer plus
        load together draw a current <em className="italic text-text">I<sub>p</sub> = (N<sub>s</sub>/N<sub>p</sub>) · I<sub>s</sub></em> at a
        voltage <em className="italic text-text">V<sub>p</sub> = (N<sub>p</sub>/N<sub>s</sub>) · V<sub>s</sub></em>. The ratio
        <em className="italic text-text"> V<sub>p</sub>/I<sub>p</sub></em> — the impedance seen looking into the primary — is therefore:
      </p>
      <Formula>Z<sub>p</sub> = (N<sub>p</sub> / N<sub>s</sub>)² · Z<sub>L</sub></Formula>
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">Z<sub>p</sub></strong> is the impedance seen looking into the primary terminals (in ohms),
        <strong className="text-text font-medium"> Z<sub>L</sub></strong> is the actual load impedance connected to the secondary (in ohms), and
        <strong className="text-text font-medium"> N<sub>p</sub>/N<sub>s</sub></strong> is the primary-to-secondary turns ratio (dimensionless).
      ...
      <TryIt
        tag="Try 18.4"
        question={<>An <strong className="text-text font-medium">8 Ω</strong> speaker is connected to a transformer with turns ratio <strong className="text-text font-medium">N_p/N_s = 20</strong>. What impedance does the primary side see?</>}
        hint="Z_p = (N_p/N_s)² · Z_s."
        answer={
          <>
            <Formula>Z<sub>p</sub> = 20² · 8 Ω = 400 · 8 = <strong className="text-text font-medium">3.2 kΩ</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <p className="mb-prose-3">
        ...
        same iron: a single continuous winding with a <em className="italic text-text">tap</em> brought out at a fraction <em className="italic text-text">k</em> of
        the way down. Feed the full winding from the source; take the load off the portion between the
        tap and the bottom. The voltage ratio is still the turns ratio — <em className="italic text-text">V<sub>s</sub>/V<sub>p</sub> = k</em> —
        but now the bottom section of the winding is shared between the two circuits.
        That sharing is the
        <Term def={<><strong className="text-text font-medium">autotransformer</strong> — a transformer with a single tapped winding instead of two galvanically isolated windings. The shared section between primary and secondary terminals carries only the difference current I<sub>s</sub> − I<sub>p</sub>, allowing it to be wound from lighter wire — for a 2:1 step, autotransformer copper and iron are roughly half that of an isolated two-winding design of the same rating.</>}>autotransformer</Term>'s
        whole point: the shared portion carries only the <em className="italic text-text">difference</em> current
        <em className="italic text-text"> I<sub>s</sub> − I<sub>p</sub> = (1 − k) I<sub>s</sub></em>, so it can be wound from much
      ...
      <p className="mb-prose-3">
        ...
        power system, the 138/69 kV transformers in older substations, and the <em className="italic text-text">variac</em> on a lab
        bench are all autotransformers<Cite id="grainger-power-systems-2003" in={SOURCES} />. The price is
        the loss of <em className="italic text-text">galvanic isolation</em>: primary and secondary share metal, so a winding short
      ...
      <p className="mb-prose-3">
        <strong className="text-text font-medium">Copper losses</strong> are the simplest: each winding has a finite resistance R<sub>cu</sub>, and
        the current flowing through it dissipates <em className="italic text-text">I²R</em> as heat. This is the dominant loss in a transformer
        operating near its rated load; sizing the wire (and therefore the window area in the core) trades against
        material cost.
      </p>
      <p className="mb-prose-3">
        <strong className="text-text font-medium"><Term def={<><strong className="text-text font-medium">Core lamination</strong> — assembling the magnetic core from many thin insulated sheets rather than as a solid block. Breaks up eddy-current paths and reduces eddy losses by orders of magnitude.</>}>Core losses</Term></strong> are
        what flows in the iron itself, and split into two:
      </p>
      <ul>
        <li>
          <strong className="text-text font-medium"><Term def={<><strong className="text-text font-medium">Hysteresis loss</strong> — energy dissipated each AC cycle as the core's magnetic domains realign with the changing applied field. Proportional to the enclosed area of the B-H loop and to the operating frequency.</>}>Hysteresis loss</Term></strong>:
          ...
          characterised this experimentally in 1893 as <em className="italic text-text">P<sub>h</sub> ∝ f · B<sub>peak</sub><sup>1.6</sup></em>
          for typical electrical steels<Cite id="steinmetz-1893" in={SOURCES} />.
        </li>
        <li>
          <strong className="text-text font-medium"><Term def={<><strong className="text-text font-medium">Eddy current</strong> — a current loop induced inside a conductor by a time-changing magnetic flux passing through it. In transformer cores, eddy currents waste energy as I²R heat; laminating the core breaks the loops up and suppresses the loss.</>}>Eddy-current loss</Term></strong>:
      ...
      <p className="mb-prose-3">
        Adjacent to saturation lives one more practical non-ideality: <em className="italic text-text">inrush</em>. Because the core
        ...
        <em className="italic text-text"> peak</em> and the integral starts at its natural average; flux swings symmetrically about
        zero and the core never saturates. Close it at a voltage <em className="italic text-text">zero-crossing</em> and the integral
      ...
      <p className="mb-prose-3">
        That spike is why the breaker upstream of a large transformer has an <em className="italic text-text">inverse-time</em>
      ...
      <TryIt
        tag="Try 18.6"
        question={<>A 4 kVA transformer at <strong className="text-text font-medium">60 Hz</strong> is wound with <strong className="text-text font-medium">N<sub>p</sub> = 200</strong> turns and runs at <strong className="text-text font-medium">B<sub>max</sub> = 1.5 T</strong> for a primary V<sub>rms</sub> = 240 V. What core cross-section A<sub>core</sub> does it need? (Use V = 4.44 · f · N · B · A.)</>}
        hint="Solve V = 4.44 · f · N · B · A for A."
        answer={
          <>
            <Formula>A<sub>core</sub> = V / (4.44 · f · N · B<sub>max</sub>)</Formula>
            <Formula>= 240 / (4.44 · 60 · 200 · 1.5) ≈ <strong className="text-text font-medium">3.0 × 10⁻³ m²</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
              About <strong className="text-text font-medium">30 cm²</strong>, or a roughly 5.5 × 5.5 cm core leg. Doubling the frequency
      ...
      <p className="mb-prose-3">
        <strong className="text-text font-medium"><Term def={<><strong className="text-text font-medium">Leakage inductance</strong> — flux generated by one winding that fails to thread the other winding. Behaves like a series inductance in the transformer's equivalent circuit and contributes to voltage regulation droop under load.</>}>Leakage inductance</Term></strong>
      ...
      <p className="mb-prose-3">
        Finally there is the <em className="italic text-text">magnetizing current</em> — the small current the primary must draw, even at
      ...
      <p className="mb-prose-3">
        ...
        <em className="italic text-text"> generator step-up transformer</em>, sitting on a concrete pad just outside the powerhouse, raises that
      ...
      <p className="mb-prose-3">
        At a regional <em className="italic text-text">bulk substation</em>, a second transformer steps the voltage down to roughly 138 kV for
        sub-transmission to the city. A third step at a <em className="italic text-text">distribution substation</em> drops the voltage further
      ...
      <p className="mb-prose-3">
        ...
        count, and the peak flux density fixed and the required core cross-section <em className="italic text-text">A</em> falls inversely with
      ...
      <p className="mb-prose-3">
        ...
        a voltage ratio equal to the turns ratio: <em className="italic text-text">V<sub>s</sub>/V<sub>p</sub> = N<sub>s</sub>/N<sub>p</sub></em>.
      ...
      >
        ...
        <CaseStudy
          ...
          specs={[
            ...
            { label: 'Mass', value: <>~5 g for a 30 W unit; <strong className="text-text font-medium">1000×</strong> lighter than a linear 60 Hz equivalent</> },
          ]}
        >
          <p className="mb-prose-2 last:mb-0">
        ...
        >
          ...
          <p className="mb-prose-2 last:mb-0">
            Beyond the basic step-down ratio, the unit has an <em className="italic text-text">on-load tap changer</em>: a motor-driven switch
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            impedance match: <em className="italic text-text">Z<sub>p</sub> = (N<sub>p</sub>/N<sub>s</sub>)² Z<sub>s</sub></em>, so a 25:1
      ...
      >
        ...
        <FAQItem q="Why doesn't a transformer work on DC?">
          <p>
            Because the secondary's induced EMF is <em className="italic text-text">−N<sub>s</sub> dΦ/dt</em>. Steady DC produces a steady flux,
            so <em className="italic text-text">dΦ/dt = 0</em> and the secondary EMF is zero. Worse, with no induced EMF on the primary to
        ...
        <FAQItem q="Can a transformer step up DC pulses?">
          <p>
            ...
            voltage; <em className="italic text-text">dΦ/dt</em> is large during each transition, so the secondary delivers proportionally large
        ...
        <FAQItem q="Why are large transformers so heavy?">
          <p>
            ...
            60 Hz. The required core cross-section scales as <em className="italic text-text">V<sub>p</sub>/(N<sub>p</sub> · f · B<sub>sat</sub>)</em>;
        ...
        <FAQItem q="What's the difference between a transformer and an inductor?">
          <p>
            ...
            two (or more) coils coupling energy <em className="italic text-text">between</em> circuits via a shared flux. The primary of a
        ...
        <FAQItem q="If transformer cores work better at higher flux, why don't designers push the flux to the maximum?">
          <p>
            ...
            losses balloon (Steinmetz's <em className="italic text-text">B<sup>1.6</sup></em> exponent compounds), and the transformer becomes a
