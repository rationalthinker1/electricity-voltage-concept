  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">diode</strong> — a two-terminal device that conducts current readily in one direction (forward) and blocks it in the other (reverse). Implemented as a vacuum tube (Fleming 1904) or a doped semiconductor junction (Shockley 1949).</>}>diode</Term>.
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">thermal voltage</strong> — <em className="italic text-text">V<sub>T</sub> = kT/q</em> ≈ 25.85 mV at T = 300 K. The natural voltage scale of any thermally-populated semiconductor.</>}>V<sub>T</sub> = kT/q</Term>
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">forward voltage</strong> — the V across a forward-biased diode at its typical operating current; the practical "knee" of the I–V curve. ≈ 0.7 V for silicon p-n, ≈ 0.3 V for Schottky, ≈ 0.2 V for low-V<sub>F</sub> germanium.</>}>V<sub>F</sub></Term>
        {' '}≈ 0.7 V and reverse blocking up to its rated peak inverse voltage. A{' '}
        <Term def={<><strong className="text-text font-medium">Schottky diode</strong> — a metal-semiconductor junction rather than p-n. Lower V<sub>F</sub> (≈ 0.3 V) and majority-carrier conduction (no slow minority-carrier storage), so it switches in picoseconds. Standard rectifier in the output of every switching power supply.</>}>Schottky</Term>
        ...
        A{' '}
        <Term def={<><strong className="text-text font-medium">Zener diode</strong> — a heavily-doped p-n diode designed to operate in reverse breakdown at a specific clamp voltage (typically 2.4 V to 200 V). The "Zener voltage" V<sub>Z</sub> is set by the doping profile and is remarkably stable; used as a voltage reference and as an over-voltage clamp.</>}>Zener</Term>
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">half-wave rectifier</strong> — one diode in series with the AC source; conducts on one polarity only. The negative half of every cycle is thrown away, so the output averages only <em className="italic text-text">V<sub>p</sub>/π</em>.</>}>half-wave rectifier</Term>,
      ...
      <p className="mb-prose-3">
        The{' '}
        <Term def={<><strong className="text-text font-medium">full-wave centre-tap rectifier</strong> — two diodes plus a centre-tapped transformer. Each diode handles one half-cycle; both halves drive the load with the same polarity. Doubles the average output to <em className="italic text-text">2 V<sub>p</sub>/π</em>.</>}>full-wave centre-tap rectifier</Term>
      ...
      <p className="mb-prose-3">
        ...
        with a large{' '}
        <Term def={<><strong className="text-text font-medium">ripple</strong> — the AC component remaining on top of an imperfectly-smoothed DC supply. For a bridge rectifier feeding a capacitor C through load current <em className="italic text-text">I</em>, the peak-to-peak ripple is <em className="italic text-text">ΔV ≈ I / (2 f<sub>line</sub> C)</em>.</>}>smoothing capacitor</Term>.
      ...
      <TryIt
        ...
        question={
          <>
            A bridge rectifier feeds a <strong className="text-text font-medium">1000 µF</strong> capacitor with a steady
            <strong className="text-text font-medium"> 1 A</strong> load current from 60 Hz mains. Estimate the peak-to-peak ripple.
          </>
        }
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              About <strong className="text-text font-medium">8.3 V peak-to-peak</strong> — enormous on a 5 V rail. That is why bulk
      ...
      <TryIt
        ...
        question={
          <>
            The same <strong className="text-text font-medium">1000 µF</strong> smoothing cap, but now the load draws only{' '}
            <strong className="text-text font-medium">100 mA</strong> from 60 Hz mains. What is the peak-to-peak ripple? Use
        ...
        answer={
          <>
            <Formula>ΔV ≈ 0.1 A · 8.33×10⁻³ s / 10⁻³ F = <strong className="text-text font-medium">0.83 V</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
              About <strong className="text-text font-medium">830 mV peak-to-peak</strong> — ten times less, exactly as the linear ΔV ∝ I
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">linear regulator</strong> — a circuit that holds its output voltage constant by adjusting a pass transistor in series with the load, dropping the difference between V<sub>in</sub> and V<sub>out</sub> as heat. Efficiency = V<sub>out</sub>/V<sub>in</sub>.</>}>linear regulator</Term>.
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">P<sub>diss</sub></strong> is the power burned as heat in the pass transistor (in watts),
        <strong className="text-text font-medium"> V<sub>in</sub></strong> is the regulator's input voltage (in volts),
        <strong className="text-text font-medium"> V<sub>out</sub></strong> is the regulated output voltage (in volts), and
        <strong className="text-text font-medium"> I<sub>load</sub></strong> is the current flowing through the load (in amperes; the same current
      ...
      <p className="mb-prose-3">
        ...
        operating margin: a standard LM7805 needs at least about <strong className="text-text font-medium">2 V of headroom</strong>
        (its{' '}
        <Term def={<><strong className="text-text font-medium">dropout voltage</strong> — the minimum V<sub>in</sub> − V<sub>out</sub> required for a linear regulator to maintain regulation. Standard NPN-pass regulators need ≈ 2 V; low-drop-out (LDO) PMOS-pass regulators need only a few hundred millivolts.</>}>dropout voltage</Term>)
      ...
      <TryIt
        ...
        question={
          <>
            An LM7805 takes <strong className="text-text font-medium">12 V</strong> in and supplies <strong className="text-text font-medium">500 mA</strong> at 5 V.
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              The junction would sit ~<strong className="text-text font-medium">228 °C above ambient</strong> — far above the 150 °C
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium"> high-side switch</strong> (a MOSFET) opens and closes at tens or hundreds of
        ...
        <Term def={<><strong className="text-text font-medium">volt-second balance</strong> — in steady state, the average voltage across an ideal inductor over one switching period must be zero (otherwise the current would grow without bound). Setting <em className="italic text-text">∫ V<sub>L</sub> dt = 0</em> across the on-time and off-time gives the conversion ratio of every basic SMPS topology.</>}>volt-second balance on the inductor</Term>:
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">buck converter</strong> — a step-down DC-DC topology consisting of a high-side switch, a low-side diode (or synchronous switch), an inductor, and an output capacitor. <em className="italic text-text">V<sub>out</sub> = D · V<sub>in</sub></em> in continuous-conduction mode.</>}>buck (step-down) converter</Term>
        {' '}with{' '}
        <Term def={<><strong className="text-text font-medium">duty cycle</strong> — the fraction of each switching period during which the switch is ON. <em className="italic text-text">D = t<sub>on</sub> / T<sub>sw</sub></em>. Continuous from 0 to 1.</>}>duty cycle</Term>
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">V<sub>in</sub></strong> is the buck converter's DC input voltage (in volts),
        <strong className="text-text font-medium"> V<sub>out</sub></strong> is the regulated DC output voltage (in volts), and
        <strong className="text-text font-medium"> D = t<sub>on</sub>/T<sub>sw</sub></strong> is the duty cycle — the fraction of each switching
      ...
      <p className="mb-prose-3">
        ...
        Modern synchronous bucks routinely hit <strong className="text-text font-medium">92–98 % efficiency</strong> — an order of
      ...
      <TryIt
        ...
        question={
          <>
            A buck converter takes <strong className="text-text font-medium">12 V</strong> in and runs at <strong className="text-text font-medium">50 %</strong> duty cycle. What
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              <strong className="text-text font-medium">6.0 V</strong>. The output is just the input multiplied by the on-fraction; the
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">boost converter</strong> — a step-up DC-DC topology: inductor on the input side, switch to ground, output diode, output capacitor. <em className="italic text-text">V<sub>out</sub> = V<sub>in</sub> / (1 − D)</em>.</>}>boost converter</Term>:
        ...
        through a diode. Volt-second balance now gives <strong className="text-text font-medium">V<sub>out</sub> = V<sub>in</sub> / (1 − D)</strong>,
      ...
      <TryIt
        ...
        question={
          <>
            A boost converter runs at <strong className="text-text font-medium">75 %</strong> duty cycle from a <strong className="text-text font-medium">12 V</strong> input.
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              <strong className="text-text font-medium">48 V</strong>, an exact 4× step-up. Push D toward 1 and the equation blows up;
      ...
      <TryIt
        ...
        question={
          <>
            A buck converter takes <strong className="text-text font-medium">V<sub>in</sub> = 24 V</strong> at duty cycle <strong className="text-text font-medium">D = 0.3</strong>.
            What is the ideal output voltage? If the load draws <strong className="text-text font-medium">5 A</strong>, what input current does
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              <strong className="text-text font-medium">7.2 V at 1.5 A in / 5 A out</strong>. The current step-up is exactly the inverse of the
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">SCR / thyristor</strong> — a four-layer p-n-p-n device that latches ON when a gate pulse fires it and stays ON until the principal current crosses zero. Designed for high voltage and current; the workhorse switch of HVDC valves, phase-controlled rectifiers, and large industrial motor drives.</>}>silicon controlled rectifier (SCR)</Term>
        {' '}in 1957 and the four-layer{' '}
        <Term def={<><strong className="text-text font-medium">thyristor</strong> — the broader family of four-layer regenerative switches that includes the SCR, the gate-turn-off thyristor (GTO), and the integrated-gate commutated thyristor (IGCT). Used wherever multi-kilovolt, multi-kiloamp switching is required.</>}>thyristor</Term>
        {' '}became the workhorse of every megawatt-class converter for the next half-century.
        Today's high-power tools — the{' '}
        <Term def={<><strong className="text-text font-medium">IGBT</strong> — insulated-gate bipolar transistor. A MOS-gated input drives a bipolar-output high-current device. Combines the easy drive of a MOSFET with the low conduction loss of a BJT. Standard switch in 1–3 kV traction drives and industrial inverters.</>}>IGBT</Term>
        {' '}(insulated-gate bipolar transistor) and the silicon-carbide{' '}
        <Term def={<><strong className="text-text font-medium">MOSFET</strong> — metal-oxide-semiconductor field-effect transistor. Voltage-controlled four-terminal switch with gigaohm gate impedance. Modern power MOSFETs (silicon and SiC) switch tens of amps at hundreds of volts in tens of nanoseconds.</>}>MOSFET</Term>
      ...
      <p className="mb-prose-3">
        ...
        wave. That is an{' '}
        <Term def={<><strong className="text-text font-medium">inverter</strong> — a circuit that converts DC to AC. The "inverse" of a rectifier. Implemented as an H-bridge of MOSFETs or IGBTs switched by sinusoidal pulse-width modulation, followed by an LC filter.</>}>inverter</Term>:
      ...
      <p className="mb-prose-3">
        ...
        <Term def={<><strong className="text-text font-medium">H-bridge</strong> — four switches (MOSFETs or IGBTs) arranged in two legs across a DC bus, with the load connected between the leg midpoints. Driving the four switches in complementary pairs lets you apply +V<sub>DC</sub>, −V<sub>DC</sub>, or zero across the load — the basic building block of every single-phase inverter and every brushed-DC drive.</>}>H-bridge</Term>:
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium">m · V<sub>DC</sub></strong>, where m ∈ [0, 1] is the modulation index. Setting
      ...
      <TryIt
        ...
        question={
          <>
            An H-bridge inverter delivers a <strong className="text-text font-medium">100 V peak</strong>, 60 Hz sine across a pure
            inductive load of <strong className="text-text font-medium">1 mH</strong>. What is the peak current?
          </>
        }
        ...
        answer={
          <>
            ...
            <p className="mb-prose-1 last:mb-0">
              About <strong className="text-text font-medium">265 A peak</strong> — large, because a 1 mH choke at 60 Hz is essentially
      ...
      <p className="mb-prose-3">
        ...
        two terms: a real-power term proportional to <strong className="text-text font-medium">cos(θ)</strong> and a
        reactive-power term proportional to <strong className="text-text font-medium">sin(θ)</strong>, where θ is the phase shift
      ...
      <p className="mb-prose-3">
        The second is{' '}
        <Term def={<><strong className="text-text font-medium">phase-locked loop (PLL)</strong> — a control loop that estimates the phase and frequency of the grid voltage in real time and produces a synchronised reference for the inverter's PWM. Without it the inverter cannot know when to fire the next current pulse.</>}>phase synchronisation</Term>:
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium"> 846 miles at ±500 kV</strong>, carrying up to 3.1 GW, with converter halls full
      ...
      <CaseStudies intro="Four working systems where this chapter's physics is the entire architecture.">
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            <Term def={<><strong className="text-text font-medium">power-factor correction (PFC)</strong> — a pre-regulator that forces the input current of an AC-DC supply to track the input voltage sinusoidally, presenting a near-resistive load to the line. Required by IEC 61000-3-2 for supplies above 75 W.</>}>PFC pre-regulator</Term>
        ...
        >
          ...
          <p className="mb-prose-2 last:mb-0">
            ...
            switches operated under different firmware. About <strong className="text-text font-medium">~60 kW</strong> of recovered
      ...
      <FAQ intro="The questions a careful reader asks after looking at the back of any power supply.">
        ...
        <FAQItem q="Why does my LED bulb sometimes flicker on a dimmer?">
          <p>
            ...
            <strong className="text-text font-medium">phase-controlled triacs</strong> (the AC version of an SCR): they conduct only
        ...
        <FAQItem q="What does 'soft-switching' mean and why does it matter?">
          <p>
            ...
            <em className="italic text-text"> zero voltage</em> (ZVS) or off at <em className="italic text-text">zero current</em> (ZCS), driving the overlap
