...
export default function Ch35HouseReplacingFixtures() {
  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <h2>Verify dead. Then verify dead again.</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>non-contact voltage tester (NCVT)</strong> — a handheld pen-style tester that detects the AC electric field around an energised conductor without making physical contact. Useful for locating live wires and identifying live breakers; unreliable as a final verification because it can give false negatives from low battery, indirect coupling, or shielded cable.</>}>non-contact voltage tester (NCVT)</Term>{' '}
      ...
      <p>
        where <strong>I<sub>body</sub></strong> is the current that flows through the human standing on the circuit
        (in amperes), <strong>V<sub>residual</sub></strong> is whatever voltage remains across the contact points
        (in volts), and <strong>R<sub>body</sub></strong> is the total impedance of the path through the body and
      ...
      <TryIt
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>the NCVT lied; you skipped step two of the verify-dead ritual</strong>.
            </p>
          </>
        }
      />

      <h2>The anatomy of a duplex receptacle, inside the box</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>break-off tab</strong> — the small stamped bridge between paired terminal screws on a duplex receptacle. Twisting it off with pliers electrically isolates the top half of the device from the bottom half, allowing each outlet to be fed independently (split receptacle, switched outlet).</>}>break-off tab</Term>{' '}
      ...
      <p>
        ...
        {' '}<Term def={<><strong>side-screw clamp</strong> — the terminal screw on the side of a receptacle or switch that compresses a hooked or pressure-plate-clamped conductor against a brass yoke. Mechanically robust; the only termination an electrician will trust for daisy-chained loads.</>}>side-screw clamp</Term>{' '}
      ...
      <p>
        ...
        {' '}<Term def={<><strong>NEMA 5-15R</strong> — the most common North American 125 V, 15 A grounded receptacle pattern. Two parallel vertical slots (neutral wider than hot) and a round D-shaped ground hole below. NEMA WD 6 is the dimensional standard.</>}>5-15R</Term>{' '}
      ...
      <p>
        ...
        {' '}<Term def={<><strong>tamper-resistant (TR)</strong> — a receptacle with internal spring-loaded shutters that block the slots unless both blades of a plug are inserted simultaneously. Required by NEC 406.12 in dwelling units to prevent child electrocution from inserted objects.</>}>TR (tamper-resistant)</Term>,
        ...
        {' '}<Term def={<><strong>weather-resistant (WR)</strong> — a receptacle constructed with UV-stabilised plastics, corrosion-resistant metal, and seals against moisture. Required by NEC 406.9 in damp and wet locations such as outdoor and bathroom installations.</>}>WR (weather-resistant)</Term>,
      ...
      <p>
        where <strong>P<sub>joint</sub></strong> is the power dissipated in the metal-to-metal contact (in watts),
        <strong> I</strong> is the load current passing through the joint (in amperes RMS), and
        <strong> R<sub>contact</sub></strong> is the contact resistance between the leaf-spring brass and the
      ...
      <p>
        The fix is mechanical, not subtle. <em>Never use backstabs on daisy-chained outlets.</em> Instead, cut a
        {' '}<Term def={<><strong>pigtail</strong> — a short stub of conductor (usually 6 inches) spliced to the incoming and outgoing branch-circuit conductors with a wire nut or push-in connector, with only the free end landed on the device terminal. Removes the device from the current path of downstream loads.</>}>pigtail</Term>{' '}
      ...
      <TryIt
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>about 8.6 W dissipated; well past the safe envelope for an enclosed
      ...
      <h2>Switch leg vs hot leg: diagnosing what is actually in the box</h2>
      ...
      <p>
        The first is the simple {' '}<Term def={<><strong>hot leg</strong> — the wiring arrangement in which the branch-circuit hot conductor enters the switch box, is interrupted by the switch, and continues on to the fixture. The neutral runs parallel through the same cable. Easiest to diagnose; standard since NEC 2011 required a neutral in every switch box.</>}>hot-leg</Term>{' '}
        ...
        {' '}<Term def={<><strong>switch leg</strong> — the conductor on the load side of a switch that runs to the fixture. Energised only when the switch is closed. Identified at the device by being the only conductor that goes dead when the switch is flipped off, with hot still present at the other terminal.</>}>switch leg</Term>{' '}
      ...
      <TryIt
        ...
        answer={
          <>
            <p>
              Two 14-2 cables in the box does <em>not</em> by itself confirm a neutral is available at the switch.
            ...
            <p>
              Answer: <strong>maybe, depending on whether a neutral splice is present in the box</strong> —
      ...
      <h2>Replacing a single-pole switch, a three-way, and a smart dimmer</h2>
      ...
      <p>
        ...
        mode: {' '}<Term def={<><strong>leading-edge dimming</strong> — phase-cut dimming that chops the start of each AC half-cycle. Uses a triac. Compatible with incandescent and magnetic-low-voltage loads; often poorly compatible with LED drivers.</>}>leading-edge</Term>{' '}
        ...
        {' '}<Term def={<><strong>trailing-edge dimming</strong> — phase-cut dimming that chops the tail of each AC half-cycle. Uses a MOSFET. Compatible with most LED drivers and electronic-low-voltage transformers; smoother dimming.</>}>trailing-edge</Term>{' '}
      ...
      <h2>Replacing a fixture: paddle fan, recessed can, pendant</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>mounting strap</strong> — the flat metal bar that screws across the front of a ceiling box and provides threaded holes for the fixture's mounting screws. Sometimes called a crossbar. Most surface-mount and pendant fixtures attach to the strap, not directly to the box.</>}>mounting strap</Term>{' '}
        (a flat metal bar screwed across the front of the box, providing two threaded holes for the fixture's
        own screws), the {' '}<Term def={<><strong>hickey</strong> — a threaded coupling that joins a pendant's stem to a threaded nipple in the ceiling box. Allows the hanging cord and conductors to pass through. Old electrician's term, still in the catalogues.</>}>hickey</Term>{' '}
      ...
      <TryIt
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>no, swap to a fan-rated box with a between-joists brace</strong>.
            </p>
          </>
        }
      />

      <h2>GFCI replacement and the LINE vs LOAD terminals</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>LINE (GFCI)</strong> — the input terminals of a GFCI receptacle, where the upstream branch-circuit conductors land. Power flows in at LINE; the differential sensor sits between LINE and the front-face outlets and LOAD.</>}>LINE</Term>{' '}
        and {' '}<Term def={<><strong>LOAD (GFCI)</strong> — the output terminals of a GFCI receptacle, where downstream daisy-chained receptacles are fed. Anything wired to LOAD is GFCI-protected by the upstream device; anything wired only to LINE is not.</>}>LOAD</Term>.
      ...
      <TryIt
        ...
        answer={
          <>
            ...
            <p>
              The downstream receptacles are <em>not</em> GFCI-protected. They are wired to the upstream side
            ...
            <p>
              Answer: <strong>no, the breaker does not trip; no, the downstream outlets are not
      ...
      <FAQ intro="Questions readers ask the first time they pull a device out of a wall.">
        ...
        <FAQItem q="Why is the elementary charge in CODATA cited in a DIY chapter?">
          <p>
            ...
            through a cross-section, scaled by the CODATA 2018 fixed value of <em>e</em> = 1.602176634×10⁻¹⁹ C
