...
export default function Ch36HouseTroubleshooting() {
  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <h3>The non-contact voltage tester</h3>
      <p>
        A {' '}
        <Term def={<><strong>NCVT</strong> (non-contact voltage tester) — a pen-shaped tool that senses the AC electric field around a live conductor through its insulation. Yes/no presence test only; reads on capacitive coupling, not on conduction.</>}>NCVT</Term>{' '}
      ...
      <h3>The two-pole probe</h3>
      <p>
        A {' '}
        <Term def={<><strong>two-pole probe</strong> — a contact voltage tester with two metal tips, internal solenoid or filament load, and a backlit display or LED bar. Examples: Fluke T5, Fluke T6, Wiggy. Loads the circuit enough (mA, not µA) to swamp capacitive leakage; the verify-dead standard.</>}>two-pole probe</Term>{' '}
      ...
      <h3>The digital multimeter</h3>
      <p>
        A {' '}
        <Term def={<><strong>DMM</strong> (digital multimeter) — the general-purpose voltmeter / ammeter / ohmmeter / continuity tester. Typical residential-grade units present 10 MΩ input impedance on the volts ranges. Reads anything but is fooled by phantom voltage on floating conductors.</>}>DMM</Term>{' '}
      ...
      <p>
        where <strong>V<sub>dmm</sub></strong> is the voltage the meter displays (in volts AC),
        <strong> V<sub>source</sub></strong> is the actual line voltage somewhere upstream in the building (in
        volts AC, typically 120 V at a North-American outlet), <strong>Z<sub>dmm</sub></strong> is the input
        impedance of the meter on its volts range (in ohms; typically 10 MΩ for a handheld
        DMM)<Cite id="keysight-34465a-datasheet" in={SOURCES} />, and <strong>Z<sub>leak</sub></strong> is the
      ...
      <h3>The clamp meter</h3>
      <p>
        A {' '}
        <Term def={<><strong>clamp meter</strong> — a current meter with a hinged ferromagnetic (or Hall-effect) jaw that closes around a single conductor. Reads AC current (and on Hall-effect units, DC current) without breaking the circuit. Resolution typically 0.1 A; range 0–600 A AC.</>}>clamp meter</Term>{' '}
        is the only one of the four meters that reads <em>current</em> directly. Its hinged jaw closes around a
      ...
      <TryIt
        tag="Try 36.1"
        question={
          <>A DMM reads <strong>87 V</strong> on a wire that should be dead. The meter's input impedance is
          10 MΩ. The wire runs parallel to a live 120 V conductor for about a foot inside a box, giving a
          coupling capacitance of about <strong>30 pF</strong>. Verify the phantom-voltage prediction using the
        ...
        answer={
          <>
            <p>The impedance of a 30 pF capacitor at 60 Hz:</p>
            <Formula>Z<sub>leak</sub> = 1 / (2π × 60 × 30×10⁻¹²) ≈ 88 MΩ</Formula>
            <p>The voltage divider with a 10 MΩ DMM input:</p>
            ...
            <p>
              ...
              damp insulation. Either way the cure is the same: <strong>switch to a two-pole probe</strong>. Its
              ~3 kΩ load impedance reduces V<sub>dmm</sub> to V<sub>source</sub> × 3 kΩ / (3 kΩ + 88 MΩ) ≈
              <strong> 4 mV</strong>, indistinguishable from zero. The wire is dead.
            </p>
          </>
        }
      />

      <h2>The phantom-voltage trap, deeply</h2>
      ...
      <p>
        where <strong>I<sub>leak</sub></strong> is the displacement current the coupling capacitor injects into
        the floating wire U (in amperes, AC), <strong>ω = 2πf</strong> is the angular frequency of the line
        (in rad/s; ω ≈ 377 rad/s at 60 Hz), <strong>C<sub>coupling</sub></strong> is the capacitance between H
        and U (in farads), and <strong>V<sub>hot</sub></strong> is the RMS voltage of the hot wire with respect
      ...
      <p>
        ...
        DMMs have a {' '}
        <Term def={<><strong>low-Z mode</strong> — a DMM range that switches an internal ~3 kΩ load across the input terminals during the voltage measurement. Collapses phantom voltages on floating conductors to a few millivolts. Found on Fluke 117, 87V, T6, and equivalents.</>}>low-Z mode</Term>{' '}
      ...
      <TryIt
        ...
        question={
          ...
          conductor for a different branch circuit. The DMM reads <strong>62 V</strong> from the dead hot screw
          to ground. Switching the same meter to its <strong>low-Z mode</strong>, the reading drops to
          <strong> 0.4 V</strong>. Is the receptacle truly dead, and what was the meter doing in the first
          reading?</>
        }
        ...
        answer={
          <>
            <p>
              ...
              across the load, and the meter rounds to 0.4 V. <strong>The 0.4 V is the real reading.</strong>
            </p>
            <p>
              Practical rule: a voltage reading on a supposedly-dead wire that drops more than 10× when you
              engage low-Z is a phantom, not a fault. A reading that <em>doesn't</em> drop is a real source —
      ...
      <h2>The dead-outlet diagnostic flowchart</h2>
      ...
      <h3>Branch A — L-N reads 120 V</h3>
      <p>
      ...
      <h3>Branch B — L-N reads 0 V, L-G reads 120 V</h3>
      <p>
        Hot is live, but the neutral has gone {' '}
        <Term def={<><strong>open neutral</strong> — a break in the grounded conductor between the receptacle and the panel's neutral bar. The hot conductor remains energised; loads see no voltage because there is no return path. Distinguished from a dead branch by a live L-G reading.</>}>open</Term>{' '}
      ...
      <h3>Branch C — L-N reads 120 V but N-G reads 5–20 V instead of ≈ 0 V</h3>
      <p>
      ...
      <h3>Branch D — all three probes read 0 V</h3>
      <p>
        The branch is dead — no hot voltage at the outlet. The fault is upstream, in this order of likelihood:
      </p>
      <ol>
        <li>
          <strong>The breaker.</strong> Walk to the panel. Look for a breaker whose handle sits between full-ON
          and full-OFF — the {' '}
          <Term def={<><strong>half-trip</strong> — the resting position of a tripped breaker handle, partway between ON and OFF. To reset, push fully to OFF first (this re-cocks the internal latch), then back to ON. Flipping straight from half-trip to ON usually does nothing.</>}>half-trip</Term>{' '}
        ...
        <li>
          <strong>An upstream GFCI.</strong> The GFCI receptacle controlling this branch may be in a different
        ...
        <li>
          <strong>A loose connection in an upstream box.</strong> Backstab failures, loose wire-nuts, and burnt
        ...
        <li>
          <strong>The breaker's bus connection.</strong> Rare but real: the breaker reads ON, but its clip on
      ...
      <TryIt
        tag="Try 36.3"
        question={
          <>An outlet probe shows: <strong>L-N = 0 V, L-G = 121 V, N-G = 121 V</strong>. What's broken, and where
        ...
        answer={
          <>
            <p>
              ...
              if the neutral conductor is <strong>open</strong> upstream and is being capacitively-or-resistively
      ...
      <h2>Tripped breakers and what tripped them</h2>

      <p>
        Once the flowchart points back to the panel, the next question is <em>why</em> the breaker tripped.
      ...
      <p>
        A {' '}
        <Term def={<><strong>thermal trip</strong> — a breaker trip caused by sustained overload heating a bimetallic strip past its release setpoint. Time to trip ranges from seconds at moderate overload to minutes near the rated current. Resets only after the bimetal cools.</>}>thermal trip</Term>{' '}
        ...
        {' '}
        <Term def={<><strong>bimetal</strong> — a strip of two metals with different thermal expansion coefficients welded together. Bends predictably with temperature; used as the slow-trip element inside thermal-magnetic breakers.</>}>bimetal</Term>{' '}
        ...
        {' '}
        <Term def={<><strong>time-current curve</strong> — the log-log plot of trip time versus current for a circuit breaker. Steeply downward-sloping in the thermal region (1.1–8× rated current) and near-vertical in the magnetic region (8× and above).</>}>time-current curve</Term>{' '}
      ...
      <p>
        where <strong>t<sub>trip</sub></strong> is the time from current onset to mechanical release (in
        seconds), <strong>I</strong> is the actual current through the breaker (in amperes RMS),
        <strong> I<sub>rating</sub></strong> is the breaker's nameplate rating (in amperes; 15 A and 20 A are
        the common residential branch ratings), and <strong>K</strong> is a calibration constant set by the
      ...
      <p>
        A {' '}
        <Term def={<><strong>magnetic trip</strong> — a breaker trip caused by a short-circuit-magnitude current generating enough flux in the breaker's solenoid coil to slam the trip lever open within milliseconds. Resets immediately because the bimetal never warmed.</>}>magnetic trip</Term>{' '}
      ...
      <h3>AFCI or GFCI electronic trip</h3>
      <p>
        An {' '}
        <Term def={<><strong>AFCI</strong> (arc-fault circuit interrupter) — a breaker (or receptacle) with a microcontroller that monitors the current waveform for the broadband-noise signature of an arc and trips when it detects one. Listed under UL 1699.</>}>AFCI</Term>{' '}
      ...
      <TryIt
        tag="Try 36.4"
        question={
          <>A <strong>15 A breaker</strong> trips every 8 minutes when a <strong>1500 W heater</strong> on a
        ...
        answer={
          <>
            <p>The heater's running current:</p>
            <Formula>I = P / V = 1500 / 120 = 12.5 A</Formula>
            <p>
              12.5 A is <strong>83% of the 15 A rating</strong> — over the NEC 80% continuous-load limit but
              ...
              <strong> thermal trip</strong> driven by ambient heating, not a magnetic trip.
            </p>
            <p>
      ...
      <h2>The clamp meter and what's drawing what</h2>
      ...
      <TryIt
        ...
        question={
          ...
          <strong> 23 A</strong>. You switch off the central air conditioner and the reading drops to
          <strong> 8 A</strong>. The AC nameplate says <strong>240 V, 14 SEER</strong>. What is the AC's
        ...
        answer={
          <>
            <p>The AC's running current and electrical power:</p>
            <Formula>I<sub>AC</sub> = 23 − 8 = 15 A on L1 (same on L2 for a 240 V load)</Formula>
            <Formula>P<sub>elec</sub> = 240 × 15 = 3 600 W</Formula>
            <p>SEER 14 means about 14 BTU of cooling per Wh of electrical input, so:</p>
            <Formula>Q<sub>cool</sub> ≈ 14 BTU/Wh × 3 600 W = 50 400 BTU/hr</Formula>
            <p>
              Answer: <strong>15 A, ~3.6 kW electrical, ~50 000 BTU/hr cooling</strong>. That matches a
      ...
      <h2>The "dead breaker" failure mode</h2>
      ...
      >
        ...
        >
          <p>
            ...
            <strong> 9.2 A</strong>, against an expected 2 A for the always-on load (refrigerator compressor,
      ...
      <FAQ intro="Questions readers ask after their first dead-outlet diagnosis.">
        ...
        <FAQItem q="My DMM measures 120.4 V on a good outlet, but the panel says 240 V should be there. Which is right?">
          <p>
            Both. A North-American split-phase service delivers 240 V <em>line-to-line</em> (between L1 and
            L2) and 120 V <em>line-to-neutral</em> (between either hot bus and the centre-tapped grounded
        ...
        <FAQItem q="Why does every current value in this chapter ultimately reduce to the elementary charge?">
          <p>
            ...
            electron charge <em>e</em><Cite id="codata-2018" in={SOURCES} />. Every 5 mA GFCI threshold,
