  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <p>
        ...
        ampacity tables come from conductor heating; voltage drop comes from <em>I·R</em>; the bond-once-only rule comes
      ...
      <h2>Step 0: confirm the panel has headroom</h2>
      ...
      <p>
        where <strong>I_demand</strong> is the calculated service demand current (in amperes RMS) that the panel needs
        to be able to carry, <strong>W_first10k</strong> is the first 10 000 VA of summed connected load (taken at
        100%), <strong>W_remainder</strong> is everything above 10 000 VA (taken at 40% to account for
        {' '}<Term def={<><strong>demand factor</strong> — the empirical fraction of nameplate load that a household will actually draw simultaneously. The dryer is on or the oven is on but rarely both at full tilt at the same moment as the air conditioner. Codified in NEC Article 220 as the diversity multipliers in 220.42 (Standard) and 220.82 (Optional).</>}>diversity</Term>{' '}
        — almost no household actually runs the dryer, the oven, the heat pump and the dishwasher all at full power
        simultaneously), and <strong>V</strong> is the service voltage (240 V for a standard split-phase residential
      ...
      <TryIt
        tag="Try 37.1"
        question={
          <>A 200 A service currently runs <strong>95 A demand</strong> under the 220.82 Optional Method. The owner
          wants to add a hardwired <strong>50 A, 240 V</strong> Level-2 EV charger as a continuous load. Does the
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>yes, there is headroom</strong>. The new total demand is roughly 158 A on a 200 A
      ...
      <h2>Step 1: pick the device, the device picks the wire</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>NEMA 5-20R</strong> — the 20 A, 125 V receptacle (one slot horizontal, one slot vertical with a T shape, plus the round ground). Will also accept a NEMA 5-15P plug. The standard 20 A receptacle for kitchen, garage, and dedicated-appliance branches.</>}>NEMA 5-20R</Term>{' '}
      ...
      <p>
        where <strong>I_conductor</strong> is the minimum allowable conductor ampacity (in amperes), <strong>I_continuous</strong>
        is the portion of the branch's load that is continuous (in amperes), and <strong>I_noncontinuous</strong> is
      ...
      <p>
        ...
        {' '}<strong>12 AWG copper</strong>: a single solid copper conductor 2.05 mm in diameter, rated for 20 A under
      ...
      <p>
        ...
        {' '}<Term def={<><strong>ampacity</strong> — the current a conductor can carry continuously under defined conditions of installation without exceeding its insulation's temperature rating. Tabulated in NEC Table 310.16 by gauge, insulation type, and ambient temperature.</>}>ampacity</Term>{' '}
      ...
      <p>
        where <strong>V_drop</strong> is the total round-trip voltage drop seen by the load (in volts),
        <strong> I</strong> is the operating current (in amperes), <strong>R_per_ft</strong> is the conductor's
        resistance per foot (in ohms per foot; the table value for 12 AWG copper at 75 °C is approximately
        1.59 mΩ/ft, or about 0.00159 Ω/ft), <strong>L_oneway</strong> is the one-way length of the run from panel to
      ...
      <TryIt
        tag="Try 37.2"
        question={
          <>You are planning a <strong>75 ft</strong> (one-way) 12 AWG copper run from the panel to a
          <strong> 14 A continuous load</strong>. R per foot for 12 AWG copper is about <strong>1.59 mΩ/ft</strong>.
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>compliant, but at the limit</strong>. If the run length crept up by even ten feet (or if
      ...
      <h2>Step 2: pick the breaker</h2>

      <p>
        Here is the subtle inversion that surprises most first-time DIY installers. The breaker is not picked to match
        the load; it is picked to match the <em>wire</em>. NEC 240.4 ties the maximum overcurrent device rating to the
      ...
      <p>
        ...
        {' '}<Term def={<><strong>AFCI</strong> (arc-fault circuit interrupter) — a protective device that monitors the current waveform for the high-frequency signature of an electrical arc and trips on it. Required in most dwelling-unit living spaces by NEC 210.12 since 2014. Listed under UL 1699.</>}>AFCI</Term>{' '}
        protection in essentially every dwelling-unit living area. The simplest way to satisfy both in one breaker is a
        {' '}<Term def={<><strong>dual-function breaker</strong> — a breaker that combines AFCI and GFCI protection in a single device. NEC 2023 increasingly defaults to dual-function on any circuit that triggers either requirement, since the two functions are now standard on the same silicon module.</>}>dual-function breaker</Term>{' '}
      ...
      <p>
        ...
        {' '}<Term def={<><strong>nail plate</strong> — a steel plate at least 1/16 inch thick that is nailed over a hole or notch in a stud or joist to protect the cable inside from a nail or screw driven through the wall from the room side. Required by NEC 300.4(A) and (D) wherever wood-to-cable clearance is less than 1.25 inches.</>}>nail plate</Term>{' '}
      ...
      <p>
        {' '}<Term def={<><strong>EMT</strong> (electrical metallic tubing) — a thin-walled steel conduit that conductors are pulled through after installation. The standard for surface-mounted runs in garages, workshops, unfinished basements, and any commercial occupancy.</>}>EMT</Term>{' '}
      ...
      <p>
        ...
        {' '}<Term def={<><strong>box fill</strong> — the volumetric accounting required by NEC 314.16 to confirm a junction or device box is large enough to safely contain its conductors, devices, clamps, and grounds. Each item counts as a fixed number of "conductor equivalents" multiplied by the per-conductor volume from Table 314.16(B).</>}>box-fill</Term>{' '}
      ...
      <p>
        where <strong>V_box</strong> is the labelled internal volume of the box (in cubic inches),
        <strong> n_cond</strong> is the number of insulated current-carrying conductors entering and remaining in the
        box (in count), <strong>v_cond</strong> is the volume allowance per conductor for that conductor's gauge from
        NEC Table 314.16(B) (in cubic inches; 2.00 for 14 AWG, 2.25 for 12 AWG, 2.50 for 10 AWG),
        <strong> V_devices</strong> is two conductor-volumes per yoke or strap that holds a device (receptacle or
        switch), <strong>V_grounds</strong> is one conductor-volume total for the entire bundle of grounding
        conductors regardless of how many there are, and <strong>V_clamps</strong> is one conductor-volume total if
      ...
      <TryIt
        tag="Try 37.3"
        question={
          <>A 4-square steel box rated <strong>21 in³</strong> has 1 receptacle, four 14 AWG insulated conductors
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>conforming with 7 in³ of margin</strong>. Adding a second receptacle on a duplex strap
      ...
      <h2>Step 6: make up the connections</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>pigtail</strong> — a short 6 inch piece of conductor wirenutted to a daisy-chain splice and run to the device. Keeps the daisy chain electrically continuous if the device is removed or fails open, and concentrates the splice geometry inside the box rather than across the device's mechanical strap.</>}>pigtail</Term>{' '}
      ...
      <h2>Step 7: land at the panel</h2>
      ...
      <p>
        ...
        {' '}<Term def={<><strong>approach boundary</strong> — the distance from energised parts inside which only qualified electrical workers may operate, defined in NFPA 70E. The "Limited Approach Boundary" for 250 V residential service is roughly 1.5 m; inside the "Restricted Approach Boundary" energised work requires explicit PPE and an energised-work permit.</>}>approach boundary</Term>{' '}
      ...
      <p>
        Remove the
        {' '}<Term def={<><strong>dead-front</strong> — the metal cover with breaker windows that sits between the panel interior and the room. Lifting it exposes the bus bars and the line-side lugs of the main breaker. NEC 408 requires it to be properly secured at every interior access.</>}>dead-front</Term>{' '}
        ...
        {' '}<Term def={<><strong>bus stab</strong> — one of the slotted copper fingers projecting laterally from a bus bar that a breaker clip grabs to make contact. Adjacent stabs alternate between L1 and L2 (Ch.28).</>}>bus stab</Term>;
      ...
      <p>
        ...
        {' '}<Term def={<><strong>neutral bar</strong> — the terminal strip in a panel that receives every white (grounded) conductor from every branch circuit. In the main panel it is bonded to the panel can through the main bonding jumper; in any sub-panel it floats relative to the can (Ch.28).</>}>neutral bar</Term>{' '}
      ...
      <p>
        ...
        {' '}<Term def={<><strong>main bonding jumper</strong> — the connection between the neutral bar and the ground bar / panel can. NEC 250.24 requires exactly one in the entire premises wiring system, at the first means of disconnect. At every sub-panel the jumper is explicitly removed.</>}>main bonding jumper</Term>{' '}
      ...
      <TryIt
        tag="Try 37.4"
        question={
          <>You are running a <strong>100 ft</strong> (one-way) branch of 14 AWG NM-B to a hardwired
          <strong> 1200 W</strong> garage door opener that runs continuously (NEC's three-hour rule does not actually
          apply here, but assume worst-case for the calculation). R per foot for 14 AWG copper is approximately
          <strong> 2.52 mΩ/ft</strong>. What is the round-trip voltage drop at full load? Is 14 AWG sufficient by
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>14 AWG fails the 3% recommendation; upsize to 12 AWG</strong>. The 15 A breaker that
      ...
      <TryIt
        tag="Try 37.5"
        question={
          <>A NEMA 5-20 receptacle is installed at the end of a <strong>30 ft</strong> 12 AWG branch from the panel,
          on a 20 A dual-function breaker. The owner plans to run a worm-drive table saw drawing <strong>24 A
        ...
        answer={
          <>
            ...
            <p>
              Answer: <strong>not compliant</strong>. The fix is a full rebuild — replace the conductor with 10 AWG,
      ...
      >
        ...
        >
          ...
          <p>
            ...
            that screw is <em>removed</em>, leaving the neutral bar electrically floating relative to the can. The
          ...
          <p>
            ...
            reads near zero with the subpanel unloaded; on the equipment ground it reads <em>exactly</em> zero, which
      ...
      <FAQ intro="Common questions on how the rules of this chapter line up against the physics of earlier ones.">
        ...
        <FAQItem q="Why is every current on every label in this chapter still a count of CODATA elementary charges?">
          <p>
            ...
            CODATA at <em>e</em> = 1.602176634×10⁻¹⁹ C exactly<Cite id="codata-2018" in={SOURCES} />. A 20 A branch
