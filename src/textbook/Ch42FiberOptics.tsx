  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <p>
        ...
        about <strong>85.2°</strong>. Equivalently, the ray's angle measured{' '}
        <em>from the fiber axis</em> must stay below <strong>4.8°</strong>. Steeper than
      ...
      <h2>Numerical aperture and the acceptance cone</h2>
      <p>
        ...
        the fiber's <Term def={<><strong>numerical aperture</strong> NA — the sine of the maximum half-angle of the entry cone in air. Determined by the index step between core and cladding: NA = √(n_core² − n_clad²). A single-mode fiber typically has NA ≈ 0.14, accepting rays within ±8° of the axis.</>}>numerical aperture</Term> NA, and it's set entirely by the index step<Cite id="agrawal-2010" in={SOURCES} />:
      </p>
      <Formula>NA = √(n_core² − n_clad²)</Formula>
      <p>
        where <strong>NA</strong> is dimensionless, <strong>n_core</strong> and{' '}
        <strong>n_clad</strong> are the refractive indices of the core and the surrounding
      ...
      <TryIt
        ...
        answer={<>
          ...
          <p>Acceptance half-angle: <strong>≈ 20°</strong>. The 4% index step here is much larger than telecom SMF (~0.3%); this is closer to a graded-index multimode datacom fiber.</p>
        </>}
      />

      <h2>Single-mode versus multimode: the V parameter</h2>
      <p>
        How small does the core need to be to admit only one transverse mode? The cutoff
        is governed by a single dimensionless number, the <Term def={<><strong>V parameter</strong> — also called the normalized frequency. Combines core radius, wavelength, and NA into one number that controls how many transverse modes a step-index fiber supports. V &lt; 2.405 ⇒ single-mode.</>}>V parameter</Term><Cite id="saleh-teich-2007" in={SOURCES} />:
      </p>
      <Formula>V = (2π a / λ) · NA</Formula>
      <p>
        where <strong>a</strong> is the core radius (metres), <strong>λ</strong> the
        free-space wavelength (metres), and <strong>NA</strong> the numerical aperture.
      ...
      <TryIt
        tag="Try 42.2"
        question={<>You're designing a multimode fiber with core radius <strong>a = 25 μm</strong> and <strong>NA = 0.20</strong> for an 850 nm datacom link. How many transverse modes does it support? (For step-index fibers, the mode count is roughly <em>V²/2</em>.)</>}
        hint={<>Compute V at λ = 850 nm, then divide V²/2.</>}
        answer={<>
          <p>V parameter:</p>
          <Formula>V = (2π × 25×10⁻⁶ / 850×10⁻⁹) × 0.20 ≈ 36.9</Formula>
          <p>Number of supported modes:</p>
          <Formula>N ≈ V²/2 ≈ 36.9² / 2 ≈ 681</Formula>
          <p>About <strong>680 modes</strong> — heavily multimode, which is why this fiber is bandwidth-limited by modal dispersion and runs at 850 nm where a cheap VCSEL is enough.</p>
        </>}
      />

      <h2>Loss: why 1550 nm won</h2>
      <p>
        The other dimension of the problem is attenuation. A real fiber doesn't carry
        light losslessly — it has <Term def={<><strong>attenuation</strong> α — the exponential decay rate of optical power along the fiber, measured in dB/km. Comes from Rayleigh scattering off molecular-scale density fluctuations frozen into the glass when it solidified, plus a small infrared absorption tail from the SiO₂ lattice. Modern single-mode silica at 1550 nm reaches ~0.18 dB/km.</>}>attenuation</Term> from two physical sources: <Term def={<><strong>Rayleigh scattering</strong> — elastic scattering of light off density variations smaller than its wavelength. Scales as 1/λ⁴, so it dominates at short wavelengths and falls dramatically at longer ones. The fundamental floor for fiber loss.</>}>Rayleigh scattering</Term> from molecular-scale density fluctuations frozen into the glass, plus infrared absorption from the SiO₂ lattice. Rayleigh scaling goes as 1/λ⁴, so longer wavelengths bleed less; lattice absorption rises sharply past 1.6 μm. The sweet spot is at <strong>1550 nm</strong>, where modern silica reaches roughly <strong>0.18–0.20 dB/km</strong>. That figure was first demonstrated by NTT in 1979 and remains within a factor of two of the theoretical Rayleigh floor<Cite id="miya-1979" in={SOURCES} />.
      </p>
      ...
      <p>
        ...
        about <strong>1000 dB/km</strong> — roughly a doubling every 3 metres — and was
      ...
      <h3>The two standard windows</h3>
      <p>
        Two operating wavelengths dominate practical deployments<Cite id="agrawal-2010" in={SOURCES} />:
      </p>
      <ul>
        <li>
          <strong>1310 nm</strong> — the "zero-dispersion" window of standard SMF.
          ...
          Used in <strong>FTTH</strong> (fiber-to-the-home), passive optical networks,
          and most datacenter interconnects up to 10 km.
        </li>
        <li>
          <strong>1550 nm</strong> — the minimum-loss window. Standard single-mode
          ...
          (1525–1565 nm, the <strong>C-band</strong>). One EDFA every 80 km on a
      ...
      <p>
        ...
        Powers in fiber optics are quoted in <Term def={<><strong>dBm</strong> — decibels relative to 1 milliwatt: P(dBm) = 10·log₁₀(P/1 mW). +0 dBm = 1 mW; +10 dBm = 10 mW; −30 dBm = 1 μW. Convenient because cascading losses add instead of multiplying.</>}>dBm</Term> precisely so that this subtraction is the natural arithmetic:
      </p>
      <Formula>P_rx = P_tx − L_connectors − α · L</Formula>
      <p>
        where <strong>P_rx</strong> is the power arriving at the receiver (dBm),{' '}
        <strong>P_tx</strong> is the power launched by the transmitter (dBm),{' '}
        <strong>L_connectors</strong> is the total connector + splice loss in the link
        (dB), <strong>α</strong> is the fiber attenuation (dB/km), and <strong>L</strong>{' '}
        ...
        <Term def={<><strong>link budget</strong> — the total optical loss the link can tolerate while still maintaining a working receiver, expressed as the dB margin between launched power and receiver sensitivity after subtracting fixed insertion losses.</>}>link budget</Term>; divide by α and you have the reach.
      </p>
      ...
      <TryIt
        tag="Try 42.3"
        question={<>A 1550 nm DFB laser launches <strong>+3 dBm</strong> into a span of ITU-T G.652 fiber at <strong>0.22 dB/km</strong>. The receiver sensitivity is <strong>−27 dBm</strong> and accumulated splice loss is <strong>3 dB</strong>. What's the maximum reach without an amplifier?</>}
        hint={<>Link budget = launch − splice − sensitivity. Divide by α.</>}
        answer={<>
          <p>Budget available for fiber loss:</p>
          <Formula>Budget = 3 − 3 − (−27) = 27 dB</Formula>
          <p>Reach at 0.22 dB/km:</p>
          <Formula>L = 27 / 0.22 ≈ 123 km</Formula>
          <p>Answer: <strong>~123 km</strong>. That's typical of a modern metro span; long-haul backbones place an EDFA every 60–100 km to extend reach to thousands of km.</p>
        </>}
      />

      <h2>Dispersion: the bandwidth limit</h2>
      <p>
        Loss caps the <em>reach</em> of a link; <Term def={<><strong>dispersion</strong> — the pulse-broadening that occurs when different components of a pulse (different transverse modes, or different wavelengths within a pulse) travel at slightly different group velocities and arrive smeared in time. Limits the <em>bit rate × distance</em> product of a link.</>}>dispersion</Term> caps its <em>data rate × distance product</em>. There are two dominant kinds in modern fiber<Cite id="agrawal-2010" in={SOURCES} />:
      </p>
      <ul>
        <li>
          <strong>Modal dispersion</strong> — present only in multimode fiber. Different
        ...
        <li>
          <strong>Chromatic dispersion</strong> — present in every fiber. Comes from the
          wavelength-dependence of the refractive index <em>n(λ)</em>: a laser pulse with
          some spectral width Δλ has its different wavelength components arrive at slightly
          different times. Quantified by the dispersion coefficient <em>D</em> in
      ...
      <p>
        ...
        <strong>Δt</strong> is the pulse-spread in picoseconds, <strong>D</strong> the
        dispersion coefficient (ps/nm·km), <strong>Δλ</strong> the spectral width of the
        source (nm), and <strong>L</strong> the fiber length (km). For coherent 100 G
      ...
      <h2>WDM, EDFAs, and the petabit pipe</h2>
      <p>
        ...
        <strong>80–96 channels</strong> on the ITU-T G.694.1 grid<Cite id="itu-t-g694-1" in={SOURCES} />. <Term def={<><strong>DWDM</strong> — Dense Wavelength Division Multiplexing. Combines multiple optical wavelengths onto a single fiber by sending each channel on a distinct ITU-grid wavelength and separating them at the receive end with a thin-film optical filter or a planar lightwave circuit. The reason a single submarine fiber pair can carry tens of Tb/s.</>}>DWDM</Term> stacks all 96 channels into one fiber pair; an array of <Term def={<><strong>EDFA</strong> — Erbium-Doped Fiber Amplifier. A meter of silica fiber doped with erbium ions, pumped by a 980 nm or 1480 nm laser. The pumped erbium amplifies any wavelength in 1525–1565 nm by stimulated emission — all channels at once. Replaced electronic repeaters in long-haul systems starting in the early 1990s.</>}>erbium-doped fiber amplifiers</Term> every 80 km lifts the whole comb back to launch power without ever converting to electronics<Cite id="desurvire-1987" in={SOURCES} />.
      </p>
      <p>
        Multiply: 96 channels × 100 Gb/s = <strong>9.6 Tb/s</strong> per fiber. A typical
        submarine cable contains 8–24 fibers, so a single cable carries <strong>~100 Tb/s
        to ~250 Tb/s</strong>. Modern record-setting links push to <strong>~25 Tb/s per fiber pair</strong>{' '}
      ...
      <CaseStudies intro="Where this physics shows up in working systems.">
        ...
        >
          <p>
            ...
            The fiber loss is about <strong>0.155 dB/km</strong> — only ~10% above
        ...
        >
          <p>
            ...
            SR4 module sits a 4-lane <strong>VCSEL</strong> array (Vertical
      ...
      <FAQ>
        <FAQItem q="If light bounces off the cladding, where exactly does the energy go?">
          ...
          limit. A thin <strong>evanescent wave</strong> does extend into the cladding by
        ...
        <FAQItem q="How small is the core of a single-mode fiber, really?">
          About <strong>8–10 μm</strong> in diameter (4–5 μm radius), surrounded by
        ...
        <FAQItem q="What's the physical limit on fiber loss?">
          ...
          <strong>0.14 dB/km</strong>, set by molecular-scale density fluctuations
        ...
        <FAQItem q="Why doesn't the fiber leak light at a bend?">
          ...
          has a minimum bend radius of about <strong>30 mm</strong> for negligible loss;
          purpose-built <em>bend-insensitive</em> fibers (ITU-T G.657) drop that to ~10 mm
        ...
        <FAQItem q="What actually goes wrong if you connect two fibers with a 1 μm misalignment?">
          ...
          offset costs roughly <strong>0.2 dB</strong> of coupling loss; a 5 μm offset
        ...
        <FAQItem q="How does the receiver decode the bits if dispersion smears them?">
          A modern <strong>coherent receiver</strong> mixes the incoming light with a
        ...
        <FAQItem q="What's the relationship between fiber dispersion and Maxwell's equations?">
          ...
          <em>n(λ)</em> propagating through a wavepacket: the group velocity{' '}
          <em>v_g = c / (n + ω · dn/dω)</em> depends on frequency, so a pulse made of
          multiple frequencies spreads as it travels. The <em>n(λ)</em> dispersion of
