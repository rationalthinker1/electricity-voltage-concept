...
export default function Ch5Capacitors() {
  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <p className="mb-prose-3">
        Move a second electron from bottom to top. <em className="italic text-text">This</em> one costs work. The field set up by the first electron points
        from the bottom plate (now slightly positive) to the top plate (now slightly negative), and the second electron, which
        is itself negative, gets a push <em className="italic text-text">toward</em> the bottom plate. You have to overcome that push to get the electron
      ...
      <p className="mb-prose-3">
        ...
        constant of proportionality the body's <em className="italic text-text">capacity</em><Cite id="volta-1782" in={SOURCES} />. We call the same
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">Q</strong> is the magnitude of charge on each plate (in coulombs; the two plates carry +Q and −Q),
        <strong className="text-text font-medium"> V</strong> is the voltage across the gap (in volts), and <strong className="text-text font-medium">C</strong> is the capacitance of the
        ...
        separated by a vacuum gap of width <strong className="text-text font-medium">d</strong>, Gauss's law applied to the surface of one plate gives a
        uniform field <strong className="text-text font-medium">E = Q/(ε₀A)</strong> in the gap, and integrating that field across the gap yields a voltage
        <strong className="text-text font-medium"> V = Ed = Qd/(ε₀A)</strong>. Re-arranging<Cite id="griffiths-2017" in={SOURCES} />:
      </p>
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium"> εᵣ</strong>, the material's <Term def="Dimensionless number εᵣ giving how much a dielectric reduces the field for the same charge on the plates. Vacuum is 1; air ≈ 1; water ≈ 80; specialised ceramics push past 1000.">relative permittivity</Term><Cite id="jackson-1999" in={SOURCES} />.
      </p>
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium"> σ = Q/A</strong> sits on the inner faces of the two plates as equal and opposite sheets;
        between them, the field is the same everywhere — uniform in magnitude and direction — with
        <strong className="text-text font-medium"> E = σ/ε₀ = V/d</strong>. Toggle the Gauss pillbox to see the operational statement of
        ...
        <strong className="text-text font-medium"> ∮ D · dA</strong> comes from that bottom cap, and it equals exactly the charge
        <strong className="text-text font-medium"> σA</strong> on the slice of plate the pillbox encloses<Cite id="jackson-1999" in={SOURCES} />.
        ...
        answer={
          <>
            <p className="mb-prose-1 last:mb-0">
              Area: <strong className="text-text font-medium">A = (0.05 m)² = 2.5×10⁻³ m²</strong>. Gap: <strong className="text-text font-medium">d = 5×10⁻⁴ m</strong>.
              ε₀ = 8.854×10⁻¹² F/m<Cite id="codata-2018" in={SOURCES} />.
            </p>
            <Formula>C = ε₀ A / d = (8.854×10⁻¹²)(2.5×10⁻³) / (5×10⁻⁴)</Formula>
            <Formula>C ≈ <strong className="text-text font-medium">44 pF</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <p className="mb-prose-3">
        The constant <strong className="text-text font-medium">ε₀ ≈ 8.854×10⁻¹² F/m</strong><Cite id="codata-2018" in={SOURCES} /> is so small that real-world
        capacitors live in the picofarad-to-millifarad range. Two metal plates of 1 cm² area held 1 mm apart in vacuum give
        <strong className="text-text font-medium"> C ≈ 0.9 pF</strong>. A whole farad in air would need plates the size of a small lake. The reason the
      ...
      <p className="mb-prose-3">
        ...
        <strong className="text-text font-medium"> v = q/C</strong> across them. To move one more coulomb's worth of charge — a tiny <strong className="text-text font-medium">dq</strong> —
        from the negative plate to the positive plate, you have to do work <strong className="text-text font-medium">v·dq</strong> against the existing field.
        The cost per coulomb is exactly the present voltage. For a real, finite chunk of charge δq, the work is
        <strong className="text-text font-medium"> δq · v</strong>, which is to say <strong className="text-text font-medium">δq · q / C</strong> — proportional to how full the cap already is.
      </p>
      <p className="mb-prose-3">
        A worked example. Suppose you have a 10 µF capacitor that already carries 100 µC of charge, so V = Q/C = 10 V. Adding
        another 1 nC costs about <strong className="text-text font-medium">10⁻⁸ J</strong>. Adding 1 nC to a completely empty cap would have cost essentially
        zero. Adding 1 nC to the same cap when it already holds 200 µC would cost <em className="italic text-text">twice</em> as much as adding it at
      ...
      <TryIt
        ...
        answer={
          <>
            <p className="mb-prose-1 last:mb-0">
              Current voltage: <strong className="text-text font-medium">V = Q/C = 100 nC / 10 nF = 10 V</strong>.
              Work to add the next 1 nC:
            </p>
            <Formula>W ≈ δq · V = (1×10⁻⁹ C)(10 V) = <strong className="text-text font-medium">10 nJ</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">U</strong> is the total energy stored in the capacitor (in joules), <strong className="text-text font-medium">Q</strong> is the final
        charge on each plate (in coulombs), <strong className="text-text font-medium">V = Q/C</strong> is the final voltage across it (in volts),
        <strong className="text-text font-medium"> C</strong> is the capacitance (in farads), and the dummy variable <strong className="text-text font-medium">q</strong> runs from 0 to <em className="italic text-text">Q</em>
        ...
        linear — if the (N+1)<sup>th</sup> charge cost the <em className="italic text-text">same</em> as the first — the answer would be QV. Because each
      ...
      <p className="mb-prose-3">
        Now the more interesting question: <em className="italic text-text">where</em> is that energy? Not on the plates themselves — the plates are still
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">u<sub>E</sub></strong> is the electric-field energy per unit volume (in J/m³),
        <strong className="text-text font-medium"> ε₀ ≈ 8.854×10⁻¹² F/m</strong> is the vacuum permittivity <Cite id="codata-2018" in={SOURCES} />,
        <strong className="text-text font-medium"> ε<sub>r</sub></strong> is the dimensionless relative permittivity of whatever fills the gap (1 for vacuum),
        and <strong className="text-text font-medium">E</strong> is the local electric-field magnitude (in V/m).
      </p>

      <EnergyInTheGapDemo />

      <p className="mb-prose-3">
        Multiply this density by the gap volume <strong className="text-text font-medium">A·d</strong> for an idealised parallel-plate cap and you recover
        exactly <strong className="text-text font-medium">½ CV²</strong>. The two answers — the integrated work and the field-volume integral — are the same
      ...
      <Pullout>
        A capacitor doesn't store charge. The plates always net to zero. What it stores is the <em className="italic text-text">separation</em> —
      ...
      <TryIt
        ...
        answer={
          <>
            <Formula>U = ½ C V² = ½ (1×10⁻⁴)(12)² = <strong className="text-text font-medium">7.2×10⁻³ J ≈ 7.2 mJ</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
              And the charge separation:
            </p>
            <Formula>Q = C V = (100×10⁻⁶)(12) = 1.2×10⁻³ C = 1.2 mC</Formula>
            <Formula>N = Q / e = 1.2×10⁻³ / 1.602×10⁻¹⁹ ≈ <strong className="text-text font-medium">7.5×10¹⁵ electrons</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
      ...
      <p className="mb-prose-3">
        where <strong className="text-text font-medium">V<sub>C</sub>(t)</strong> is the voltage across the capacitor as a function of time (in volts),
        <strong className="text-text font-medium"> V<sub>0</sub></strong> is the (constant) source/battery voltage (in volts), <strong className="text-text font-medium">t</strong> is time
        elapsed since the switch closed (in seconds), <strong className="text-text font-medium">R</strong> is the series resistance (in ohms), and
        <strong className="text-text font-medium"> C</strong> is the capacitance (in farads). The product <strong className="text-text font-medium">RC</strong> has units of seconds and sets
      ...
      <TryIt
        ...
        answer={
          <>
            <Formula>τ = R C = (1×10⁴ Ω)(1×10⁻⁴ F) = <strong className="text-text font-medium">1 s</strong></Formula>
            <p className="mb-prose-1 last:mb-0">
            ...
            <Formula>5τ = <strong className="text-text font-medium">5 s</strong> &nbsp; (to reach ≈ 99% of V₀)</Formula>
          </>
        }
      />

      <p className="mb-prose-3">
        The product <strong className="text-text font-medium">τ = RC</strong> is the <Term def="τ = RC. The characteristic time for an RC circuit to respond — V_C reaches (1 − 1/e) ≈ 63% of its final value in one τ, and 99.3% by 5τ.">time constant</Term> — the time for V_C to reach (1 − 1/e) ≈ 63% of V₀.
      ...
      <p className="mb-prose-3">
        ...
        we ask where the energy actually <em className="italic text-text">flows</em> as a capacitor charges (Chapter 7), and again as one of the two
      ...
      >
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            <strong className="text-text font-medium"> 150 µF</strong> to roughly <strong className="text-text font-medium">2 kV</strong> and you have stored
            <strong className="text-text font-medium"> ½ C V² ≈ 300 J</strong> of energy — the order of a joule per kilogram of patient mass, enough to
            depolarise the heart muscle and reset its rhythm<Cite id="horowitz-hill-2015" in={SOURCES} />. Connect the
            terminals through the chest, whose transthoracic impedance is around <strong className="text-text font-medium">50 Ω</strong>, and the discharge
            time constant <strong className="text-text font-medium">τ = RC ≈ 7 ms</strong> is short enough to deliver the energy in a single biphasic pulse.
          </p>
          <p className="mb-prose-2 last:mb-0">
            Every piece of this chapter is in that device. The energy is sourced from a step-up charging circuit and
            <em className="italic text-text"> stored in the field</em> between the capacitor's plates; the discharge is governed by an RC time constant;
        ...
        >
          ...
          <p className="mb-prose-2 last:mb-0">
            The single underlying fact is the geometry rule <strong className="text-text font-medium">C = ε₀ εᵣ A / d</strong>: change the geometry, change the
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            so. Putting both into <strong className="text-text font-medium">C = ε₀ εᵣ A / d</strong> gives capacitances in the thousands of farads per
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            slowly trickles charge into a <strong className="text-text font-medium">~150 µF</strong> electrolytic capacitor over several seconds, building it
            up to roughly <strong className="text-text font-medium">330 V</strong>. The cap then holds <strong className="text-text font-medium">½ CV² ≈ 8 J</strong> in its field.
          </p>
          <p className="mb-prose-2 last:mb-0">
      ...
      >
        <FAQItem q="What does it mean to “store” charge if the total charge on a capacitor is zero?">
          <p>
            The capacitor's two plates always have net charges of <strong className="text-text font-medium">+Q</strong> and <strong className="text-text font-medium">−Q</strong>, summing
            to zero. What's stored is the <em className="italic text-text">separation</em>: a configuration in which positive charge sits on one
        ...
        <FAQItem q="Why does adding the N-th charge cost more work than the first?">
          <p>
            Because by the time you add the N-th charge there are already <strong className="text-text font-medium">N−1</strong> like charges on the same
            ...
            The work cost is <strong className="text-text font-medium">q·V</strong>, and <strong className="text-text font-medium">V</strong> grows linearly with how many charges are
        ...
        <FAQItem q="Why is the stored energy ½CV², not CV²?">
          <p>
            ...
            <em className="italic text-text"> final</em> voltage V, the total work would be QV. Because the first charges paid almost nothing and the
        ...
        <FAQItem q="Why does series-connecting capacitors decrease the total capacitance?">
          <p>
            ...
            <strong className="text-text font-medium"> 1/C_total = 1/C₁ + 1/C₂ + …</strong>. Geometrically: stacking two identical caps in series is like
        ...
        <FAQItem q="Can you charge a capacitor with zero resistance in the loop?">
          <p>
            ...
            charging transient, is exactly <em className="italic text-text">½ CV²</em> regardless of the value of R<Cite id="horowitz-hill-2015" in={SOURCES} />.
        ...
        <FAQItem q="Why is τ = RC and not R/C or R+C?">
          <p>
            ...
            is coulombs per volt. Multiply them: <strong className="text-text font-medium">R·C</strong> has units of (V/(C/s))·(C/V) = <strong className="text-text font-medium">seconds</strong>. Neither R/C nor
            R+C has units of time. More mechanically, the ODE
            <strong className="text-text font-medium"> dQ/dt = (V₀ − Q/C)/R</strong> rearranges to <strong className="text-text font-medium">R·C · dQ/dt = (CV₀ − Q)</strong>, in which the
        ...
        <FAQItem q="Why does a capacitor block DC but pass AC?">
          <p>
            ...
            <strong className="text-text font-medium"> Z = 1/(jωC)</strong>: infinite at DC (ω = 0), small at high frequency<Cite id="horowitz-hill-2015" in={SOURCES} />.
        ...
        <FAQItem q="What's a supercapacitor and how is it different from a regular capacitor?">
          <p>
            ...
            surface area of activated carbon, hundreds of m² per gram. The geometry rule <strong className="text-text font-medium">C = ε₀ εᵣ A / d</strong> then
