  ...
  return (
    <ChapterShell chapter={chapter}>
      ...
      <p>
        ...
        <M tex="d\mathbf{A}" /> is the outward-normal area element of any surface bounded by the
        loop. For a flat loop in a uniform field, this is just <M tex="BA\cos\theta" />, where{' '}
        <M tex="\theta" /> is the angle between the field and the loop's normal. SI unit is the{' '}
      ...
      <p>
        ...
        <M tex="\Phi_B" />
        is the magnetic flux through any surface bounded by the loop (in webers), <M tex="t" /> is
        time (in seconds), and the minus sign encodes Lenz's law — the induced EMF drives a current
      ...
      <p>
        ...
        <M tex="\nabla \cdot B = 0" /> guarantees that flux is conserved through closed surfaces and
        therefore depends only on the boundary
        <Cite id="griffiths-2017" in={SOURCES} />. Second, if the loop is wound <M tex="N" /> times
        around the same flux path, every turn sees the same dΦ/dt and the EMFs add: the total is{' '}
      ...
      <p>
        ...
        E exists everywhere in space, not just where the loop happens to be. The wire is a probe;
        the field is what's actually there
      ...
      <TryIt
        ...
        question={
          <>
            ...
            <M tex="\Delta\Phi = 10^{-4}\,\text{Wb}" /> over <M tex="\Delta t = 1\,\text{ms}" />.
      ...
      <p>
        ...
        the magnet, the brighter the lamp burns. The induced EMF scales with <M tex="|d\Phi/dt|" />,
      ...
      <p>
        ...
        flows. The next section is what that minus sign actually means.
      </p>
      ...
      <p>
        ...
        work. The flux through a flat coil of area <M tex="A" /> rotating at angular rate{' '}
      ...
      <p>
        ...
        an EMF in volts. A pure sine wave whose peak is <M tex="NBA\omega" />. Crank up{' '}
        <M tex="N" />, <M tex="B" />, <M tex="A" />, or <M tex="\omega" /> and the amplitude grows
        in proportion. The frequency you get is <M tex="f = \omega/(2\pi)" />. North American
        wall-outlet power is 60 Hz, which corresponds to a generator shaft turning at{' '}
        <M tex="\omega = 2\pi \cdot 60 \approx 377\,\text{rad/s}" /> — about 3,600 revolutions per
        minute. European 50 Hz is the same equation with a different pulley
        <Cite id="griffiths-2017" in={SOURCES} />.
      </p>
      ...
      <p>
        ...
        flux through a flat loop is not <M tex="B \cdot A" /> — it is{' '}
        <M tex="B \cdot A \cdot \cos\theta" />, where θ is the angle between the loop's normal and
        the field. Spinning the loop sweeps that angle linearly in time, so the flux traces a
      ...
      <p>
        ...
        <M tex="\text{EMF} = NBA\omega\sin(\omega t)" />.
      </p>
      ...
      <p>
        ...
        . Wind two coils around a shared iron core and the same flux <M tex="\Phi(t)" /> threads
      ...
      <p>
        ...
        through one turn (in Wb/s = V). Divide and the dΦ/dt cancels exactly, leaving the
      ...
      <p>
        ...
        relation on the currents — in the ideal lossless limit, <M tex="I_2/I_1 = N_1/N_2" />, so
        power <M tex="VI" /> is preserved across the transformer. You trade volts for amps and back,
        at a cost (in a real transformer) of a few percent in core and copper losses.
      </p>
      ...
      <p>
        ...
        ten times the voltage and a tenth the current, your transmission losses drop by a factor of{' '}
      ...
      <p>
        ...
        <M tex="L" />, defined by <M tex="V = -L\,dI/dt" />. For two separate coils, a changing
        current in one induces a voltage in the other through their shared flux — that is{' '}
      ...
      <p>
        ...
        to Ampère's law — a changing <M tex="E" /> produces a curling <M tex="B" /> in empty space
        too
        ...
        the next chapter — where the field stops being a mathematical bookkeeper for forces and
        starts carrying actual energy across actual empty space.
      </p>

      <CaseStudies
        intro={
          <>
            Three industries built directly on top of <M tex="\text{EMF} = -d\Phi/dt" /> — operating
            at wildly different scales and frequencies, all running the same minus sign.
          </>
        }
      >
        <CaseStudy
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            multi-pole field winding at angular rate <M tex="\omega" /> inside a stator carrying{' '}
            <M tex="N" /> turns of copper, and the stator sees a flux that swings as{' '}
            <M tex="\Phi(t) = NBA\cos(\omega t)" />. Faraday's law immediately gives the peak
            terminal voltage <M tex="NBA\omega" /> at exactly the angular frequency of the rotor
            ...
            frequency, turns its rotor at a rational multiple of the same <M tex="\omega" />.
          </p>
          <p className="mb-prose-2 last:mb-0">
            ...
            architecture of the grid exists because the relation <M tex="V_2/V_1 = N_2/N_1" /> is
            exact in the lossless limit and stays within a few percent in practice.
          </p>
          <p className="mb-prose-2 last:mb-0">
        ...
        >
          ...
          <p className="mb-prose-2 last:mb-0">
            ...
            runs the equation <M tex="\text{EMF} = -N\,d\Phi/dt" /> written for an air-gap coupling,
            at a frequency chosen to make <M tex="d\Phi/dt" /> large enough to deliver a meaningful
            current.
          </p>
        </CaseStudy>
        ...
        >
          <p className="mb-prose-2 last:mb-0">
            ...
            <Cite id="lucia-induction-2014" in={SOURCES} />. The resulting oscillating <M tex="B" />{' '}
            field penetrates the glass (which is essentially invisible to it) and enters the iron
            base of any compatible pan placed on top.
          </p>
          <p className="mb-prose-2 last:mb-0">
      ...
      >
        <FAQItem q="Why does a magnet at rest near a coil produce no current, but one in motion does?">
          <p>
            Faraday's law cares about <M tex="d\Phi/dt" />, not <M tex="\Phi" />. A stationary
            magnet pins down a perfectly real flux through the loop, but the time derivative of a
            constant is zero, so the EMF around the loop is zero
            <Cite id="feynman-II-17" in={SOURCES} />. Move the magnet and the flux through the loop
            changes, which (by <M tex="\nabla \times E = -\partial B/\partial t" />) means a curling
            electric field appears in the wire and pushes the free electrons around
        ...
        <FAQItem q="What is &ldquo;flux,&rdquo; in plain words?">
          ...
          <Formula size="lg" tex="\Phi_B = \iint \vec{B} \cdot d\vec{A}" />
          <p>
            For a flat loop in a uniform field, that's just <M tex="BA\cos\theta" />: field strength
            times area times the cosine of the angle between the field and the loop's normal
        ...
        <FAQItem q="Could you generate AC from a stationary coil if the magnetic field were changing on its own?">
          <p>
            ...
            moves; an alternating current in the primary makes the iron core's <M tex="B" />{' '}
            oscillate, and the oscillating flux through the secondary loop induces an EMF
            ...
            <M tex="d\Phi/dt" /> appears in the equation
        ...
        <FAQItem q="Why is a transformer useless on DC?">
          <p>
            ...
            <M tex="d\Phi/dt = 0" /> in the secondary — no induced EMF
        ...
        <FAQItem q="Does Lenz's law violate Newton's third law? The magnet seems to feel a &ldquo;back force.&rdquo;">
          <p>
            ...
            <M tex="F = qv \times B" />, and the loop's induced current pushes the magnet via the
            equal-and-opposite field it generates. Energy conservation and momentum conservation are
            both intact
            <Cite id="feynman-II-17" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="What's the difference between EMF and voltage?">
          <p>
            ...
            <M tex="\oint E \cdot d\ell = -d\Phi/dt \neq 0" /> — the field isn't conservative, so
            calling it a &ldquo;potential difference&rdquo; is technically wrong, even though a
            voltmeter on the loop reads it just fine
            <Cite id="feynman-II-17" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="Why do generators need to be physically big to make big currents?">
          <p>
            The peak EMF from a rotating coil is <M tex="NBA\omega" />
            <Cite id="griffiths-2017" in={SOURCES} />. Frequency <M tex="\omega" /> is fixed by the
            grid (377 rad/s in North America), and B is capped by the saturation field of iron (~1.5
            T). That leaves only <M tex="N" /> (turns) and <M tex="A" /> (loop area) as engineering
            knobs, and the current you can draw from the machine is bounded by how much copper you
            can pack into those windings without melting it. Doubling power means roughly doubling
            physical size; that's why a 1-GW turbogenerator is the size of a locomotive and not the
            size of a microwave
            <Cite id="feynman-II-17" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="What's a &ldquo;self-induced EMF&rdquo; versus a &ldquo;mutual&rdquo; one?">
          <p>
            ...
            through coil 2, inducing EMF <M tex="V_2 = -M\,dI_1/dt" />. A transformer is engineered
            to maximise <M tex="M" />; a noise-suppression choke is engineered to maximise its own{' '}
            <M tex="L" />
            <Cite id="feynman-II-17" in={SOURCES} />.
          </p>
        </FAQItem>
        ...
        <FAQItem q="Why does opening a switch on an inductor cause a spark?">
          <p>
            An inductor stores energy <M tex="\tfrac{1}{2}LI^{2}" /> in its magnetic field. When you
            open the switch, you're trying to drive <M tex="I" /> to zero in microseconds, which
            means <M tex="dI/dt" /> spikes to a huge negative number. The self-induced EMF{' '}
            <M tex="V = -L\,dI/dt" /> spikes to a correspondingly huge{' '}
        ...
        <FAQItem q="Why does the grid run at 50 or 60 Hz instead of 1 Hz or 1 kHz?">
          <p>
            ...
            <M tex="V/(Nf)" />, so halving <M tex="f" /> means doubling iron cross-section to avoid
            saturation
        ...
        <FAQItem q="How fast does induction &ldquo;communicate&rdquo;? Is it instant?">
          <p>
            ...
            <M tex="\nabla \times E = -\partial B/\partial t" />, combined with Maxwell's added
            displacement current{' '}
            <M tex="\nabla \times B = \mu_0\varepsilon_0\,\partial E/\partial t" />, has wave
            solutions travelling at exactly <M tex="c = 1/\sqrt{\mu_0\varepsilon_0}" />
