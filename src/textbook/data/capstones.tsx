  ...
  steps: [
    {
      ...
      solution: (
        <>
          <p>The numeric box you will keep on the desk for the rest of the design:</p>
          <ul>
            <li><strong>V<sub>in,ac</sub></strong> = 90 V–264 V RMS, 50/60 Hz</li>
            <li><strong>V<sub>in,dc,min</sub></strong> ≈ 90 × √2 − ripple ≈ 110 V (after the bridge + bulk cap, low line)</li>
            <li><strong>V<sub>in,dc,max</sub></strong> ≈ 264 × √2 ≈ 374 V (peak, high line, no load)</li>
            <li><strong>V<sub>out</sub></strong> = 5.0 V ± 5% — the USB-PD 5 V profile<Cite id="usb-pd-r3" in={USBC_SOURCES}/></li>
            <li><strong>I<sub>out,max</sub></strong> = 2.0 A → <strong>P<sub>out</sub></strong> = 10 W</li>
            <li><strong>η</strong> = 0.80 → <strong>P<sub>in</sub></strong> = 10 / 0.80 = <strong>12.5 W</strong> at full load</li>
            <li>Input current at low line: <InlineMath>I<sub>in,rms</sub> ≈ 12.5 W / (90 V × PF)</InlineMath>. With a typical bridge-rectifier power factor of 0.5, that is roughly <strong>0.28 A RMS</strong> — but the peak current into the bulk cap is several amps because conduction happens only near the AC peak<Cite id="erickson-maksimovic-2020" in={USBC_SOURCES}/>.</li>
          </ul>
          <p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>The trade table for a 10 W universal-input brick:</p>
          <ul>
            <li>
    ...
    },
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li>The MOSFET drain stress stays below 700 V (about half the safe margin on a common 800 V part).</li>
            <li>The duty cycle at low line stays below ~50% (above that, current-mode control gets nasty).</li>
          </ul>
          <p>
      ...
      solution: (
        <>
          <p>The reflected voltage relates secondary to primary by the turns ratio:</p>
          ...
          <p>The MOSFET drain at high-line, in the worst-case off-state:</p>
          <Formula>V<sub>ds,peak</sub> = V<sub>in,dc,max</sub> + V<sub>or</sub> + V<sub>spike</sub></Formula>
          <p>where <strong>V<sub>spike</sub></strong> is the ringing produced by leakage inductance discharging into the snubber (typically clamped to ~150 V by an RCD snubber).</p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>Rearrange the energy equation:</p>
          ...
          <p>Round to a stocking value: <strong>L<sub>p</sub> = 270 µH</strong>.</p>
          <p>Duty cycle at low line:</p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>The capacitive part:</p>
          <Formula>C<sub>out</sub> = I<sub>out</sub> / (f<sub>sw</sub> · ΔV<sub>ripple</sub>)</Formula>
          <p>where <strong>C<sub>out</sub></strong> is the reservoir capacitance (F), <strong>I<sub>out</sub></strong> = 2 A is the average output current, <strong>f<sub>sw</sub></strong> = 65 kHz is the switching frequency, and <strong>ΔV<sub>ripple</sub></strong> = 50 mV is the allowed capacitive ripple.</p>
          <Formula>C<sub>out</sub> = 2 / (65 000 × 0.050) ≈ <strong>615 µF</strong></Formula>
          <p>Round up to a standard value: <strong>2 × 470 µF in parallel</strong> (low-ESR aluminum electrolytic, 10 V rating). Parallel-ing halves the ESR and the ripple-current stress per cap.</p>
          <p>The ESR ripple is still ugly — even at ESR ≈ 25 mΩ (two caps in parallel) the spike is about 540 mV. The standard fix is a <strong>post-LC filter</strong>:</p>
          <ul>
            <li>1 µH bead inductor between the bulk caps and the connector.</li>
            <li>22 µF X5R ceramic on the connector side.</li>
          </ul>
          <p>The 1 µH × 22 µF LC has a corner frequency around 34 kHz, attenuating the 65 kHz switching ripple by another ~12 dB and reducing the output noise to under 100 mV<sub>pp</sub> — well within USB-PD spec<Cite id="usb-pd-r3" in={USBC_SOURCES}/>.</p>
        </>
      ),
    },
    {
      ...
      problem: (
        <>
          ...
          <p>Assume:</p>
          <ul>
            <li>MOSFET R<sub>ds(on)</sub> = 2 Ω cold, 3 Ω hot; turn-on/turn-off energy 4 µJ per cycle (datasheet at V<sub>ds</sub> = 400 V, I = 1 A).</li>
            <li>Schottky V<sub>F</sub> = 0.4 V at I<sub>out</sub> = 2 A.</li>
            <li>Transformer copper loss 0.5 W, core loss 0.3 W at this drive level.</li>
            <li>Snubber dissipates 5% of input power.</li>
            <li>Standby controller draws 50 mW; sensing/feedback another 50 mW.</li>
          </ul>
        </>
      ),
      ...
      solution: (
        <>
          <p>For a DCM flyback, the primary RMS current is:</p>
          <Formula>I<sub>p,rms</sub> = I<sub>pk</sub> · √(D / 3)</Formula>
          <Formula>I<sub>p,rms</sub> = 1.2 · √(0.19/3) ≈ <strong>0.30 A</strong></Formula>
          <p>The buckets:</p>
          <ul>
            <li><strong>MOSFET conduction</strong>: 0.30² × 3 Ω = <strong>0.27 W</strong></li>
            <li><strong>MOSFET switching</strong>: 4 µJ × 65 kHz = <strong>0.26 W</strong></li>
            <li><strong>Secondary diode</strong>: 0.4 V × 2 A = <strong>0.80 W</strong></li>
            <li><strong>Snubber</strong>: 5% × 12.5 W = <strong>0.63 W</strong></li>
            <li><strong>Transformer copper + core</strong>: 0.5 + 0.3 = <strong>0.80 W</strong></li>
            <li><strong>Bulk cap + bridge</strong> (estimated): <strong>0.25 W</strong></li>
            <li><strong>Controller + sensing</strong>: <strong>0.10 W</strong></li>
            <li><strong>Total losses</strong>: ≈ <strong>3.1 W</strong></li>
          </ul>
          <Formula>η<sub>actual</sub> = P<sub>out</sub> / (P<sub>out</sub> + P<sub>loss</sub>) = 10 / 13.1 ≈ <strong>76.3%</strong></Formula>
          <p>Slightly below the 80% target. Two cheap wins recover that headroom:</p>
          <ul>
            <li>Replace the Schottky with a <strong>synchronous-rectifier MOSFET</strong> (R<sub>ds(on)</sub> ≈ 8 mΩ at this current). Loss drops from 0.80 W to about 0.13 W — recover 0.7 W, which is the entire efficiency miss<Cite id="erickson-maksimovic-2020" in={USBC_SOURCES}/>.</li>
            <li>Use a more aggressive RCD snubber tuned to the leakage inductance, dropping that 5% to about 3%.</li>
          </ul>
          <p>With both, η rises to about 82%, comfortably meeting Energy Star "Level VI" external power supply standards<Cite id="usb-pd-r3" in={USBC_SOURCES}/>.</p>
        </>
      ),
    },
    {
      ...
      solution: (
        <>
          <p>Option A — <strong>opto-isolated feedback</strong> (the standard for tight regulation):</p>
          <ul>
            <li>A <strong>TL431 shunt regulator</strong> on the secondary acts as a precision op-amp. Its 2.5 V reference compares against a resistor divider from V<sub>out</sub>.</li>
            <li>The TL431 modulates current through an <strong>optocoupler LED</strong>; the photo-transistor on the primary side pulls the controller's FB pin.</li>
            <li>This is the exact same negative-feedback architecture as the Ch.16 inverting op-amp gain stage: a high-gain error amplifier compares output to reference and drives the actuator (here, the duty cycle) until the error is nulled<Cite id="horowitz-hill-2015" in={USBC_SOURCES}/>.</li>
          </ul>
          <p>Option B — <strong>primary-side regulation (PSR)</strong> (cheaper, looser regulation, used in 5 V phone chargers):</p>
          <ul>
            <li>An <strong>auxiliary winding</strong> on the transformer (typically 8–10 turns) sits in the same flux as the secondary. During the off-time, it sees the secondary voltage scaled by N<sub>aux</sub>/N<sub>sec</sub>.</li>
            <li>The controller samples the auxiliary winding voltage at the moment the secondary current reaches zero (the "knee" of the flyback waveform) — that snapshot equals V<sub>out</sub> + V<sub>F</sub> reflected onto the aux winding.</li>
            <li>This is precisely the Ch.7 / Ch.22 picture: a third coil sharing the same flux gets the same dΦ/dt-driven voltage, and the controller treats that as a noiseless image of the regulated output.</li>
          </ul>
          <p>
  ...
  steps: [
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li>Energy used: <strong>820 kWh</strong></li>
            <li>Energy rate: <strong>$0.16 / kWh</strong></li>
            <li>Delivery / distribution charge: <strong>$0.08 / kWh</strong></li>
            <li>Demand peak: <strong>5.2 kW</strong> (15-minute window)</li>
            <li>Time-of-use split: 60% off-peak, 40% peak</li>
          </ul>
          <p>
      ...
      solution: (
        <>
          ...
          <p>(b) Average power over a 30-day month (730 hours):</p>
          <Formula>P<sub>avg</sub> = 820 kWh / 730 h ≈ <strong>1.12 kW</strong></Formula>
          <p>(c) Peak-to-average ratio:</p>
    ...
    },
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li>HVAC (heat pump): runs 4 hours/day at 2.5 kW</li>
            <li>Water heater (resistance, electric): 3 kWh/day</li>
            <li>Refrigerator + freezer: 1.5 kWh/day</li>
            <li>Cooking (electric range + microwave + small): 2 kWh/day</li>
            <li>Lighting (LED): 0.5 kWh/day</li>
            <li>Plug loads (TV, laptops, charger bricks): 3 kWh/day</li>
            <li>EV charging (level 2, every other day, 25 kWh per session): ?</li>
            <li>Phantom loads (always-on): 0.8 kWh/day</li>
          </ul>
          <p>
      ...
      solution: (
        <>
          <p>Monthly totals:</p>
          <ul>
            <li>HVAC: 2.5 kW × 4 h × 30 = <strong>300 kWh</strong></li>
            <li>Water heater: 3 × 30 = <strong>90 kWh</strong></li>
            <li>Refrigerator + freezer: 1.5 × 30 = <strong>45 kWh</strong></li>
            <li>Cooking: 2 × 30 = <strong>60 kWh</strong></li>
            <li>Lighting: 0.5 × 30 = <strong>15 kWh</strong></li>
            <li>Plug loads: 3 × 30 = <strong>90 kWh</strong></li>
            <li>EV (15 sessions × 25 kWh): <strong>375 kWh</strong></li>
            <li>Phantom: 0.8 × 30 = <strong>24 kWh</strong></li>
            <li><strong>Total: 999 kWh</strong></li>
          </ul>
          <p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>1/0 AWG aluminum: cross-section 53.5 mm², ρ ≈ 2.65×10⁻⁸ Ω·m. Per Ch.4:</p>
          <Formula>R = ρL/A = 2.65×10⁻⁸ × 30 / 53.5×10⁻⁶ ≈ <strong>14.9 mΩ</strong> (one way)</Formula>
          <p>The current loop uses both conductors (hot and neutral on a 120 V loop, or two hots on a 240 V loop), so the round-trip resistance is doubled:</p>
          <Formula>R<sub>loop</sub> ≈ 29.7 mΩ</Formula>
          <p>Voltage drop at average load (I = 4.7 A on a 240 V circuit):</p>
          ...
          <p>About <Num value={0.48/820*100}/>% of your monthly draw lost just in the service drop. At the peak load of 5.2 kW, the instantaneous loss climbs to (21.7)² × 0.0297 ≈ 14 W, still less than 0.3% of the delivered power<Cite id="nec-2023" in={TRACE_SOURCES}/>.</p>
        </>
      ),
    },
    {
      ...
      solution: (
        <>
          ...
          <p>(b) Primary current at 1.12 kW (unity power factor assumed for simplicity):</p>
          <Formula>I<sub>1</sub> = P / V<sub>1</sub> = 1120 / 7200 ≈ <strong>0.156 A</strong></Formula>
          <p>(c) Copper loss scales as load², where the load fraction is S/S<sub>rated</sub>:</p>
          <Formula>S/S<sub>rated</sub> = 1.12 / 25 = 0.045</Formula>
          <Formula>P<sub>cu,avg</sub> = 350 × 0.045² ≈ <strong>0.7 W</strong></Formula>
          <p>Core loss is roughly load-independent at 80 W. So at your load:</p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>The substation feeds many neighbourhoods, so 224 kW is a small fraction of its rating. The neighbourhood's share of the loss:</p>
          <Formula>S/S<sub>rated</sub> = 224 / 10 000 = 0.0224</Formula>
          <p>Copper loss attributable to the neighbourhood (scales as load²):</p>
          <Formula>P<sub>cu</sub> = 35 000 × 0.0224² ≈ <strong>17.6 W</strong></Formula>
          <p>Core loss <em>per neighbourhood</em> (shared by all loads on the substation, roughly 50 neighbourhoods at full diversified load):</p>
          <Formula>P<sub>core,share</sub> = 8000 / 50 ≈ <strong>160 W</strong></Formula>
          <p>Combined substation loss attributable to this 224 kW load: ≈ <strong>178 W</strong>, or 0.08%. Cumulative loss so far from house to substation:</p>
          <ul>
            <li>Service drop: 0.66 W per house × 200 = 132 W</li>
            <li>Distribution transformer (80 W × 50 pole pigs to serve 200 homes): 4 000 W</li>
            <li>Primary feeder: 321 W</li>
            <li>Substation transformer (this load's share): 178 W</li>
            <li><strong>Subtotal: ~4.6 kW lost for 224 kW delivered</strong></li>
          </ul>
          <p>
    ...
    },
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li><strong>69 kV sub-transmission</strong> from regional switching yard to substation (~50 km)</li>
            <li><strong>230 kV bulk transmission</strong> from generating area to regional switching yard (~200 km)</li>
            <li>
      ...
      solution: (
        <>
          <p>69 kV sub-transmission, 50 km, your neighbourhood's share of 224 kW:</p>
          ...
          <p>230 kV bulk transmission, 200 km:</p>
          ...
          <p>Cumulative house-to-generator loss summary, for the neighbourhood's 224 kW:</p>
          <ul>
            <li>Aggregated service drops: 132 W</li>
            <li>Pole transformers: 4 000 W</li>
            <li>12.47 kV feeder: 321 W</li>
            <li>Substation transformer: 178 W</li>
            <li>69 kV sub-transmission: 105 W</li>
            <li>230 kV transmission: 38 W</li>
            <li>Generator step-up transformer (similar size to substation transformer, ~150 W): 150 W</li>
            <li><strong>Total: ~4.9 kW</strong> out of 224 kW gross at the generator → <strong>~2.2% delivery loss</strong></li>
          </ul>
          <p>
    ...
    },
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li>The stator line current at full output.</li>
            <li>The mechanical torque the turbine must supply (assume 98% generator efficiency).</li>
            <li>Where in the generator the I²R losses go and how they are removed.</li>
          </ul>
        </>
      ),
      ...
      solution: (
        <>
          <p>Stator line current:</p>
          <Formula>I<sub>stator</sub> = 500 000 000 / (1.732 × 22 000 × 0.95) ≈ <strong>13 800 A</strong></Formula>
          <p>That's 13.8 kA flowing through each of three stator windings — through hollow water-cooled copper bars typically 100 mm × 50 mm in cross-section<Cite id="fitzgerald-kingsley-umans-2014" in={TRACE_SOURCES}/>.</p>
          <p>Mechanical input power at 98% efficiency:</p>
          <Formula>P<sub>mech</sub> = 500 / 0.98 ≈ <strong>510 MW</strong></Formula>
          <p>Mechanical angular velocity:</p>
          ...
          <ul>
            <li><strong>Stator copper I²R</strong> (the biggest): ~4 MW</li>
            <li><strong>Rotor field copper I²R</strong>: ~1 MW</li>
            <li><strong>Stator iron core (eddy + hysteresis)</strong>: ~3 MW</li>
            <li><strong>Windage and friction</strong>: ~1 MW</li>
            <li><strong>Stray load loss</strong>: ~1 MW</li>
          </ul>
          <p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>Cumulative efficiency from the chain above (each loss as fraction of generator output):</p>
          <ul>
            <li>Generator internal (98%): 2% loss</li>
            <li>Step-up transformer + transmission + sub-transmission + substation: ~3% total</li>
            <li>Distribution (feeder + pole transformer + service drop): ~2.5%</li>
            <li><strong>End-to-end efficiency ≈ 92.5%</strong></li>
          </ul>
          <p>So 1 kWh at the panel required:</p>
          <Formula>E<sub>shaft</sub> = 1 / 0.925 ≈ <strong>1.08 kWh</strong> at the turbine shaft</Formula>
          <p>And in terms of fuel energy (assuming a 33% efficient coal plant or 60% efficient combined-cycle gas):</p>
  ...
  stretch: {
    ...
    problem: (
      <>
        ...
        <ol>
          <li>A subcritical coal plant (~33% thermal efficiency, ~95 g CO₂ per MJ of coal heat)</li>
          <li>A combined-cycle gas turbine (~60% thermal efficiency, ~56 g CO₂ per MJ of gas heat)</li>
          <li>A hydroelectric plant (~98% generation efficiency, 0 g CO₂ direct)</li>
        </ol>
        <p>
    ...
    solution: (
      <>
        ...
        <ul>
          <li>From coal: need to cut 919 → ~0 → almost <strong>100% kWh reduction</strong>. Not possible by efficiency; you have to change the generation source.</li>
          <li>From CCGT: need 298 → ~0 → again ~100%. Same conclusion.</li>
        </ul>
        <p>
  ...
  steps: [
    {
      ...
      solution: (
        <>
          ...
          <p>Three spectral lines:</p>
          <ul>
            <li><strong>Carrier</strong> at 1.000 000 MHz, amplitude 1 mV</li>
            <li><strong>Lower sideband</strong> at 0.999 000 MHz, amplitude 0.25 mV</li>
            <li><strong>Upper sideband</strong> at 1.001 000 MHz, amplitude 0.25 mV</li>
          </ul>
          <p>
    ...
    },
    {
      ...
      solution: (
        <>
          <p>For f<sub>0</sub> = 1.00 MHz, C = 365 pF (capacitor at maximum mesh; we tune by reducing C):</p>
          ...
          <p>For Q — to reject the adjacent station at 1.010 MHz (10 kHz away), we want the tank's −3 dB bandwidth to be of order 10 kHz:</p>
          <Formula>Q = f<sub>0</sub> / Δf = 1 000 000 / 10 000 = <strong>100</strong></Formula>
          <p>The equivalent parallel resistance (also called the dynamic resistance R<sub>D</sub>) is:</p>
    ...
    },
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li>The RC time constant is long compared to 1/f<sub>c</sub> (so the cap holds between carrier peaks).</li>
            <li>The RC time constant is short compared to 1/f<sub>audio,max</sub> = 1/5 kHz = 200 µs (so the cap can follow the audio envelope down).</li>
          </ul>
          <p>
      ...
      solution: (
        <>
          <p>Lower bound on τ<sub>RC</sub> (must be ≫ 1 µs, say ≥ 10 µs):</p>
          <Formula>C ≥ 10 µs / 10 kΩ = <strong>1.0 nF</strong></Formula>
          <p>Upper bound (must be ≪ 200 µs, say ≤ 50 µs):</p>
          <Formula>C ≤ 50 µs / 10 kΩ = <strong>5.0 nF</strong></Formula>
          <p>Pick C = <strong>3.3 nF</strong> as a stocking value in the middle:</p>
          <Formula>τ<sub>RC</sub> = R · C = 10 000 · 3.3×10⁻⁹ = <strong>33 µs</strong></Formula>
          <p>That's 33× longer than the carrier period (1 µs, fine — the cap holds) and 6× shorter than the highest audio period (200 µs, fine — the cap discharges fast enough that the envelope doesn't get clipped on a fast downward swing).</p>
          <p>
    ...
    },
    {
      ...
      problem: (
        <>
          ...
          <ul>
            <li>Voltage gain ≈ 40 (32 dB).</li>
            <li>AC coupling on the input to block the DC pedestal.</li>
            <li>Audio bandwidth from 100 Hz to 5 kHz.</li>
          </ul>
          <p>
      ...
      solution: (
        <>
          <p>Gain = 40 → R<sub>f</sub>/R<sub>g</sub> = 39. Pick standard values:</p>
          <Formula>R<sub>f</sub> = 39 kΩ, R<sub>g</sub> = 1 kΩ → Gain = 1 + 39 = <strong>40</strong></Formula>
          <p>Input coupling cap to set the 100 Hz lower corner with the op-amp's input bias resistor R<sub>in</sub> = 100 kΩ (to ground; the standard way to bias a non-inverting op-amp's positive input):</p>
          <Formula>C<sub>in</sub> = 1 / (2π · f · R<sub>in</sub>) = 1 / (2π · 100 · 100 000) ≈ <strong>16 nF</strong></Formula>
          <p>Round to <strong>22 nF</strong> for ~70 Hz corner, well below the speech range.</p>
          <p>Output low-pass to roll off above 5 kHz, using a series R<sub>o</sub> = 1 kΩ and shunt C<sub>o</sub>:</p>
          <Formula>C<sub>o</sub> = 1 / (2π · 5000 · 1000) ≈ <strong>32 nF</strong> → round to 33 nF</Formula>
          <p>Check the gain stays flat across the audio band. The closed-loop bandwidth of the gain-of-40 stage with a 10 MHz GBW op-amp:</p>
          <Formula>BW = GBW / Gain = 10⁷ / 40 = <strong>250 kHz</strong></Formula>
          <p>Way beyond the audio band; the output LP filter, not the op-amp, sets the upper corner. The whole stage looks like:</p>
          <ul>
            <li><strong>Input</strong>: 22 nF AC coupling → 100 kΩ to ground → op-amp + input.</li>
            <li><strong>Feedback</strong>: 1 kΩ from − input to ground; 39 kΩ from − input to output.</li>
            <li><strong>Output</strong>: 1 kΩ in series, 33 nF to ground.</li>
          </ul>
          <p>
    ...
    },
    {
      ...
      solution: (
        <>
          ...
          <ul>
            <li>From the op-amp's output, drive a small <strong>bias network</strong>: a 470 Ω resistor from +V to a node A, then two 1N4148 diodes from A to a node B, then a 470 Ω from B to −V. The op-amp's signal is injected into the centre via two small (220 Ω) resistors from output to A and B respectively.</li>
            <li>Node A → base of NPN; Node B → base of PNP.</li>
            <li>NPN emitter and PNP emitter join → 220 µF series cap → speaker → ground.</li>
            <li>Most importantly: the speaker side of the cap is tied back to the op-amp's <strong>− input</strong> through the feedback network. The op-amp closes the loop around the whole push-pull. Output offset, crossover non-linearity, and current-gain mismatch all get suppressed by the loop gain<Cite id="sedra-smith-2014" in={RADIO_SOURCES}/>.</li>
          </ul>
          <p>Current capability of the BD139/BD140 in this topology:</p>
          <Formula>I<sub>load,peak</sub> = (V<sub>cc</sub> − V<sub>CE,sat</sub> − I·R<sub>E</sub>) / R<sub>speaker</sub></Formula>
          <p>With V<sub>cc</sub> = 9 V (typical battery-powered AM radio), V<sub>CE,sat</sub> ≈ 0.5 V, and a small 0.22 Ω emitter degeneration resistor:</p>
          <Formula>I<sub>load,peak</sub> ≈ (9 − 0.5)/(4 + 0.22) ≈ <strong>2 A</strong></Formula>
          <p>(More than enough for a 0.5 W output at 4 Ω.)</p>
          <p>Power dissipation in each transistor at maximum sine-wave drive:</p>
          <Formula>P<sub>diss</sub> = V<sub>cc</sub>² / (π² · R<sub>load</sub>) − P<sub>out</sub>/2</Formula>
          <p>For V<sub>cc</sub> = 9 V into 4 Ω at full sine drive: P<sub>diss</sub> ≈ 9²/(π²·4) − 0.5 ≈ 1.55 W per transistor — well within the BD139's 12.5 W rating with a small clip-on heat sink<Cite id="horowitz-hill-2015" in={RADIO_SOURCES}/>.</p>
        </>
      ),
    },
    {
      ...
      solution: (
        <>
          <p>Block-by-block, with impedance and amplitude:</p>
          <ol>
            <li><strong>Ferrite-rod antenna + tank</strong>: induced EMF 1 mV at 1 MHz, source impedance ~43 kΩ at resonance (the tank's R<sub>D</sub>).</li>
            <li><strong>JFET buffer (gain ≈ 1)</strong>: matches the tank's high impedance to the next stage without loading it.</li>
            <li><strong>RF amplifier (BJT, gain ≈ 180)</strong>: takes 1 mV up to ~180 mV at 1 MHz.</li>
            <li><strong>Envelope detector (Schottky + 10 kΩ‖3.3 nF)</strong>: 180 mV RF → ~80 mV audio (lossy: ~7 dB attenuation in the diode).</li>
            <li><strong>Op-amp audio amp (gain 40)</strong>: 80 mV → 3.2 V at the op-amp output.</li>
            <li><strong>Class-AB push-pull buffer (gain ≈ 1)</strong>: drives the speaker at ~1 V<sub>rms</sub> with ~250 mA — about 0.25 W into 4 Ω.</li>
          </ol>
          <p>Power gain:</p>
  ...
  stretch: {
    ...
    problem: (
      <>
        ...
        <ol>
          <li>Pick the IF (standard: 455 kHz for AM broadcast).</li>
          <li>Compute the local-oscillator (LO) frequency needed to receive 1 MHz.</li>
          <li>Identify the <strong>image frequency</strong> — a second band that the mixer also folds onto the IF — and decide how to reject it.</li>
          <li>Specify the IF strip: a ceramic 455 kHz filter with 6 kHz bandwidth, followed by two cascaded gain stages.</li>
        </ol>
      </>
    ),
    solution: (
      <>
        ...
        <ul>
          <li><strong>Constant-frequency amplification</strong>: all the gain happens at one fixed frequency where the bias networks and stability margins can be optimised once.</li>
          <li><strong>Sharp adjacent-channel rejection</strong>: a 6 kHz ceramic filter is impossible to build directly at 1 MHz with a single LC tank but trivial at 455 kHz with off-the-shelf parts.</li>
          <li><strong>Single tuning knob</strong>: a ganged variable capacitor tunes the preselector and LO together, keeping their separation at exactly the IF — the entire AM band is covered without ever changing the IF strip's tuning<Cite id="horowitz-hill-2015" in={RADIO_SOURCES}/>.</li>
        </ul>
        <p>
