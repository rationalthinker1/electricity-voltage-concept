...
export function BackEMFInRunningMotorDemo({ figure }: Props) {
  const [tauLoad, setTauLoad] = useState(0.10);   // N·m
  const [restartTick, setRestartTick] = useState(0);

  const stateRef = useRef({ tauLoad });
  useEffect(() => { stateRef.current.tauLoad = tauLoad; }, [tauLoad]);
  ...
  const computed = useMemo(() => {
    ...
    return { omegaSS: omegaClamped, backEMFss, Iss, Istart, rpm: (omegaClamped * 60) / (2 * Math.PI) };
  }, [tauLoad]);
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      ctx.moveTo(padL, yE(V_SUPPLY)); ctx.lineTo(padL + plotW, yE(V_SUPPLY));
      ctx.stroke();
      ctx.setLineDash([]);
  ...
  return (
    <Demo
      ...
      caption={<>
        Brushed DC motor with V = 12 V, R = 1 Ω, k_e = 0.05 V·s/rad. At <em>t</em> = 0 the rotor is stationary and
        the full 12 V drops across the winding, so I = V/R = 12 A — the inrush. As the rotor accelerates, back-EMF
        rises (E = k_e ω); the net voltage across R falls; current collapses to its small running value, just enough
        to balance load torque. Increase the load and steady-state current rises again.
      </>}
    >
      ...
      <DemoControls>
        <MiniSlider
          ...
          value={tauLoad} min={0} max={0.4} step={0.01}
          format={v => v.toFixed(2) + ' N·m'}
          onChange={setTauLoad}
        />
        <MiniReadout label="I startup (ω=0)" value={<Num value={computed.Istart} digits={1} />} unit="A" />
        <MiniReadout label="I running (ss)" value={<Num value={computed.Iss} digits={2} />} unit="A" />
        <MiniReadout label="back-EMF (ss)" value={<Num value={computed.backEMFss} digits={2} />} unit="V" />
        <MiniReadout label="speed (ss)" value={<Num value={computed.rpm} digits={0} />} unit="rpm" />
        <button
          type="button"
