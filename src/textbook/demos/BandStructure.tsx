...
const MATERIALS: Record<MatKind, { label: string; Eg: number; color: string }> = {
  ...
  diamond: { label: 'Diamond (C)',          Eg: 5.50, color: 'rgba(91,174,248,0.95)' },
};
...
export function BandStructureDemo({ figure }: Props) {
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.moveTo(arrowX, yOf(vbTop)); ctx.lineTo(arrowX, yOf(cbBot));
      ctx.stroke();
      // arrowheads
      ctx.beginPath();
      ...
      ctx.fillText(`${MATERIALS[mat].label}   T = ${T.toFixed(0)} K   kT = ${(PHYS.k_B * T / EV * 1000).toFixed(1)} meV   exp(−E_g/2kT) ≈ ${occ.toExponential(2)}`, padL, 6);
  ...
  return (
    <Demo
      ...
      caption={<>
        At T = 0 the valence band is full and the conduction band is empty — no conduction. At finite T,
        thermally-excited electrons populate the CB with a Boltzmann factor exp(−E_g/2kT). Si (1.12 eV) and Ge (0.67 eV)
        give measurable conduction at room temperature; diamond (5.50 eV) is a near-perfect insulator;
        GaAs (1.42 eV) is the standard direct-gap material for LEDs and lasers.
      </>}
    >
