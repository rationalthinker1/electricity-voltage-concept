...
export function BoostConverterDemo({ figure }: Props) {
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      for (let k = 0; k < Ncyc; k++) {
        ...
        if (first) { ctx.moveTo(x0, yLo); first = false; }
  ...
  return (
    <Demo
      ...
      caption={<>
        Inductor in series with the input, switch to ground, diode to the output. While the switch is on,
        the inductor stores energy from V<sub>in</sub>; while it is off, that stored energy is forced
        through the diode into the output capacitor, which must therefore sit <em>above</em> V<sub>in</sub>.
        Volt-second balance on L gives <strong>V<sub>out</sub> = V<sub>in</sub> / (1 − D)</strong>. The input
        current is the inductor average and is always larger than the output current — by the same factor
        the voltage steps up. Real converters cap at D ≈ 0.85 because resistive losses in the switch and
        inductor blow up the implicit equation past that.
      </>}
    >
      ...
      <DemoControls>
        <MiniSlider
          label="V_in"
          value={Vin} min={2} max={24} step={0.5}
          format={v => v.toFixed(1) + ' V'}
          onChange={setVin}
        />
        <MiniSlider
          label="duty D"
          value={duty} min={0.05} max={0.90} step={0.01}
          format={v => (v * 100).toFixed(0) + ' %'}
          onChange={setDuty}
        />
        <MiniSlider
          label="R_load"
          value={Rload} min={2} max={200} step={1}
        ...
        <MiniReadout label="ripple V"  value={<Num value={computed.Vripple} digits={2} />} unit="V" />
