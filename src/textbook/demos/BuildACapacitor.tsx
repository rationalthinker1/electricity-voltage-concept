...
export function BuildACapacitorDemo({ figure }: Props) {
  ...
  const historyRef = useRef<Array<{ n: number; V: number; U: number }>>([{ n: 0, V: 0, U: 0 }]);
  ...
  const stateRef = useSimState({ Q, A_m2, d_m, C, V, U, workForNext });
  ...
  const setup = useSimLoop(
    ...
    ({ ctx, w: W, h: H, colors }, _state, _dt, _simTime, ctx0) => {
      ...
      if (Math.abs(s.Q) > 0 && usable > 8) {
        const Nfield = Math.max(
          3,
          Math.min(22, Math.round(Math.log10(Math.abs(s.Q) / Q_PER_CLICK + 1) * 9)),
        );
  ...
  return (
    <Demo
      ...
      caption={
        <>
          Each click moves <strong>1 nC</strong> from the bottom plate to the top (the bottom is
          left with an equal-and-opposite induced charge). Watch the voltage rise <em>linearly</em>{' '}
          with the number of charges and the stored energy rise <em>quadratically</em>. The bar on
          the right shows the work the next click costs — it grows because each new charge has to
          push against the field of all the charges already there.
        </>
      }
