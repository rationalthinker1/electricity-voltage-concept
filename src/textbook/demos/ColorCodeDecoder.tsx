...
const COLOURS: Colour[] = [
  ...
  { key: 'silver', label: 'Silver', swatch: '#c0c0c0', digit: null, mult: -2, tol: 0.1 },
];
...
export function ColorCodeDecoderDemo({ figure }: Props) {
  ...
  return (
    <Demo
      ...
      caption={
        <>
          The IEC 60062 colour code: two (or three) digit bands, a multiplier band, and a tolerance
          band. Four bands gives you two significant figures (E12/E24 series, ±2–10%); five gives
          three (E96, ±1% or better). Click colours to set each band; the readout shows the nominal
          value and the tolerance window.
        </>
      }
        ...
        {mode === 4 ? (
          ...
          <>
            ...
            <BandRow
              ...
              value={bMult}
              options={MULT_COLOURS}
              onChange={setBMult}
            />
          </>
        )}
        <BandRow
          label={`Band ${mode === 4 ? 4 : 5} (tolerance)`}
          value={bTol}
          options={TOL_COLOURS}
          onChange={setBTol}
        />
