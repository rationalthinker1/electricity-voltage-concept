...
const FAMILIES: Record<DiodeKind, {
  label: string;
  Is: number;
  n: number;
  ...
  schottky: { label: 'Schottky (1N5817)',     Is: 1e-5,  n: 1.0, Vz: 20,  color: 'rgba(108,197,194,0.95)' },
  zener:    { label: 'Zener (1N4733A, 5.1 V)', Is: 1e-9, n: 1.0, Vz: 5.1, color: 'rgba(91,174,248,0.95)' },
};
...
export function DiodeCharacteristicDemo({ figure }: Props) {
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      // axes ranges
      const Vmin = -7, Vmax = 1.2;
      const Imin = -0.05, Imax = 0.1;        // A (so ±50 mA reverse, +100 mA forward)
      ...
      ctx.moveTo(xOf(0), padT); ctx.lineTo(xOf(0), padT + plotH);
      ctx.stroke();
  ...
  return (
    <Demo
      ...
      caption={<>
        ...
        all three diodes leak only nanoamps — until the Zener hits its avalanche breakdown
        at −5.1 V and clamps. Slide T: V<sub>T</sub> = kT/q is only 25.85 mV at room temperature,
        which is why the diode's forward voltage drifts ~−2 mV/K with heat.
      </>}
    >
      ...
      <DemoControls>
        <MiniSlider
          label="V applied"
          value={V} min={-6} max={1.0} step={0.01}
          format={v => v.toFixed(2) + ' V'}
          onChange={setV}
        />
        <MiniSlider
          label="T"
          value={T} min={200} max={400} step={1}
