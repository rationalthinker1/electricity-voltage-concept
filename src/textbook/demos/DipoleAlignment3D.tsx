...
export function DipoleAlignment3DDemo({ figure }: Props) {
  ...
  return (
    <Demo
      ...
      caption={
        <>
          A translucent slab of dielectric sits in a uniform external <strong>E_ext</strong> along
          +x. The interior is populated by molecular dipoles — short arrows from a blue tail (−) to
          a pink head (+). At <em>E = 0</em> they tumble isotropically and the mean alignment{' '}
          <em>⟨cos θ⟩</em> is zero; turn up the field and they tilt toward +x against thermal noise,
          with the equilibrium alignment given by the Langevin function <em>L(pE/kT)</em>. The
          dipole tails accumulate on the left face and the heads on the right — that's the bound
          surface charge, and it's what reduces the field inside the block by the factor{' '}
          <em>
            ε<sub>r</sub>
          </em>
          .
