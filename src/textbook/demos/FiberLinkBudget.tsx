...
export function FiberLinkBudgetDemo() {
  ...
  useEffect(() => { stateRef.current = { launchDbm, alpha, extraLoss, rxSens }; },
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      ctx.moveTo(padL, yRx); ctx.lineTo(padL + plotW, yRx);
      ctx.stroke();
      ctx.setLineDash([]);
  ...
  return (
    <Demo
      ...
      caption={<>
        The plot is a one-equation argument: <strong>P_rx = P_tx − L_conn − α·L</strong>.
        Drag the sliders to see how launch power, fiber loss, and connector loss trade
        against receiver sensitivity. A typical metro link at 1550 nm puts +0 dBm of
        DFB-laser power into G.652 fiber at 0.22 dB/km with 2 dB of accumulated splice
        loss; a coherent receiver sits at roughly −28 dBm. That budget reaches ~120 km
        of fiber without amplification — the spacing between EDFAs on a long-haul span.
      </>}
    >
      <AutoResizeCanvas height={240} setup={setup} />
      <DemoControls>
        <MiniSlider label="launch P" value={launchDbm} min={-5} max={5} step={0.5}
          format={v => `${v.toFixed(1)} dBm`} onChange={setLaunchDbm} />
        <MiniSlider label="α" value={alpha} min={0.15} max={3} step={0.01}
          format={v => `${v.toFixed(2)} dB/km`} onChange={setAlpha} />
        <MiniSlider label="connector + splice loss" value={extraLoss} min={0} max={10} step={0.5}
          format={v => `${v.toFixed(1)} dB`} onChange={setExtraLoss} />
        <MiniSlider label="Rx sens" value={rxSens} min={-40} max={-15} step={1}
          format={v => `${v.toFixed(0)} dBm`} onChange={setRxSens} />
