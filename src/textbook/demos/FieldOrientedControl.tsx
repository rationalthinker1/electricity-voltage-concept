...
export function FieldOrientedControlDemo({ figure }: Props) {
  ...
  const stateRef = useSimState({ iqRef, idRef });
  ...
  const setup = useSimLoop(
    ...
    ({ ctx, w, h, colors }, _state, _dt, _simTime, ctx0) => {
      ...
      for (let g = 1; g < 4; g++) {
        ...
        ctx.beginPath();
        ctx.moveTo(x0R, yg);
        ctx.lineTo(x0R + colW, yg);
        ctx.stroke();
      }
      ctx.strokeStyle = colors.borderStrong;
      ctx.beginPath();
      ctx.moveTo(x0R, y0 + plotH / 2);
      ctx.lineTo(x0R + colW, y0 + plotH / 2);
      ctx.stroke();
