...
const HALF_CELLS: Array<{ key: string; label: string; E: number }> = [
  ...
  { key: 'H', label: '2H⁺ + 2e⁻ → H₂  (SHE)', E: 0.0 },
  { key: 'Cu', label: 'Cu²⁺ + 2e⁻ → Cu', E: +0.34 },
  { key: 'Ag', label: 'Ag⁺ + e⁻ → Ag', E: +0.8 },
  ...
  return (
    <Demo
      ...
      caption={
        <>
          ...
          <strong>E°_cell = E°_cathode − E°_anode</strong>. A positive E°_cell means the reaction
    ...
    >
      ...
      <EquationStrip
        ...
        left={<M tex="E°_{\text{cell}} = E°_{\text{cathode}} - E°_{\text{anode}}" />}
        rightLabel="For selected half-cells"
        right={
          <M
            tex={`${cathode.E >= 0 ? '+' : ''}${cathode.E.toFixed(2)} - (${anode.E >= 0 ? '+' : ''}${anode.E.toFixed(2)}) = ${Ecell >= 0 ? '+' : ''}${Ecell.toFixed(2)}\\,\\text{V}`}
          />
        }
      />
    </Demo>
