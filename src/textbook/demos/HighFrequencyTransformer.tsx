...
export function HighFrequencyTransformerDemo({ figure }: Props) {
  ...
  const setup = useSimLoop(
    ...
    ({ ctx, w, h, colors }, _state, _dt, _simTime) => {
      ...
      drawLabel(ctx, { text: `${formatHz(f)}`, x: rightCX, y: 12 });
      drawLabel(ctx, { text: `V ≈ ${V_REF_CM3} cm³`, x: leftCX, y: cy + refSide / 2 + 24, font: '10px "JetBrains Mono", monospace' });
      drawLabel(ctx, { text: `mass ≈ ${M_REF_G} g`, x: leftCX, y: cy + refSide / 2 + 38 });
      drawLabel(ctx, { text: `V ≈ ${formatVol(V_REF_CM3 * scale)}`, x: rightCX, y: cy + refSide / 2 + 24, color: colors.text });
      drawLabel(ctx, { text: `mass ≈ ${formatMass(M_REF_G * scale)}`, x: rightCX, y: cy + refSide / 2 + 38 });
      ctx.save();
      ctx.globalAlpha = 0.65;
      drawLabel(ctx, { text: '100 W reference', x: 8, y: h - 16, size: 9, font: '9px "JetBrains Mono", monospace', baseline: 'top' });
      drawLabel(ctx, { text: 'V ∝ 1/f   (constant V, B_max, N)', x: w - 8, y: h - 16, align: 'right' });
      let tag = '';
