...
export function InRushCurrentDemo({ figure }: Props) {
  ...
  return (
    <Demo
      ...
      caption={
        <>
          Because Φ is the time-integral of V<sub>p</sub>, the instant of switch-on sets the DC
          offset of the flux waveform. Closing at the voltage peak (θ = 90°) starts the integral at
          its natural average — Φ swings symmetrically around zero. Closing at a voltage
          zero-crossing (θ = 0°) leaves the integral with a half-cycle of accumulation before the
          next reversal, driving Φ to ~2 × its steady-state peak. That overshoot pushes the core
          well past B<sub>sat</sub>, and the magnetising current jumps by an order of magnitude or
          more. Real distribution transformers see 10–30× nominal inrush this way; it is why
    ...
    >
      ...
      <DemoControls>
        ...
        />
        <MiniReadout
          label="Φ peak / Φ steady"
          value={<Num value={Math.abs(Bpeak)} digits={2} />}
          unit="×"
        />
        <MiniReadout
          label="I_inrush / I_mag,ss"
          value={<Num value={Iratio} digits={1} />}
          unit="×"
        />
