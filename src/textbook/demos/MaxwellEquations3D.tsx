...
function traceLine(
  start: Vec3, fieldFn: (p: Vec3) => Vec3, sign: number, steps: number,
  dt: number, maxRadius: number,
): Vec3[] {
...
function drawArrow3D(
  ctx: CanvasRenderingContext2D, from: Vec3, to: Vec3,
  cam: OrbitCamera, w: number, h: number,
  color: string, lineWidth = 1.6, headSize = 6,
) {
...
function drawGaussE(
  ...
  const grad = ctx.createRadialGradient(
    origin.x - rad * 0.3, origin.y - rad * 0.3, rad * 0.2,
  ...
  const faceCenters: Vec3[] = [
    v3(BOX, 0, 0), v3(-BOX, 0, 0),
    v3(0, BOX, 0), v3(0, -BOX, 0),
    v3(0, 0, BOX), v3(0, 0, -BOX),
  ];
  const normals: Vec3[] = [
    v3(1, 0, 0), v3(-1, 0, 0),
    v3(0, 1, 0), v3(0, -1, 0),
...
function drawGaussB(
  ...
  for (let a = 0; a < N_AZ; a++) {
    ...
    const cphi = Math.cos(phi0), sphi = Math.sin(phi0);
...
function drawAmpere(
  ...
  const Elen = Math.max(0.08, Math.min(0.30, Math.abs(Enow) * 0.18));
  ...
  const ringRadii: Array<{ y: number; r: number; teal: number }> = [
    ...
    { y: GAP_HALF + wireLen * 0.5, r: 0.20, teal: 0.85 },
    { y: -GAP_HALF - wireLen * 0.5, r: 0.20, teal: 0.85 },
  ];
  ...
  for (const { y, r, teal } of ringRadii) {
    ...
    for (let i = 0; i < N_RING; i++) {
      ...
      drawArrow3D(
        ctx, from, to, cam, w, h,
        `rgba(108,197,194,${teal.toFixed(2)})`, 1.8, 7,
      );
    }
  }
...
const MODE_BLURBS: Record<Mode, string> = {
  'gauss-e':  'A point charge inside the box sprays radial E-arrows. Every outward arrow pierces a face; total flux out equals Q/ε₀.',
  'gauss-b':  'A bar magnet inside the box. Every B-line that enters one face exits another — no monopoles, net flux through any closed surface is zero.',
  'faraday':  'A wire loop pierced by a magnetic flux ramping over time. The changing B induces an electric circulation around the loop: ∮ E·dℓ = −dΦ_B/dt.',
  'ampere':   'A charging capacitor. Conduction current flows in the wires; in the gap, no current — only a growing E. Maxwell\'s displacement-current term fills in for the missing conduction current and B curls around the gap exactly as it does around the wire.',
};
...
export function MaxwellEquations3DDemo({ figure }: Props) {
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      switch (s.mode) {
        case 'gauss-e': drawGaussE(ctx, cam, w, h, s.q); break;
        case 'gauss-b': drawGaussB(ctx, cam, w, h, s.mDipole); break;
        case 'faraday': drawFaraday(ctx, cam, w, h, s.dBdt, tNow); break;
        case 'ampere':  drawAmpere(ctx, cam, w, h, s.dEdt, tNow); break;
      }
    ...
    switch (mode) {
      ...
      case 'gauss-b':
        return (
          <MiniSlider
            ...
            value={mDipole} min={0.3} max={3} step={0.05}
      ...
      case 'faraday':
        return (
          <MiniSlider
            label="dB/dt"
            value={dBdt} min={-3} max={3} step={0.05}
      ...
      case 'ampere':
        return (
          <MiniSlider
            label="dE/dt"
            value={dEdt} min={-3} max={3} step={0.05}
  ...
  const eqDisplay = (() => {
    switch (mode) {
      ...
      case 'faraday':
        return <Formula>∮ E · dℓ = − dΦ<sub>B</sub>/dt</Formula>;
      case 'ampere':
        return <Formula>∮ B · dℓ = μ₀ ( I<sub>enc</sub> + ε₀ dΦ<sub>E</sub>/dt )</Formula>;
    }
  })();
    ...
    switch (mode) {
      case 'gauss-e':
        return (
          <>
            <MiniReadout
              label="∮ E·dA"
              value={<Num value={computed.fluxE} />}
              unit="V·m"
            />
            <MiniReadout
              label="Q/ε₀"
      ...
      case 'ampere':
        return (
          <>
            <MiniReadout
              label="I_displacement"
              value={<Num value={computed.Id} />}
              unit="A"
            />
            <MiniReadout
              label="∮ B·dℓ = μ₀ I_d"
  ...
  return (
    <Demo
      ...
      caption={
        <>
          A single 3D scene with four toggleable facets. Mode 1 puts a point charge
          inside the cube and shows radial E-flux through its faces; the integral
          ∮ E·dA equals Q/ε₀ exactly. Mode 2 puts a bar magnet inside the same cube;
          every B-line that enters one face exits another, so ∮ B·dA = 0 by inspection.
          Mode 3 ramps a magnetic flux through a wire loop and shows the induced E
          curling around the loop's edge. Mode 4 charges a capacitor: conduction
          current in the wires, no current in the gap, but a growing E whose
          displacement-current term ε₀ dΦ_E/dt produces exactly the missing B circulation.
          The slider sets the diagnostic quantity in each mode; the live readouts show
          both sides of the equation matching numerically.
        </>
      }
    >
