...
export function MotorEfficiencyMapDemo({ figure }: Props) {
  ...
  const stateRef = useSimState({ motorType, tauOp, omegaOp });
  ...
  return (
    <Demo
      ...
      caption={
        <>
          Efficiency map η(τ, ω) for four motor families. Bright zone = peak efficiency (~0.95 for
          PMSM, ~0.93 for induction, ~0.85 for brushed DC). Light loads and low speeds are penalised
          by speed-dependent iron and friction losses that don't scale with output. EV gearbox
          ratios are chosen to keep the motor inside its bright zone for the typical cruise
          operating point.
        </>
      }
    >
