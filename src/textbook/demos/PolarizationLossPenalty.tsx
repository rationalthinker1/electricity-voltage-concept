...
export function PolarizationLossPenaltyDemo({ figure }: Props) {
  ...
  useEffect(() => { stateRef.current = { alphaDeg }; }, [alphaDeg]);
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw() {
      ...
      ctx.moveTo(wcx + Rmid + 4, cy);     ctx.lineTo(colW * 2.5 - Rt - 6, cy);
      ctx.stroke();
  ...
  return (
    <Demo
      ...
      caption={<>
        A dipole antenna couples only to the E-field component along its axis. With the transmitter
        vertical and the receiver tilted by angle <strong>α</strong>, the received power is
        <strong> cos²(α)</strong> times its co-polar maximum — exactly Malus's law for optics.
        Cross-polarised (α = 90°) is a complete null on paper; in practice 20–40 dB of suppression,
        more than enough to break a link. GPS and SatCom avoid the problem by using circular
        polarisation, which costs only a fixed 3 dB regardless of orientation.
      </>}
    >
      <AutoResizeCanvas height={260} setup={setup} />
      <DemoControls>
        <MiniSlider label="α" value={alphaDeg} min={0} max={90} step={1}
          format={v => v.toFixed(0) + '°'} onChange={setAlphaDeg} />
        <MiniReadout label="cos²α" value={frac.toFixed(3)} />
        <MiniReadout label="loss"
