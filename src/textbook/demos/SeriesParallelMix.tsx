...
export function SeriesParallelMixDemo({ figure }: Props) {
  ...
  const setup = useSimLoop(
    ...
    ({ ctx, w, h, dpr, colors }, _state, _dt, simTime) => {
      ...
      drawLabel(ctx, {
        text: `${labelTopology} — Kirchhoff in action`,
        x: w / 2,
        y: h - 14,
  ...
  return (
    ...
    >
      ...
      <DemoControls>
        {TOPOLOGIES.map((tp) => (
          <MiniToggle
            key={tp.id}
            label={tp.label}
            checked={topology === tp.id}
            onChange={() => setTopology(tp.id)}
          />
        ))}
        <MiniSlider
          label="R₁"
      ...
      <EquationStrip
        ...
        left={<M tex={topologyMeta.symbolic} />}
        rightLabel="Live substitution"
        right={<M tex={substitutedExpr} />}
      />
    </Demo>
  );
}
...
function drawProbesAndLabels(
  ...
  switch (topology) {
    case 'series-parallel': {
      ...
      drawLabel(ctx, {
        text: `I₂ = ${net.branches[0]!.value.toFixed(2)} A`,
        x: xR23,
        y: L.yTop + 14,
        font: '10px "JetBrains Mono", monospace',
        align: 'center',
      ...
      drawLabel(ctx, {
        text: `I₃ = ${net.branches[1]!.value.toFixed(2)} A`,
        x: xR23,
        y: branchY + 14,
        font: '10px "JetBrains Mono", monospace',
        align: 'center',
    ...
    case 'three-parallel': {
      ...
      drawLabel(ctx, {
        text: `I₃ = ${net.branches[2]!.value.toFixed(2)} A`,
        x: xR1,
        y: yR3 + 14,
        font: '10px "JetBrains Mono", monospace',
        align: 'center',
