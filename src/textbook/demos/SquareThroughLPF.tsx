...
export function SquareThroughLPFDemo() {
  ...
  useEffect(() => { stateRef.current = { fcRatio }; }, [fcRatio]);
  ...
  return (
    <Demo
      ...
      caption={
        <>
          Each harmonic is scaled by 1/√(1 + (f/f_c)²) and phase-shifted by −arctan(f/f_c). As f_c drops below f₀ the
          fundamental survives, but every odd harmonic above it is cut hard — the square turns into a softened, slewed
          waveform. Drag f_c past 10·f₀ and you recover the corners; drop below 1 and you see roughly a sine at the
          fundamental.
        </>
      }
    >
      ...
      <DemoControls>
        <MiniSlider
          label="f_c / f₀"
          value={fcRatio} min={0.3} max={20} step={0.1}
          format={v => v.toFixed(1)}
          onChange={setFcRatio}
        />
        <MiniReadout
          label="|H| at fundamental"
          value={Hfund.toFixed(3)}
        />
        <MiniReadout
