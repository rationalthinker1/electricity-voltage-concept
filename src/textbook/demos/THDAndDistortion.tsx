...
export function THDAndDistortionDemo({ figure }: Props) {
  ...
  return (
    <Demo
      ...
      caption={
        <>
          Soft clipping at A_clip ≥ 1 leaves the sine untouched — THD is essentially zero. Drive
          harder and the tips flatten; the waveform develops only <em>odd</em> harmonics because the
          clipping is symmetric. THD is the industry-standard scalar: RMS of harmonics above the
          fundamental, divided by the fundamental's RMS. A guitar amp's overdrive sits in the 10–30%
    ...
    >
      ...
      <EquationStrip
        ...
        left={<M tex={`\\text{THD} = \\frac{\\sqrt{b_2^2 + b_3^2 + \\cdots}}{b_1}`} />}
        rightLabel={`Clip level = ${Aclip.toFixed(2)}`}
        right={
          <M
            tex={`\\text{THD} = \\frac{\\sqrt{\\sum_{n\\geq2}b_n^2}}{${coeffs[0].toFixed(3)}} = ${thdPct.toFixed(1)}\\%`}
          />
        }
      />
    </Demo>
