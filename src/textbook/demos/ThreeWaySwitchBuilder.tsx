...
function diagnose(wires: Wire[]): { mistakes: string[]; badWireIds: Set<string> } {
  ...
  const s1tToS2t: Wire[] = wires.filter(
    (w) =>
  ...
  for (const w of wires) {
    ...
    const b = w.to;
    if (
      (isNeutralTerminal(a) && isSwitchTerminal(b)) ||
      (isNeutralTerminal(b) && isSwitchTerminal(a))
    ) {
      if (
        !mistakes.includes(
          'Switches must interrupt the hot only — neutral goes straight to the bulb.',
        )
      ) {
...
export function ThreeWaySwitchBuilderDemo({ figure }: Props) {
    ...
    setInProgressFrom((prev) => {
      ...
      setWires((ws) => {
        const exists = ws.some(
          (w) => (w.from === prev && w.to === t) || (w.from === t && w.to === prev),
        );
        if (exists) return ws;
  ...
  return (
    <Demo
      ...
      caption={
        <>
          Click any terminal to start a wire, then click another terminal to finish it. Click a wire
          to delete it. Click either switch's body to flip its position. The bulb lights when there
          is a closed path from <strong>L-hot</strong> through both switches to{' '}
          <strong>B-hot</strong> and a separate path from <strong>L-neu</strong> to{' '}
          <strong>B-neu</strong>. If you wire something obviously wrong — both travellers landing on
