...
export function TwoSpeedsDemo({ figure }: Props) {
  ...
  const setup = useCallback((info: CanvasInfo) => {
    ...
    function draw(now: number) {
      ...
      function drawTrack(y: number, label: string, color: string) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(trackLeft, y); ctx.lineTo(trackRight, y); ctx.stroke();
  ...
  return (
    <Demo
      ...
      caption={<>
        Two tracks the same physical length (20 cm). The blue dot is one electron drifting at the actual <em>v_d</em> for 1 A through 2.5 mm² of copper. The amber pulse is the electromagnetic signal in the wire, looping once per second on screen because in real time it makes that trip in about one nanosecond. The ratio of speeds is roughly <strong>10¹³</strong>.
      </>}
